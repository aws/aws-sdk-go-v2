...
func marshalListEnumGoCode(s *Shape) string {
	s.API.imports["github.com/aws/aws-sdk-go-v2/private/protocol"] = true
	w := &bytes.Buffer{}
	if err := marshalEnumTmpl.ExecuteTemplate(w, "list enum defs", s); err != nil {
		panic(fmt.Sprintf("failed to render shape's fields marshaler, %v", err))
	}
...
{{ define "marshaler" -}}
	{{- if $.IsShapeType "list" -}}
		...
		{{- if (and $helperName (not $.Ref.Shape.NestedShape.IsEnum)) -}}
			{{ $helperName }}
		{{- else -}}
