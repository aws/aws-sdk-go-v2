...
func EncodeStringList(vs []string) func(ListEncoder) {
	return func(le ListEncoder) {
		for _, v := range vs {
			le.ListAddValue(QuotedValue{StringValue(v)})
		}
	}
}
...
func EncodeStringMap(vs map[string]string) func(MapEncoder) {
	return func(me MapEncoder) {
		for k, v := range vs {
			me.MapSetValue(k, QuotedValue{StringValue(v)})
		}
	}
}
...
// EncodeBytesList returns a function that will add list byte slices to
// a list encoder
func EncodeBytesList(vs [][]byte) func(ListEncoder) {
	return func(le ListEncoder) {
		for _, v := range vs {
			le.ListAddValue(BytesValue(v))
		}
	}
}
