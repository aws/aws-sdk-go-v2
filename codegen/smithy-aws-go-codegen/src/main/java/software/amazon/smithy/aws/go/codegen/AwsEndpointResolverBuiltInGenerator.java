                    ...
                    .forEach(parameter -> {
                        ...
                        if (parameter.getBuiltIn().get().equals("SDK::Endpoint")) {
                            template =
                                ...
                                $L $P
                                """;
                            docs = goDocTemplate("Base endpoint that can potentially be modified during Endpoint resolution.");
                            ...
                            paramSymbol = SymbolUtils.createValueSymbolBuilder("FIPSEndpointState", AwsGoDependency.AWS_CORE).build();
                        } else if (parameter.getBuiltIn().get().equals("AWS::UseDualStack")) {
                            docs = goDocTemplate("Sourced BuiltIn value in a historical enabled or disabled state.");
                            paramSymbol = SymbolUtils.createValueSymbolBuilder("DualStackEndpointState", AwsGoDependency.AWS_CORE).build();
                        } else {
                            docs = parameter.getDocumentation().isPresent() ? goDocTemplate(parameter.getDocumentation().get()) : goDocTemplate("");
                            paramSymbol = parameterAsSymbol(parameter);
                        }
