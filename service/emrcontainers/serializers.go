...
type awsRestjson1_serializeOpUpdateVirtualCluster struct {
}

func (*awsRestjson1_serializeOpUpdateVirtualCluster) ID() string {
	return "OperationSerializer"
}

func (m *awsRestjson1_serializeOpUpdateVirtualCluster) HandleSerialize(ctx context.Context, in middleware.SerializeInput, next middleware.SerializeHandler) (
	out middleware.SerializeOutput, metadata middleware.Metadata, err error,
) {
	_, span := tracing.StartSpan(ctx, "OperationSerializer")
	endTimer := startMetricTimer(ctx, "client.call.serialization_duration")
	...
	if err := awsRestjson1_serializeOpHttpBindingsUpdateVirtualClusterInput(input, restEncoder); err != nil {
		return out, metadata, &smithy.SerializationError{Err: err}
	}
	...
	jsonEncoder := smithyjson.NewEncoder()
	if err := awsRestjson1_serializeOpDocumentUpdateVirtualClusterInput(input, jsonEncoder.Value); err != nil {
		return out, metadata, &smithy.SerializationError{Err: err}
	}
...
func awsRestjson1_serializeOpHttpBindingsUpdateVirtualClusterInput(v *UpdateVirtualClusterInput, encoder *httpbinding.Encoder) error {
	if v == nil {
		return fmt.Errorf("unsupported serialization of nil %T", v)
	}
...
func awsRestjson1_serializeOpDocumentUpdateVirtualClusterInput(v *UpdateVirtualClusterInput, value smithyjson.Value) error {
	object := value.Object()
	defer object.Close()
...
func awsRestjson1_serializeDocumentSchedulerConfiguration(v *types.SchedulerConfiguration, value smithyjson.Value) error {
	...
	if v.MaxConcurrentJobRuns != nil {
		ok := object.Key("maxConcurrentJobRuns")
		ok.Integer(*v.MaxConcurrentJobRuns)
	}

	if v.MaxInQueueJobRuns != nil {
		ok := object.Key("maxInQueueJobRuns")
		ok.Integer(*v.MaxInQueueJobRuns)
	}
