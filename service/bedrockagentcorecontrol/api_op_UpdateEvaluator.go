...
// modification.
func (c *Client) UpdateEvaluator(ctx context.Context, params *UpdateEvaluatorInput, optFns ...func(*Options)) (*UpdateEvaluatorOutput, error) {
	if params == nil { params = &UpdateEvaluatorInput{} }
	
	result, metadata, err := c.invokeOperation(ctx, "UpdateEvaluator", params, optFns, c.addOperationUpdateEvaluatorMiddlewares)
	if err != nil { return nil, err }
	
	out := result.(*UpdateEvaluatorOutput)
...
func (c *Client) addOperationUpdateEvaluatorMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = addIdempotencyToken_opUpdateEvaluatorMiddleware(stack, options); err != nil {
	return err
	}
	if err = addOpUpdateEvaluatorValidationMiddleware(stack); err != nil {
	return err
	}
	if err = stack.Initialize.Add(newServiceMetadataMiddleware_opUpdateEvaluator(options.Region, ), middleware.Before); err != nil {
	return err
	}
...
type idempotencyToken_initializeOpUpdateEvaluator struct {
	tokenProvider IdempotencyTokenProvider
}

func (*idempotencyToken_initializeOpUpdateEvaluator) ID() string {
	return "OperationIdempotencyTokenAutoFill"
}

func (m *idempotencyToken_initializeOpUpdateEvaluator) HandleInitialize(ctx context.Context, in middleware.InitializeInput, next middleware.InitializeHandler) (
	...
	if !ok { return out, metadata, fmt.Errorf("expected middleware input to be of type *UpdateEvaluatorInput ")}
...
func addIdempotencyToken_opUpdateEvaluatorMiddleware(stack *middleware.Stack, cfg Options) error {
	return stack.Initialize.Add(&idempotencyToken_initializeOpUpdateEvaluator{tokenProvider: cfg.IdempotencyTokenProvider}, middleware.Before)
}

func newServiceMetadataMiddleware_opUpdateEvaluator(region string) *awsmiddleware.RegisterServiceMetadata {
