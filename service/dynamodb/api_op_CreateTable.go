...
// CreateTable request, DynamoDB immediately returns a response with a TableStatus
// of CREATING . After the table is created, DynamoDB sets the TableStatus  to
// ACTIVE . You can perform read and write operations only on an ACTIVE table.
// You can optionally define secondary indexes on the new table, as part of the
// CreateTableoperation. If you want to create multiple tables with secondary
// indexes on them, you must create the tables sequentially. Only one table with
// secondary indexes can be in the CREATING state at any given time. You can use
// the DescribeTable  action to check the table status.
...
type CreateTableInput struct {
	...
	// hash attribute. The term "hash attribute" derives from the DynamoDB usage of an
	...
	// the sort key value. For a simple primary key (partition key), you must provide
	// exactly one element with a KeyType  of HASH. For a composite primary key
	// (partition key and sort key), you must provide exactly two elements, in this
	// order: The first element must have a KeyType  of HASH, and the second element
	// must have a KeyType  of RANGE . For more information, see Working with Tables (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/WorkingWithTables.html#WorkingWithTables.primary.key)
