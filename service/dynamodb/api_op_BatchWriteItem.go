...
// you use the UpdateItem  action. The individual PutItem  and DeleteItem
// operations specified in BatchWriteItem  are atomic; however BatchWriteItem as
// a whole is not. If any requested operations fail because the table's provisioned
...
// BatchWriteItemin a loop. Each iteration would check for unprocessed items and
// submit a new BatchWriteItem request with those unprocessed items until all
// items have been processed. If none of the items can be processed due to
// insufficient provisioned throughput on all of the tables in the request, then
// BatchWriteItem returns a ProvisionedThroughputExceededException. If DynamoDB
// returns any unprocessed items, you should retry the batch operation on those
// items. However, we strongly recommend that you use an exponential backoff
// algorithm. If you retry the batch operation immediately, the underlying read or
// write requests can still fail due to throttling on the individual tables. If you
// delay the batch operation using exponential backoff, the individual requests in
// the batch are much more likely to succeed. For more information, see Batch
// Operations and Error Handling (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ErrorHandling.html#Programming.Errors.BatchOperations)
...
type BatchWriteItemOutput struct {
	...
	// UnprocessedItemsentry consists of a table name and, for that table, a list of
