...
// sort key value and a comparison operator in KeyConditionExpression. To further
// refine the Query  results, you can optionally provide a FilterExpression . A
// FilterExpressiondetermines which items within the results should be returned
// to you. All of the other results are discarded. A Query operation always
// returns a result set. If no matching items are found, the result set will be
// empty. Queries that do not return results consume the minimum number of read
// capacity units for that type of read operation. DynamoDB calculates the number
// of read capacity units consumed based on item size, not on the amount of data
// that is returned to an application. The number of capacity units consumed will
// be the same whether you request all of the attributes (the default behavior) or
// just some of them (using a projection expression). The number will also be the
// same whether or not you use a FilterExpression . Query results are always
// sorted by the sort key value. If the data type of the sort key is Number, the
// results are returned in numeric order; otherwise, the results are returned in
// order of UTF-8 bytes. By default, the sort order is ascending. To reverse the
// order, set the ScanIndexForward  parameter to false. A single Query operation
// will read up to the maximum number of items set (if using the Limit parameter)
// or a maximum of 1 MB of data and then apply any filtering to the results using
...
type QueryInput struct {
	...
	// ":disc":{"S":"Discontinued"} }You could then use these values in an
	// expression, such as this: ProductStatus IN (:avail, :back, :disc) For more
	// information on expression attribute values, see Specifying Conditions (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.SpecifyingConditions.html)
	// in the Amazon DynamoDB Developer Guide.
	...
	// Queryaction. The condition must perform an equality test on a single partition
	// key value. The condition can optionally perform one of several comparison tests
	// on a single sort key value. This allows Query to retrieve one item with a
	// given partition key value and sort key value, or several items that have the
	// same partition key value but different sort key values. The partition key
	// equality test is required, and must be specified in the following format:
	// partitionKeyName = :partitionkeyval If you also want to provide a condition
	// for the sort key, it must be combined using AND with the condition for the
	// sort key. Following is an example, using the = comparison operator for the sort
	...
	// example, the following KeyConditionExpression parameter causes an error
	...
	// ExpressionAttributeNames and ExpressionAttributeValues , see Using
	// Placeholders for Attribute Names and Values (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ExpressionPlaceholders.html)
	// in the Amazon DynamoDB Developer Guide.
	...
	// LastEvaluatedKeyto apply in a subsequent operation to continue the operation.
...
type QueryOutput struct {
	...
	// if the ReturnConsumedCapacity parameter was specified. For more information,
	...
	// ScannedCountis the number of matching items before the filter was applied. If
