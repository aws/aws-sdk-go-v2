...
// same item. The aggregate size of the items in the transaction cannot exceed 4
// MB. The actions are completed atomically so that either all of them succeed, or
...
type TransactWriteItemsInput struct {
	...
	// contains a ConditionCheck , Put , Update , or Delete object. These can operate
	// on items in different tables, but the tables must reside in the same Amazon Web
	...
	// single call. Although multiple identical calls using the same client request
	// token produce the same result on the server (no side effects), the responses to
	// the calls might not be the same. If the ReturnConsumedCapacity parameter is
	// set, then the initial TransactWriteItems call returns the amount of write
	...
	// with the same client token is treated as a new request. Do not resubmit the same
	// request with the same client token for more than 10 minutes, or the result might
	// not be idempotent. If you submit a request with the same client token but a
	// change in other parameters within the 10-minute idempotency window, DynamoDB
