...
// results using FilterExpression . If LastEvaluatedKey is present in the
...
// applications can request a parallel Scan  operation by providing the Segment
// and TotalSegments  parameters. For more information, see Parallel Scan (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.ParallelScan)
...
type ScanInput struct {
	...
	// information on expression attribute values, see Condition Expressions (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.SpecifyingConditions.html)
	// in the Amazon DynamoDB Developer Guide.
	...
	//
	// FilterExpression criteria are not returned. A FilterExpression is applied
	// after the items have already been read; the process of filtering does not
	// consume any additional read capacity units. For more information, see Filter
	// Expressions (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/QueryAndScan.html#Query.FilterExpression)
	// in the Amazon DynamoDB Developer Guide.
	...
	// scan a table or an index, then the first thread specifies a Segment value of
	// 0, the second thread specifies 1, and so on. The value of LastEvaluatedKey
	// returned from a parallel Scan  request must be used as ExclusiveStartKey with
	// the same segment ID in a subsequent Scan  operation. The value for Segment
	...
	// TotalSegmentscorresponds to the number of application workers that will
	// perform the parallel scan. For example, if you want to use four application
	// threads to scan a table or an index, specify a TotalSegments value of 4. The
	// value for TotalSegments must be greater than or equal to 1, and less than or
...
type ScanOutput struct {
	...
	// see Provisioned Throughput (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ProvisionedThroughput.html#ItemSizeCalculations.Reads)
	// in the Amazon DynamoDB Developer Guide.
	...
	// you did not use a filter in the request, then Count  is the same as
	// ScannedCount .
	Count int32
	...
	// Scan operation. For more information, see Count and ScannedCount (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/QueryAndScan.html#Count)
