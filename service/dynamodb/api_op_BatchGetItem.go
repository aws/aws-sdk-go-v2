...
// for UnprocessedKeys . You can use this value to retry the operation starting
...
// exceed the 16 MB limit). It also returns an appropriate UnprocessedKeys value
// so you can get the next page of results. If desired, your application can
// include its own logic to assemble the pages of results into one dataset. If none
// of the items can be processed due to insufficient provisioned throughput on all
...
// Error Handling (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ErrorHandling.html#BatchOperations)
