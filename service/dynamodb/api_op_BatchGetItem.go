...
// operation starting with the next item to get. If you request more than 100
// items, BatchGetItem  returns a ValidationException with the message "Too many
// items requested for the BatchGetItem call." For example, if you ask to retrieve
...
// UnprocessedKeysvalue so you can get the next page of results. If desired, your
// application can include its own logic to assemble the pages of results into one
// dataset. If none of the items can be processed due to insufficient provisioned
...
// are much more likely to succeed. For more information, see Batch Operations
// and Error Handling (https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ErrorHandling.html#BatchOperations)
...
// ProjectionExpressionparameter. If a requested item does not exist, it is not
// returned in the result. Requests for nonexistent items consume the minimum read
// capacity units according to the type of read. For more information, see
...
type BatchGetItemInput struct {
	...
	// per BatchGetItem request. Each element in the map of items to retrieve
