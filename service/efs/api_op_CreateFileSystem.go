...
//
// Otherwise, this
// operation returns a FileSystemAlreadyExists error with the ID of the existing
// file system. For basic use cases, you can use a randomly generated UUID for the
// creation token. The idempotent operation allows you to retry a CreateFileSystem
...
// can learn of its existence from the FileSystemAlreadyExists error. For more
// information, see Creating a file system (https://docs.aws.amazon.com/efs/latest/ug/creating-using-create-fs.html#creating-using-create-fs-part1)
...
// creation status by calling the DescribeFileSystems operation, which among
// other things returns the file system state. This operation accepts an optional
...
// maxIOperformance mode can scale to higher levels of aggregate throughput and
...
// for the file system in your VPC. For more information, see CreateMountTarget.
// You mount your Amazon EFS file system on an EC2 instances in your VPC by using
// the mount target. For more information, see Amazon EFS: How it Works (https://docs.aws.amazon.com/efs/latest/ug/how-it-works.html)
...
type CreateFileSystemInput struct {
	...
	// operations per second with a tradeoff of slightly higher latencies for most file
	// operations. The performance mode can't be changed after the file system has been
	// created. The maxIO mode is not supported on file systems using One Zone
	// storage classes.
	...
	// ThroughputMode is set to provisioned. The upper limit for throughput is 1024
	// MiB/s. To increase this limit, contact Amazon Web Services Support. For more
	// information, see Amazon EFS quotas that you can increase (https://docs.aws.amazon.com/efs/latest/ug/limits.html#soft-limits)
	...
	// "Key":"Name","Value":"{value}"key-value pair. Each key must be unique. For
