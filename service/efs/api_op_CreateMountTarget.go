...
//
//     * VPC in which Amazon EFS creates the
// mount target
//
//     * Availability Zone in which Amazon EFS creates the mount
// target
//
//     * IP address range from which Amazon EFS selects the IP address of
// the mount target (if you don't specify an IP address in the request)
//
// After
// creating the mount target, Amazon EFS returns a response that includes, a
...
// mount targets
//
//     * Must not be in the same Availability Zone as any of the
// subnets of the existing mount targets
//
// If the request satisfies the
// requirements, Amazon EFS does the following:
//
//     * Creates a new mount target
// in the specified subnet.
//
//     * Also creates a new network interface in the
// subnet as follows:
//
...
// private IP address).
//
//         * If the request provides SecurityGroups, this
...
//
//         * Assigns
// the description Mount target fsmt-id for file system fs-id  where  fsmt-id  is
// the mount target ID, and  fs-id  is the FileSystemId.
//
//         * Sets the
// requesterManaged property of the network interface to true, and the requesterId
// value to EFS.
//
//     Each Amazon EFS mount target has one corresponding
// requester-managed EC2 network interface. After the network interface is created,
// Amazon EFS sets the NetworkInterfaceId field in the mount target's description
// to the network interface ID, and the IpAddress field to its address. If network
// interface creation fails, the entire CreateMountTarget operation fails.
//
// The
// CreateMountTarget call returns only after creating the network interface, but
// while the mount target state is still creating, you can check the mount target
// creation status by calling the DescribeMountTargets operation, which among other
// things returns the mount target state. We recommend that you create a mount
// target in each of the Availability Zones. There are cost considerations for
// using a file system in an Availability Zone through a mount target created in
// another Availability Zone. For more information, see Amazon EFS
// (http://aws.amazon.com/efs/). In addition, by always using a mount target local
...
// you can't access your file system through that mount target. This operation
// requires permissions for the following action on the file system:
//
//     *
...
func addOperationCreateMountTargetStackSlots(stack *middleware.Stack) error {
...
func addOperationCreateMountTargetMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := addOperationCreateMountTargetStackSlots(stack); err != nil {
		return err
	}
