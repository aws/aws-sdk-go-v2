...
func (v *DeleteEnvironmentOutput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.DeleteEnvironmentResult)
	v.SerializeMembers(s)
	s.CloseStruct()
}
...
func (v *DeleteEnvironmentOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.DeleteEnvironmentResult, func(s *smithy.Schema) error {
		switch s {
		}
		return nil
	})
...
func (c *Client) addOperationDeleteEnvironmentMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteEnvironment, schemas.DeleteEnvironmentRequest, schemas.DeleteEnvironmentResult)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteEnvironment, schemas.DeleteEnvironmentRequest, schemas.DeleteEnvironmentResult), output: &DeleteEnvironmentOutput{}}, middleware.After); err != nil {
		return err
	}
