...
// AWS accounts in your organization. This information includes standard event data
// (such as the AWS Region and service), an event description, and (depending on
// the event) possible metadata. This operation doesn't return affected entities,
// such as the resources related to the event. To return affected entities, use the
...
// for a public event, you must specify a null value for the awsAccountId
// parameter. If you specify an account ID for a public event, AWS Health returns
// an error message because public events aren't specific to an account.
//
// * To
// return event details for an event that is specific to an account in your
// organization, you must specify the awsAccountId parameter in the request. If you
// don't specify an account ID, AWS Health returns an error message because the
