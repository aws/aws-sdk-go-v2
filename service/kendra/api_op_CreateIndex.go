...
// index is ready to use. Once the index is active you can index your documents
// using the BatchPutDocument API or using one of the supported data sources. For
// an example of creating an index and data source using the Python SDK, see
...
type CreateIndexInput struct {
	...
	// ENTERPRISE_EDITIONfor your production databases. Once you set the edition for
	// an index, it can't be changed. The Edition parameter is optional. If you don't
	// supply a value, the default is ENTERPRISE_EDITION. For more information on
	// quota limits for enterprise and developer editions, see Quotas (https://docs.aws.amazon.com/kendra/latest/dg/quotas.html)
	// .
	...
	// The user context policy. ATTRIBUTE_FILTER All indexed content is searchable
	// and displayable for all users. If you want to filter search results on user
	// context, you can use the attribute filters of _user_id  and _group_ids or you
	// can provide user and group information in UserContext. USER_TOKEN Enables
