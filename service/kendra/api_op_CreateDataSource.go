...
// raised. Amazon S3 and custom (https://docs.aws.amazon.com/kendra/latest/dg/data-source-custom.html)
// data sources are the only supported data sources in the Amazon Web Services
// GovCloud (US-West) region. For an example of creating an index and data source
// using the Python SDK, see Getting started with Python SDK (https://docs.aws.amazon.com/kendra/latest/dg/gs-python.html)
...
type CreateDataSourceInput struct {
	...
	// the document ingestion process. For more information on how to create, modify
	// and delete document metadata, or make other content alterations when you ingest
	...
	// Kendra (https://docs.aws.amazon.com/kendra/latest/dg/iam-roles.html). You
	// can't specify the RoleArn  parameter when the Type  parameter is set to CUSTOM
	// . If you do, you receive a ValidationException  exception. The RoleArn
	// parameter is required for all other data sources.
	RoleArn *string
	...
	// StartDataSourceSyncJob API to update the index. You can't specify the Schedule
