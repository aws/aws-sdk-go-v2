...
type GetDASHStreamingSessionURLInput struct {
	...
	// type, the MPEG-DASH manifest is continually updated with the latest fragments as
	// they become available. We recommend that the media player retrieve a new
	// manifest on a one-second interval. When this type of session is played in a
	...
	// - LIVE_REPLAY : For
	// sessions of this type, the MPEG-DASH manifest is updated similarly to how it is
	...
	// the MPEG-DASH manifest contains all the fragments for the session, up to the
	// number that is specified in MaxManifestFragmentResults. The manifest must be
	...
	//
	// In all playback modes,
	// if FragmentSelectorType is PRODUCER_TIMESTAMP, and if there are multiple
	// fragments with the same start timestamp, the fragment that has the larger
	// fragment number (that is, the newer fragment) is included in the MPEG-DASH
	// manifest. The other fragments are not included. Fragments that have different
	// timestamps but have overlapping durations are still included in the MPEG-DASH
	// manifest. This can lead to unexpected behavior in the media player. The default
	// is LIVE.
