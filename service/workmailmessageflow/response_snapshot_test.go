...
func TestCheckResponseSnapshot_GetRawMessageContent(t *testing.T) {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "GetRawMessageContent.response", err)
	}
}

func TestCheckResponseSnapshot_PutRawMessageContent(t *testing.T) {
	want := &PutRawMessageContentOutput{}
	status, header, body, err := serdeRespReadSnapshot("PutRawMessageContent.response")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "PutRawMessageContent.response", err)
	}
}

func TestCheckResponseSnapshot_Error_InvalidContentLocation(t *testing.T) {
	want := &types.InvalidContentLocation{
		Message: ptr.String("__Message__"),
	}
	status, header, body, err := serdeRespReadSnapshot("InvalidContentLocation.error")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	var got *types.InvalidContentLocation
	if !errors.As(opErr, &got) {
		t.Fatalf("expected types.InvalidContentLocation, got %v", opErr)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("error response snapshot mismatch for %s: %v", "InvalidContentLocation.error", err)
	}
}

func TestCheckResponseSnapshot_Error_MessageFrozen(t *testing.T) {
	want := &types.MessageFrozen{
		Message: ptr.String("__Message__"),
	}
	status, header, body, err := serdeRespReadSnapshot("MessageFrozen.error")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	var got *types.MessageFrozen
	if !errors.As(opErr, &got) {
		t.Fatalf("expected types.MessageFrozen, got %v", opErr)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("error response snapshot mismatch for %s: %v", "MessageFrozen.error", err)
	}
}
