...
func (v *DeleteGroupInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.ClientToken != nil {
		s.WriteString(schemas.DeleteGroupRequest_ClientToken, *v.ClientToken)
	}
	if v.DirectoryId != nil {
		s.WriteString(schemas.DeleteGroupRequest_DirectoryId, *v.DirectoryId)
	}
	if v.SAMAccountName != nil {
		s.WriteString(schemas.DeleteGroupRequest_SAMAccountName, *v.SAMAccountName)
	}
}
...
func (v *DeleteGroupOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.DeleteGroupResult, func(s *smithy.Schema) error {
		switch s {
		}
		return nil
	})
...
func (c *Client) addOperationDeleteGroupMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteGroup, schemas.DeleteGroupRequest, schemas.DeleteGroupResult)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteGroup, schemas.DeleteGroupRequest, schemas.DeleteGroupResult), output: &DeleteGroupOutput{}}, middleware.After); err != nil {
		return err
	}
