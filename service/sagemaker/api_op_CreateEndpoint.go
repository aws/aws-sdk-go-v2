...
// Endpoint example notebook. (https://github.com/aws/amazon-sagemaker-examples/blob/master/sagemaker-fundamentals/create-endpoint/create_endpoint.ipynb)
...
// and deploys the model(s) on them. When you call CreateEndpoint, a load call is
// made to DynamoDB to verify that your endpoint configuration exists. When you
...
// minimize the potential impact of a DynamoDB eventually consistent read. When
...
// use the DescribeEndpoint API. If any of the models hosted at this endpoint get
// model data from an Amazon S3 location, SageMaker uses Amazon Web Services
// Security Token Service to download model artifacts from the S3 path you
// provided. Amazon Web Services STS is activated in your Amazon Web Services
...
// want to grant access to use the CreateEndpoint  and CreateEndpointConfig API
