...
// notebook instance. In the SageMaker console, when you choose Open next to a
...
// show the page. The IAM role or user used to call this API defines the
...
// specify. Use the NotIpAddress  condition operator and the aws:SourceIP
// condition context key to specify the list of IP addresses that you want to have
// access to the notebook instance. For more information, see Limit Access to a
// Notebook Instance by IP Address (https://docs.aws.amazon.com/sagemaker/latest/dg/security_iam_id-based-policy-examples.html#nbi-ip-filter)
