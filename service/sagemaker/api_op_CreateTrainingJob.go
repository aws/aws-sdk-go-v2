...
type CreateTrainingJobInput struct {
	...
	// interrupted and when there is flexibility when the training job is run. The
	// complete and intermediate results of jobs are stored in an Amazon S3 bucket, and
	// can be used as a starting point to train models incrementally. Amazon SageMaker
	...
	// InputDataConfigdescribes the input data and its location. Algorithms can
	// accept input data from one or more channels. For example, an algorithm might
	// have two channels of input data, training_data  and validation_data. The
	// configuration for each channel provides the S3, EFS, or FSx location where the
	// input data is stored. It also provides information about the stored data: the
	// MIME type, compression method, and whether the data is wrapped in RecordIO
	// format. Depending on the input mode that the algorithm supports, SageMaker
	// either copies input data files from an S3 bucket to a local directory in the
	// Docker container, or makes it available as input streams. For example, if you
	// specify an EFS location, input data files are available as input streams. They
