...
// to provision. Then you call the CreateEndpoint API. Use this API if you want
// to use SageMaker hosting services to deploy models into production. In the
// request, you define a ProductionVariant, for each model that you want to
...
// instances to deploy. If you are hosting multiple models, you also assign a
// VariantWeightto specify how much traffic you want to allocate to each model.
// For example, suppose that you want to host two models, A and B, and you assign
// traffic weight 2 for model A and 1 for model B. SageMaker distributes two-thirds
// of the traffic to Model A, and one-third to model B. When you call
// CreateEndpoint, a load call is made to DynamoDB to verify that your endpoint
// configuration exists. When you read data from a DynamoDB table supporting
...
type CreateEndpointConfigInput struct {
	...
	// The
	// KMS key policy must grant permission to the IAM role that you specify in your
	// CreateEndpoint , UpdateEndpoint requests. For more information, refer to the
	// Amazon Web Services Key Management Service sectionUsing Key Policies in Amazon
	...
	// for the KmsKeyId  parameter. If you specify a value for KmsKeyId when using
