...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/sagemaker/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_SAGEMAKER")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "SageMaker", cfg.ConfigSources)
	if found && err == nil {
...
var bddNodes = [45]int32{
	-1, 1, -1, 0, 14, 3, 1, 4, 100000012, 2, 5, 100000012, 3, 8, 6, 4, 7, 100000011, 5, 100000009, 100000010, 4, 12, 9, 6, 10, 100000008, 7, 100000006, 11, 8, 100000006, 100000007, 5, 13, 100000005, 6, 100000004, 100000005, 3, 100000001, 15, 4, 100000002, 100000003}
...
func resolveResult(idx int32, params *EndpointParameters, c *conditionContext) (smithyendpoints.Endpoint, error) {
	...
	case 4:
		uriString := func() string {
			...
			out.WriteString("https://api.sagemaker-fips.")
