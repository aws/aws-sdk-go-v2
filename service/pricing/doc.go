...
// that compare your internal workloads with Amazon Web Services. Use GetServices
// without a service code to retrieve the service codes for all AWS services, then
// GetServiceswith a service code to retrieve the attribute names for that
// service. After you have the service code and attribute names, you can use
// GetAttributeValuesto see what values are available for an attribute. With the
// service code and an attribute name and value, you can use GetProducts to find
// specific products that you're interested in, such as an AmazonEC2 instance,
// with a Provisioned IOPS volumeType. Service Endpoint Amazon Web Services Price
// List service API provides the following two endpoints:
