...
func (c *Client) addOperationGetGraphMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = stack.Initialize.Add(newServiceMetadataMiddleware(options.Region, "GetGraph", options.EndpointResolver != nil), middleware.Before); err != nil {
		return err
	}
...
// GraphStoppedWaiterOptions are waiter options for GraphStoppedWaiter
type GraphStoppedWaiterOptions struct {
	...
	// MinDelay is the minimum amount of time to delay between retries. If unset,
	// GraphStoppedWaiter will use default minimum delay of 20 seconds. Note that
	// MinDelay must resolve to a value lesser than or equal to the MaxDelay.
	MinDelay time.Duration

	// MaxDelay is the maximum amount of time to delay between retries. If unset or
	// set to zero, GraphStoppedWaiter will use default max delay of 1800 seconds. Note
	// that MaxDelay must resolve to value greater than or equal to the MinDelay.
	MaxDelay time.Duration
...
// GraphStoppedWaiter defines the waiters for GraphStopped
type GraphStoppedWaiter struct {
	client GetGraphAPIClient

	options GraphStoppedWaiterOptions
}

// NewGraphStoppedWaiter constructs a GraphStoppedWaiter.
func NewGraphStoppedWaiter(client GetGraphAPIClient, optFns ...func(*GraphStoppedWaiterOptions)) *GraphStoppedWaiter {
	options := GraphStoppedWaiterOptions{}
	options.MinDelay = 20 * time.Second
	options.MaxDelay = 1800 * time.Second
	options.Retryable = graphStoppedStateRetryable

	for _, fn := range optFns {
		fn(&options)
	}
	return &GraphStoppedWaiter{
		client:  client,
		options: options,
	}
}

// Wait calls the waiter function for GraphStopped waiter. The maxWaitDur is the
// maximum wait duration the waiter will wait. The maxWaitDur is required and must
// be greater than zero.
func (w *GraphStoppedWaiter) Wait(ctx context.Context, params *GetGraphInput, maxWaitDur time.Duration, optFns ...func(*GraphStoppedWaiterOptions)) error {
	_, err := w.WaitForOutput(ctx, params, maxWaitDur, optFns...)
	return err
}

// WaitForOutput calls the waiter function for GraphStopped waiter and returns the
// output of the successful operation. The maxWaitDur is the maximum wait duration
// the waiter will wait. The maxWaitDur is required and must be greater than zero.
func (w *GraphStoppedWaiter) WaitForOutput(ctx context.Context, params *GetGraphInput, maxWaitDur time.Duration, optFns ...func(*GraphStoppedWaiterOptions)) (*GetGraphOutput, error) {
	if maxWaitDur <= 0 {
		return nil, fmt.Errorf("maximum wait time for waiter must be greater than zero")
	}
	...
	return nil, fmt.Errorf("exceeded max wait time for GraphStopped waiter")
}

func graphStoppedStateRetryable(ctx context.Context, input *GetGraphInput, output *GetGraphOutput, err error) (bool, error) {
