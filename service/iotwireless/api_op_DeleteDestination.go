...
func (v *DeleteDestinationOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.DeleteDestinationResponse, func(s *smithy.Schema) error {
		switch s {
		}
		return nil
	})
...
func (c *Client) addOperationDeleteDestinationMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteDestination, schemas.DeleteDestinationRequest, schemas.DeleteDestinationResponse)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteDestination, schemas.DeleteDestinationRequest, schemas.DeleteDestinationResponse), output: &DeleteDestinationOutput{}}, middleware.After); err != nil {
		return err
	}
