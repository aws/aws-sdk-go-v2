...
// set of deployment rules and deployment success and failure conditions used by
// CodeDeploy during a deployment.
// - Deployment: The process and the components
// used when updating a Lambda function, a containerized application in an Amazon
// ECS service, or of installing content on one or more instances.
// - Application
// revisions: For an Lambda deployment, this is an AppSpec file that specifies the
// Lambda function to be updated and one or more functions to validate deployment
// lifecycle events. For an Amazon ECS deployment, this is an AppSpec file that
// specifies the Amazon ECS task definition, container, and port where production
// traffic is rerouted. For an EC2/On-premises deployment, this is an archive file
// that contains source content—source code, webpages, executable files, and
// deployment scripts—along with an AppSpec file. Revisions are stored in Amazon S3
// buckets or GitHub repositories. For Amazon S3, a revision is uniquely identified
// by its Amazon S3 object key and its ETag, version, or both. For GitHub, a
// revision is uniquely identified by its commit ID.
//
// This guide also contains
// information to help you get details about the instances in your deployments, to
// make on-premises instances available for CodeDeploy deployments, to get details
// about a Lambda function deployment, and to get details about Amazon ECS service
// deployments. CodeDeploy Information Resources
