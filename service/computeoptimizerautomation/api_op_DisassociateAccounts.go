...
func (c *Client) addOperationDisassociateAccountsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = addIdempotencyToken_opDisassociateAccountsMiddleware(stack, options); err != nil {
		return err
	}
	if err = addOpDisassociateAccountsValidationMiddleware(stack); err != nil {
		return err
	}
	if err = stack.Initialize.Add(newServiceMetadataMiddleware(options.Region, "DisassociateAccounts", options.EndpointResolver != nil), middleware.Before); err != nil {
		return err
	}
...
type idempotencyToken_initializeOpDisassociateAccounts struct {
	tokenProvider IdempotencyTokenProvider
}

func (*idempotencyToken_initializeOpDisassociateAccounts) ID() string {
	return "OperationIdempotencyTokenAutoFill"
}

func (m *idempotencyToken_initializeOpDisassociateAccounts) HandleInitialize(ctx context.Context, in middleware.InitializeInput, next middleware.InitializeHandler) (
	...
	if !ok {
		return out, metadata, fmt.Errorf("expected middleware input to be of type *DisassociateAccountsInput ")
	}
...
func addIdempotencyToken_opDisassociateAccountsMiddleware(stack *middleware.Stack, cfg Options) error {
	return stack.Initialize.Add(&idempotencyToken_initializeOpDisassociateAccounts{tokenProvider: cfg.IdempotencyTokenProvider}, middleware.Before)
}
