...
func (v *ListAutomationEventsOutput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListAutomationEventsResponse)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListAutomationEventsOutput) SerializeMembers(s smithy.ShapeSerializer) {
	serializeAutomationEvents(s, schemas.ListAutomationEventsResponse_automationEvents, v.AutomationEvents)
	if v.NextToken != nil {
		s.WriteString(schemas.ListAutomationEventsResponse_nextToken, *v.NextToken)
	}
}
...
func (c *Client) addOperationListAutomationEventsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = stack.Initialize.Add(newServiceMetadataMiddleware(options.Region, "ListAutomationEvents"), middleware.Before); err != nil {
		return err
	}
