...
// empty, you can either set Artwork  to null, or set the Merge Policy to
// "Replace" and use an empty Artwork array. To pass through existing artwork
// unchanged, set the Merge Policy to "Prepend", "Append", or "Fallback", and use
// an empty Artwork  array.
type Artwork struct {
	...
	// the top and bottom and/or left and right sides of the output album art to make
...
type AudioCodecOptions struct {
	...
	// audio, but the larger the file. Valid values are 16  and 24. The most common
	// bit depth is 24 .
	BitDepth *string
...
type AudioParameters struct {
	...
	// single-channel tracks. The outputs of OneChannelPerTrack for a specific
	...
	// contain audio data from an input channel are MOS, or Mit Out Sound, tracks. The
	// outputs of OneChannelPerTrackWithMosTo8Tracks for a specific channel value and
	...
	// output file, select auto. The output of a specific channel value and inputs
	// are as follows:
	...
	// use. Valid values include: auto , AAC-LC , HE-AAC , HE-AACv2  If you specify
	// auto, Elastic Transcoder chooses a profile based on the bit rate of the output
	// file.
...
type CaptionSource struct {
	...
	// For more information on ISO language codes
	// and language names, see the List of ISO 639-1 codes.
	Language *string
...
type CreateJobOutput struct {
	...
	// CaptionSourcesarray. For more information on embedded files, see the Subtitles
	...
	// seconds. For HLSv3 format playlists, each media segment is stored in a
	// separate .ts  file. For HLSv4  and Smooth playlists, all media segments for an
...
type CreateJobPlaylist struct {
	...
	// must be unique. Elastic Transcoder automatically appends the relevant file
	// extension to the file name ( .m3u8  for HLSv3  and HLSv4  playlists, and .ism
	// and .ismc  for Smooth  playlists). If you include a file extension in Name,
	...
	// Video:Profile , and Video:FrameRate  to Video:KeyframesMaxDist ratio must be
...
type Encryption struct {
	...
	// base64-encoded: 128 , 192 , or 256. The key must also be encrypted by using
	// the Amazon Key Management Service.
	Key *string
	...
	// For the AES modes, your private encryption keys
	// and your unencrypted data are never stored by AWS; therefore, it is important
	// that you safely manage your encryption keys. If you lose them, you won't be able
...
type HlsContentProtection struct {
	...
	// field blank. The series of random bits created by a random bit generator, unique
	// for every encryption operation, that you want Elastic Transcoder to use to
	// encrypt your output files. The initialization vector must be base64-encoded, and
	...
	// If Elastic Transcoder is generating your key for you, you must leave this
	// field blank. The MD5 digest of the key that you want Elastic Transcoder to use
	// to encrypt your output file, and that you want Elastic Transcoder to use as a
...
type Job struct {
	...
	// key/value pairs per job. Elastic Transcoder does not guarantee that key/value
	// pairs are returned in the same order in which you specify them. Metadata keys
	// and values  must use characters from the following list:
	//     - 0-9
	//     - A-Z and a-z
	//     - Space
...
// Outputobject lists information about the first output. This duplicates the
// information that is listed for the first output in the Outputs  object.
type JobOutput struct {
	...
	// conversion used. If no ColorSpaceConversionMode was defined in the preset,
	...
	// (MPEG-TS), SegmentDuration is the target maximum duration of each segment in
	// seconds. For HLSv3 format playlists, each media segment is stored in a
	// separate .ts  file. For HLSv4 , MPEG-DASH , and Smooth playlists, all media
	...
	// might be shorter or longer. The range of valid values is 1 to 60 seconds. If the
	// duration of the video is not evenly divisible by SegmentDuration, the duration
	// of the last segment is the remainder of total length/SegmentDuration. Elastic
	// Transcoder creates an output-specific playlist for each output HLS output that
	// you specify in OutputKeys. To add an output to the master playlist for this job,
	...
	// specify in Preset for the current output. Watermarks are added to the output
	// video in the sequence in which you list them in the job output—the first
	// watermark in the list is added to the output video first, the second watermark
	// in the list is added next, and so on. As a result, if the settings in a preset
	// cause Elastic Transcoder to place all watermarks in the same location, the
	// second watermark that you add covers the first one, the third one covers the
...
type Pipeline struct {
	...
	// pipeline. If you use either s3  or s3-aws-kms  as your Encryption:Mode, you
	// don't need to provide a key with your job because a default key, known as an
	// AWS-KMS key, is created for you automatically. You need to provide an AWS-KMS
	// key only if you want to use a non-default AWS-KMS key, or if you are using an
...
type PipelineOutputConfig struct {
	...
	// of 30 users and/or predefined Amazon S3 groups. If you include Permissions,
...
type PresetWatermark struct {
	...
	// is Left  or Right. If you specify an offset that causes the watermark to
	...
	// the black bars, it is cropped. Use the value of Target to specify whether you
	// want to include the black bars that are added by Elastic Transcoder, if any, in
	...
	//     - integer percentage (%): The range of valid values is 0 to 100.
	// For
	// example, if you specify Top  for VerticalAlign  and 5px  for VerticalOffset,
...
type Thumbnails struct {
	...
	// mutually exclusive. Do not use them together. The aspect ratio of thumbnails.
	// Valid values include: auto , 1:1 , 4:3 , 3:2 , 16:9  If you specify auto,
	// Elastic Transcoder tries to preserve the aspect ratio of the video in the output
	// file.
	AspectRatio *string
	...
	// the top and bottom and/or left and right sides of thumbnails to make the total
	// size of the thumbnails match the values that you specified for thumbnail
	...
	// you use the values MaxWidth , MaxHeight , SizingPolicy , and PaddingPolicy
	// instead of Resolution  and AspectRatio. The two groups of settings are
	// mutually exclusive. Do not use them together. The width and height of thumbnail
	// files in pixels. Specify a value in the format width  x  height  where both
	// values are even integers. The values cannot exceed the width and height that you
...
type TimeSpan struct {
	...
	// output file from StartTime to the end of the file. If you specify a value longer
...
type VideoParameters struct {
	...
	// of settings are mutually exclusive. Do not use them together. The display aspect
	// ratio of the video in the output file. Valid values include: auto , 1:1 , 4:3 ,
	// 3:2 , 16:9  If you specify auto, Elastic Transcoder tries to preserve the
	// aspect ratio of the input file. If you specify an aspect ratio for the output
	// file that differs from aspect ratio of the input file, Elastic Transcoder adds
	...
	// values depend on the values of Level  and Profile . If you specify auto,
	...
	// profile: Level - Maximum video bit rate in kilobits/second (baseline and main
	...
	// , 2.1 , 2.2 , 3 , 3.1 , 3.2 , 4 , 4.1 MaxReferenceFrames (H.264 Only)
	...
	//     - 4 - 32768
	//     - 4.1 - 32768
	// MaxBitRate (Optional, H.264/MPEG2/VP8/VP9 only) The maximum
	// number of bits per second in a video buffer; the size of the buffer is specified
	// by BufferSize. Specify a value between 16 and 62,500. You can reduce the
	// bandwidth required to stream a video by reducing the maximum bit rate, but this
	// also reduces the quality of the video. BufferSize (Optional, H.264/MPEG2/VP8/VP9
	...
	// InterlacedMode (Optional, H.264/MPEG2 Only) The interlace mode for the output
	// video. Interlaced video is used to double the perceived frame rate for a video
	// by interlacing two fields (one field on every other line, the other field on the
	...
	// result in blurred images and flickering. Valid values include Progressive (no
	// interlacing, top to bottom), TopFirst  (top field first), BottomFirst (bottom
	...
	// Bt.709 is the standard for high definition video. Valid values include None ,
	// Bt709toBt601 , Bt601toBt709 , and Auto . If you chose Auto  for
	// ColorSpaceConversionModeand your output is interlaced, your frame rate is one
	// of 23.97 , 24 , 25 , 29.97 , 50 , or 60 , your SegmentDuration is null, and
	// you are using one of the resolution changes from the list below, Elastic
	...
	// ColorSpaceConversionModewas applied. ChromaSubsampling The sampling pattern
	// for the chroma (color) channels of the output video. Valid values include
	// yuv420p and yuv422p . yuv420p samples the chroma information of every other
	// horizontal and every other vertical line, yuv422p samples the color
	...
	// include: auto , 10 , 15 , 23.97 , 24 , 25 , 29.97 , 30 , 60  If you specify
	// auto, Elastic Transcoder uses the detected frame rate of the input source. If
	// you specify a frame rate, we recommend that you perform the following
	// calculation: Frame rate = maximum recommended decoding speed in luma
	// samples/second / (width in pixels * height in pixels) where:
	...
	// needs to have a ratio of 1:3. Valid outputs would have FrameRate of 30, 25,
	// and 10, and KeyframesMaxDist of 90, 75, and 30, respectively. Alternately,
	// this can be achieved by setting FrameRate to auto and having the same values
	...
	// When you set PaddingPolicy  to Pad, Elastic Transcoder may add black bars to
	// the top and bottom and/or left and right sides of the output video to make the
	// total size of the output video match the values that you specified for MaxWidth
	// and MaxHeight .
	PaddingPolicy *string
	...
	// that you use the values MaxWidth , MaxHeight , SizingPolicy , PaddingPolicy,
	// and DisplayAspectRatio  instead of Resolution  and AspectRatio. The two groups
	// of settings are mutually exclusive. Do not use them together. The width and
	// height of the video in the output file, in pixels. Valid values are auto and
	// width x height:
...
// are not in the same region as the pipeline. Using resources in the same region,
// such as your Amazon S3 buckets, Amazon SNS notification topics, and AWS KMS key,
