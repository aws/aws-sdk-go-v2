...
// Lastto search from the time that you specify until the end of the audio. For
...
type CallAnalyticsJob struct {
	...
	// specified in TranscriptFileUri  (or RedactedTranscriptFileUri, if you
	// requested transcript redaction). If the status is FAILED , FailureReason
	...
	// 2022-05-04T12:33:13.922000-07:00represents a transcription job that started
	// processing at 12:33 PM UTC-7 on May 4, 2022.
	CompletionTime *time.Time

	// The date and time the specified Call Analytics job request was made.
	// Timestamps are in the format YYYY-MM-DD'T'HH:MM:SS.SSSSSS-UTC . For example,
	// 2022-05-04T12:32:58.761000-07:00represents a transcription job that started
	...
	// about why the Call Analytics job request failed. The FailureReason field
	...
	// Timestamps are in the format YYYY-MM-DD'T'HH:MM:SS.SSSSSS-UTC . For example,
	// 2022-05-04T12:32:58.789000-07:00represents a transcription job that started
	// processing at 12:32 PM UTC-7 on May 4, 2022.
	StartTime *time.Time
...
type CallAnalyticsJobSettings struct {
	...
	// LanguageIdSettingssupports two to five language codes. Each language code you
	...
	// en-AU, your custom vocabulary is not applied to your transcription. If you
	// include LanguageOptions  and include en-US as the only English language
	// dialect, your custom vocabulary is applied to your transcription. If you want to
	// include a custom language model, custom vocabulary, or custom vocabulary filter
	// with your request but do not want to use automatic language identification, use
	// instead the parameter with the LanguageModelName , VocabularyName , or
	// VocabularyFilterNamesub-parameters. For a list of languages supported with
	// Call Analytics, refer to Supported languages and language-specific features (https://docs.aws.amazon.com/transcribe/latest/dg/supported-languages.html)
	// .
	...
	// The language of the specified custom language model must match the language code
	...
	// table. To transcribe speech in Modern Standard Arabic ( ar-SA), your media
...
type CallAnalyticsJobSummary struct {
	...
	// Provides the status of your Call Analytics job. If the status is COMPLETED,
	...
	// If CallAnalyticsJobStatus  is FAILED , FailureReason contains information
	// about why the Call Analytics job failed. See also: Common Errors (https://docs.aws.amazon.com/transcribe/latest/APIReference/CommonErrors.html)
	// .
	FailureReason *string
...
// if your agent is the first participant to speak, you would set ChannelId  to 0
// (to indicate the first channel) and ParticipantRole  to AGENT (to indicate
...
// custom language models. LanguageIdSettings supports two to five language
...
type LanguageModel struct {
	...
	// A unique name, chosen by you, for your custom language model. This name is
...
type Media struct {
	...
	// Note that the
	// Amazon S3 bucket that contains your input media must be located in the same
	// Amazon Web Services Region where you're making your transcription request.
	MediaFileUri *string
	...
	// Amazon S3 bucket that contains your input media must be located in the same
	// Amazon Web Services Region where you're making your transcription request.
	// RedactedMediaFileUriproduces a redacted audio file in addition to a redacted
...
type MedicalTranscriptionJob struct {
	...
	// about why the transcription job request failed. The FailureReason field
	// contains one of the following values:
	//     - Unsupported media format . The media format specified in MediaFormat isn't valid. Refer to MediaFormat for a list of supported formats.
	//     - The media format provided does not match the detected media format . The media format specified in MediaFormat doesn't match the format of the input file. Check the media format of your media file and correct the specified value.
...
type MedicalTranscriptionJobSummary struct {
	...
	// If TranscriptionJobStatus  is FAILED , FailureReason contains information
	// about why the transcription job failed. See also: Common Errors (https://docs.aws.amazon.com/transcribe/latest/APIReference/CommonErrors.html)
	// .
	FailureReason *string
	...
	// OutputKeyin your request, your output is located in the path you specified in
	// your request. If the value is SERVICE_BUCKET, the location is a
	// service-managed Amazon S3 bucket. To access a transcript stored in a
	// service-managed bucket, use the URI shown in the TranscriptFileUri  field.
...
type MedicalTranscriptionSetting struct {
	...
	// both ShowSpeakerLabels  and ChannelIdentification in the same request.
	// Including both parameters returns a BadRequestException. For more information,
	// see Transcribing multi-channel audio (https://docs.aws.amazon.com/transcribe/latest/dg/channel-id.html)
	// .
	...
	// include ShowAlternatives  in your transcription request. If you include
	// ShowAlternatives , you must also include MaxAlternatives, which is the maximum
	// number of alternative transcriptions you want Amazon Transcribe Medical to
	// generate. For more information, see Alternative transcriptions (https://docs.aws.amazon.com/transcribe/latest/dg/how-alternatives.html)
	// .
	...
	// MaxSpeakerLabels . You can't include ShowSpeakerLabels  and
...
// A time range, in percentage, between two points in your media file. You can
// use StartPercentage  and EndPercentage to search a custom segment. For
...
// and the 50 percent mark of your media file. You can use also First to search
// from the start of the media file until the time that you specify. Or use Last
// to search from the time that you specify until the end of the media file. For
// example, setting First to 10 only searches for your specified criteria in the
// audio contained in the first 10 percent of the media file. If you prefer to use
...
type Settings struct {
	...
	// ChannelIdentification in the same request. Including both parameters returns a
	// BadRequestException . For more information, see Transcribing multi-channel
	...
	// To include alternative transcriptions within your transcription output,
	// include ShowAlternatives in your transcription request. If you have
	...
	// speech by channel. If you include ShowAlternatives , you must also include
	// MaxAlternatives, which is the maximum number of alternative transcriptions you
	// want Amazon Transcribe to generate. For more information, see Alternative
	// transcriptions (https://docs.aws.amazon.com/transcribe/latest/dg/how-alternatives.html)
	// .
	ShowAlternatives *bool
...
// Generate subtitles for your media file with your transcription request. You
// can choose a start index of 0 or 1, and you can specify either WebVTT or SubRip
// (or both) as your output format. Note that your subtitle files are placed in the
...
type Subtitles struct {
	...
	// widely used standard of 1. If you're uncertain which value to use, we
...
type Transcript struct {
	...
	// RedactedTranscriptFileUriprovides you with a temporary URI you can use for
	// secure access to your transcript. Temporary URIs for service-managed Amazon S3
	// buckets are only valid for 15 minutes. If you get an AccesDenied error, you
	// can get a new temporary URI by running a GetTranscriptionJob  or
	...
	// your request. If you didn't include OutputBucketName in your transcription job
	// request, your transcript is stored in a service-managed bucket, and
	// TranscriptFileUriprovides you with a temporary URI you can use for secure
...
// Call Analytics transcription output. Rules using TranscriptFilter are designed
// to match:
...
type TranscriptFilter struct {
	...
	// Targetsvalue, only that exact phrase is flagged. Note that semantic matching
	// is not supported. For example, if your customer says "speak to the manager",
	// instead of "speak to a manager", your content is not flagged.
	//
	// This member is required.
...
// the specified transcription job, check the TranscriptionJobStatus field. If
...
type VocabularyInfo struct {
	...
	// vocabulary must contain terms in only one language. A custom vocabulary can only
	// be used to transcribe files in the same language as the custom vocabulary. For
	// example, if you create a custom vocabulary using US English ( en-US), you can
	// only apply this custom vocabulary to files that contain English audio.
	LanguageCode LanguageCode
