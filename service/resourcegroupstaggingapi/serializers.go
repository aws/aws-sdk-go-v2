...
type awsAwsjson11_serializeOpListRequiredTags struct {
}

func (*awsAwsjson11_serializeOpListRequiredTags) ID() string {
	return "OperationSerializer"
}

func (m *awsAwsjson11_serializeOpListRequiredTags) HandleSerialize(ctx context.Context, in middleware.SerializeInput, next middleware.SerializeHandler) (
	out middleware.SerializeOutput, metadata middleware.Metadata, err error,
) {
	_, span := tracing.StartSpan(ctx, "OperationSerializer")
	endTimer := startMetricTimer(ctx, "client.call.serialization_duration")
	...
	input, ok := in.Parameters.(*ListRequiredTagsInput)
	_ = input
	if !ok {
		return out, metadata, &smithy.SerializationError{Err: fmt.Errorf("unknown input parameters type %T", in.Parameters)}
	}
	...
	httpBindingEncoder.SetHeader("Content-Type").String("application/x-amz-json-1.1")
	httpBindingEncoder.SetHeader("X-Amz-Target").String("ResourceGroupsTaggingAPI_20170126.ListRequiredTags")

	jsonEncoder := smithyjson.NewEncoder()
	if err := awsAwsjson11_serializeOpDocumentListRequiredTagsInput(input, jsonEncoder.Value); err != nil {
		return out, metadata, &smithy.SerializationError{Err: err}
	}
...
func awsAwsjson11_serializeOpDocumentListRequiredTagsInput(v *ListRequiredTagsInput, value smithyjson.Value) error {
	object := value.Object()
	defer object.Close()
