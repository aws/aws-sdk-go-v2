...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/partnercentralrevenuemeasurement/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_PARTNERCENTRAL_REVENUE_MEASUREMENT")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "PartnerCentral Revenue Measurement", cfg.ConfigSources)
	if found && err == nil {
...
var bddNodes = [24]int32{
	-1, 1, -1, 0, 8, 3, 1, 4, 100000007, 2, 5, 100000007, 3, 7, 6, 4, 100000005, 100000006, 4, 100000004, 100000003, 4, 100000001, 100000002}
...
func resolveResult(idx int32, params *EndpointParameters, c *conditionContext) (smithyendpoints.Endpoint, error) {
	...
	case 3:
		uriString := func() string {
			...
			out.WriteString("https://partnercentral-prm.us-gov.")
	...
	case 4:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://partnercentral-prm-fips.us-gov.")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
		}()
		uri, err := url.Parse(uriString)
	...
	case 5:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://partnercentral-prm-fips.global.")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
		}()
		uri, err := url.Parse(uriString)
	...
	case 6:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://partnercentral-prm.global.")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
		}()
		uri, err := url.Parse(uriString)
