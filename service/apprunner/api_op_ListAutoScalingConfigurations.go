...
// the latest revision of each requested name.
func (c *Client) ListAutoScalingConfigurations(ctx context.Context, params *ListAutoScalingConfigurationsInput, optFns ...func(*Options)) (*ListAutoScalingConfigurationsOutput, error) {
	...
	result, metadata, err := c.invokeOperation(ctx, "ListAutoScalingConfigurations", params, optFns, addOperationListAutoScalingConfigurationsMiddlewares)
	if err != nil {
		return nil, err
	}
...
type ListAutoScalingConfigurationsInput struct {
	...
	// name. Keep as false to list all revisions for each requested configuration name.
	// Default: false
	LatestOnly bool
...
func addOperationListAutoScalingConfigurationsMiddlewares(stack *middleware.Stack, options Options) (err error) {
