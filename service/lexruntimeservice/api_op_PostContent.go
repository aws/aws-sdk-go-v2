...
// PostContentoperation supports audio input at 8kHz and 16kHz. You can use 8kHz
...
type PostContentInput struct {
	...
	// You pass this value as the Content-Type HTTP header. Indicates the audio
	// format or text. The header value must start with one of the following prefixes:
	...
	// Content-TypeHTTP header. You can stream audio data to Amazon Lex or you can
	...
	// attributes. Don't create any request attributes with the prefix x-amz-lex:.
...
type PostContentOutput struct {
	...
	// encodedInputTranscriptfield contains the text extracted from the audio stream.
	...
	// processing the audio that you send. The encodedInputTranscript field is
	// base-64 encoded. You must decode the field before you can use the value.
	...
	// dialogAction.typein its response, Amazon Lex decides on the next course of
	...
	// The text used to process the request. You can use this field only in the
	// de-DE, en-AU, en-GB, en-US, es-419, es-ES, es-US, fr-CA, fr-FR, and it-IT
	// locales. In all other locales, the inputTranscript field is null. You should
	// use the encodedInputTranscript field instead. If the input was an audio
	// stream, the inputTranscript field contains the text extracted from the audio
	// stream. This is the text that is actually processed to recognize intents and
	// slot values. You can use this information to determine if Amazon Lex is
	...
	// field is null. You should use the encodedMessage field instead. The message to
	// convey to the user. The message can come from the bot's configuration or from a
	// Lambda function. If the intent is not configured with a Lambda function, or if
	// the Lambda function returned Delegate  as the dialogAction.type in its
	// response, Amazon Lex decides on the next course of action and selects an
	// appropriate message from the bot's configuration based on the current
	// interaction context. For example, if Amazon Lex isn't able to understand user
	// input, it uses a clarification prompt message. When you create an intent you can
	// assign messages to groups. When messages are assigned to groups Amazon Lex
	// returns one message from each group in the response. The message field is an
	...
	// returns is determined by the valueSelectionStrategy selected when the slot
