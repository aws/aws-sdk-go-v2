...
type Artifact struct {
	...
	// the artifact (log) generated by an Android fuzz test.
	//
...
type DeviceSelectionConfiguration struct {
	...
	//
	// * FORM_FACTOR: The device form factor. Valid values
	// are PHONE or TABLET.
	//
	...
	//
	// * REMOTE_DEBUG_ENABLED: Whether the
	// device is enabled for remote debugging. Valid values are TRUE or FALSE. Because
	...
	//
	// * The FORM_FACTOR attribute
	// can be set to PHONE or TABLET.
	//
	// * The FLEET_TYPE attribute can be set to PUBLIC
	// or PRIVATE.
	//
	// This member is required.
...
type Sample struct {
	...
	// * TX
	//
	// * TX_RATE: The total number of
	// bytes per second (TCP and UDP) that are received, by app process.
	Type SampleType
...
type ScheduleRunTest struct {
	...
	//
	// * latest runs the latest Appium version supported
	// by Device Farm (1.9.1).
	//
	// * For default, Device Farm selects a compatible version
	...
	//
	// * event_count: The number
	// of events, between 1 and 10000, that the UI fuzz test should perform.
	//
	// *
	// throttle: The time, in ms, between 0 and 1000, that the UI fuzz test should wait
	// between events.
	//
	// * seed: A seed to use for randomizing the UI fuzz test. Using
	// the same seed value between tests ensures identical event sequences.
	//
	// For
	// Explorer tests:
	//
	// * username: A user name to use if the Explorer encounters a
	// login form. If not supplied, no user name is inserted.
	//
	// * password: A password
	// to use if the Explorer encounters a login form. If not supplied, no password is
	// inserted.
	//
	// For Instrumentation:
	//
