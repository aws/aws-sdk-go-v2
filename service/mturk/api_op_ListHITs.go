...
func (v *ListHITsOutput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListHITsResponse)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListHITsOutput) SerializeMembers(s smithy.ShapeSerializer) {
	serializeHITList(s, schemas.ListHITsResponse_HITs, v.HITs)
	if v.NextToken != nil {
		s.WriteString(schemas.ListHITsResponse_NextToken, *v.NextToken)
	}
	if v.NumResults != nil {
		s.WriteInt32(schemas.ListHITsResponse_NumResults, *v.NumResults)
	}
}
