...
// - UpdatePipeline, which updates a pipeline with
// edits or changes to the structure of the pipeline.
//
...
// - PutJobSuccessResult, which provides details
// of a job success.
//
// Third party jobs, which are instances of an action created by
// a partner action and integrated into AWS CodePipeline. Partner actions are
// created by members of the AWS Partner Network. You can work with third party
// jobs by calling:
// - AcknowledgeThirdPartyJob, which confirms whether a job worker
// has received the specified job.
// - GetThirdPartyJobDetails, which requests the
// details of a job for a partner action.
// - PollForThirdPartyJobs, which determines
// whether there are any jobs to act on.
// - PutThirdPartyJobFailureResult, which
// provides details of a job failure.
