...
//
//     * CreatePipeline, which creates a uniquely
// named pipeline.
//
...
//
//     *
// GetPipelineExecution, which returns information about a specific execution of a
// pipeline.
//
...
// which returns action-level details for past executions. The details include full
// stage and action-level details, including individual action duration, status,
// any errors that occurred during the execution, and input and output artifact
// location details.
//
//     * ListPipelines, which gets a summary of all of the
// pipelines associated with your account.
//
//     * ListPipelineExecutions, which
...
//
//     * StopPipelineExecution, which stops the specified
...
// pipeline.
//
// Pipelines include stages. Each stage contains one or more actions
// that must complete before the next stage begins. A stage results in success or
...
// can call GetPipelineState, which displays the status of a pipeline, including
...
//
//     *
// DisableStageTransition, which prevents artifacts from transitioning to the next
// stage in a pipeline.
//
//     * EnableStageTransition, which enables transition of
// artifacts between stages in a pipeline.
//
// Using the API to integrate with AWS
// CodePipeline For third-party integrators or developers who want to create their
// own integrations with AWS CodePipeline, the expected sequence varies from the
// standard API user. To integrate with AWS CodePipeline, developers need to work
// with the following items: Jobs, which are instances of an action. For example, a
// job for a source action might import a revision of an artifact from a source.
// You can work with jobs by calling:
//
//     * AcknowledgeJob, which confirms whether
// a job worker has received the specified job.
//
//     * GetJobDetails, which returns
// the details of a job.
//
//     * PollForJobs, which determines whether there are any
// jobs to act on.
//
//     * PutJobFailureResult, which provides details of a job
// failure.
//
...
// partner action and integrated into AWS CodePipeline. Partner actions are created
// by members of the AWS Partner Network. You can work with third party jobs by
// calling:
//
//     * AcknowledgeThirdPartyJob, which confirms whether a job worker
// has received the specified job.
//
//     * GetThirdPartyJobDetails, which requests
// the details of a job for a partner action.
//
//     * PollForThirdPartyJobs, which
// determines whether there are any jobs to act on.
//
//     *
