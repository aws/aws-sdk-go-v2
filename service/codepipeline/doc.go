...
//
// * ListActionExecutions, which returns
...
//
// * ListPipelineExecutions, which gets a summary of
// the most recent executions for a pipeline.
//
// * StartPipelineExecution, which runs
// the most recent revision of an artifact through the pipeline.
//
// *
// StopPipelineExecution, which stops the specified pipeline execution from
// continuing through the pipeline.
//
// * UpdatePipeline, which updates a pipeline
// with edits or changes to the structure of the pipeline.
//
// Pipelines include
// stages. Each stage contains one or more actions that must complete before the
// next stage begins. A stage results in success or failure. If a stage fails, the
// pipeline stops at that stage and remains stopped until either a new version of
// an artifact appears in the source location, or a user takes action to rerun the
// most recent artifact through the pipeline. You can call GetPipelineState, which
...
// of stages and actions, see AWS CodePipeline Pipeline Structure Reference
...
// transitions, which allow the transition of artifacts from one stage to the next
// in a pipeline after the actions in one stage complete. You can work with
// transitions by calling:
//
// * DisableStageTransition, which prevents artifacts from
// transitioning to the next stage in a pipeline.
//
// * EnableStageTransition, which
// enables transition of artifacts between stages in a pipeline.
//
// Using the API to
...
//
// * PutJobFailureResult, which
// provides details of a job failure.
//
// * PutJobSuccessResult, which provides
...
//
// * AcknowledgeThirdPartyJob, which confirms whether
// a job worker has received the specified job.
//
// * GetThirdPartyJobDetails, which
// requests the details of a job for a partner action.
//
