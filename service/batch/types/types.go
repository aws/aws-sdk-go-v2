...
type AttemptDetail struct {
	...
	// the attempt transitioned from the RUNNING  state to a terminal state, such as
	// SUCCEEDED or FAILED ).
	StoppedAt *int64
...
type ComputeEnvironmentDetail struct {
	...
	// automatically based on the job queue demand. If the state is DISABLED, then
	// the Batch scheduler doesn't attempt to place jobs within the environment. Jobs
	...
	// DISABLEDstate may continue to incur billing charges. To prevent additional
	...
	// more information about infrastructure updates, see Updating compute
...
// job queue. All of the compute environments must be either EC2 ( EC2  or SPOT)
// or Fargate ( FARGATE  or FARGATE_SPOT); EC2 and Fargate compute environments
// can't be mixed. All compute environments that are associated with a job queue
// must share the same architecture. Batch doesn't support mixing compute
...
type ComputeResource struct {
	...
	// in the Amazon EKS User Guide and Amazon ECS clusters in Local Zones,
	...
	// the default is ECS_AL2. One or two values can be provided. This parameter
	...
	// Ec2Configurationstructure. This parameter isn't applicable to jobs that are
	...
	// launch any instance type within those families (for example, c5  or p3), or
	// you can specify specific sizes within a family (such as c5.8xlarge). You can
	// also choose optimal to select instance types (from the C4, M4, and R4 instance
	...
	// to a SPOT compute environment. This role is required if the allocation
	// strategy set to BEST_FIT or if the allocation strategy isn't specified. For
	// more information, see Amazon EC2 spot fleet role (https://docs.aws.amazon.com/batch/latest/userguide/spot_fleet_IAM_role.html)
	// in the Batch User Guide. This parameter isn't applicable to jobs that are
	...
	// ListTagsForResourceAPI operation. This parameter isn't applicable to jobs that
...
type ComputeResourceUpdate struct {
	...
	// compute environments using Amazon EKS clusters. When you update the
	// desiredvCpus setting, the value must be between the minvCpus  and maxvCpus
	// values. Additionally, the updated desiredvCpus value must be greater than or
	...
	// The Amazon Machine Image (AMI) ID used for instances launched in the compute
	// environment. This parameter is overridden by the imageIdOverride  member of the
	// Ec2Configurationstructure. To remove the custom AMI ID and use the default AMI
	...
	// both BEST_FIT_PROGRESSIVE  and SPOT_CAPACITY_OPTIMIZED allocation strategies
	// using On-Demand or Spot Instances, and the BEST_FIT strategy using Spot
	// Instances, Batch might need to exceed maxvCpus to meet your capacity
	// requirements. In this event, Batch never exceeds maxvCpus by more than a
	...
	// imageIdOverrideparameters or be determined by the launch template that's
...
type ContainerDetail struct {
	...
	// ECS_AVAILABLE_LOGGING_DRIVERSenvironment variable before containers placed on
	...
	// using resourceRequirements, the number of MiB of memory reserved for the job.
	// For other jobs, including all run on Fargate resources, see
	...
	// the host container instance (similar to the root  user). The default value is
	// false. This parameter isn't applicable to jobs that are running on Fargate
	...
	// Ulimits in the Create a container (https://docs.docker.com/engine/api/v1.23/#create-a-container)
	// section of the Docker Remote API (https://docs.docker.com/engine/api/v1.23/)
	...
	// vcpus and resourceRequirements  object. This parameter maps to CpuShares in
...
type ContainerOverrides struct {
	...
	// definition. Environment variables cannot start with " AWS_BATCH". This naming
	...
	// memory requirement that's specified in the resourceRequirements structure in
	// the job definition. To override memory requirements that are specified in the
	...
	// definition resource requirements (https://docs.aws.amazon.com/batch/latest/userguide/troubleshooting.html#override-resource-requirements)
	// in the Batch User Guide.
	//
	...
	// vcpusparameter set in the job definition, but doesn't override any vCPU
...
type Ec2Configuration struct {
	...
	// parameter isn't specified, then a recent Amazon ECS-optimized Amazon Linux 2
	...
	// Amazon ECS optimized AMI for that image type that's supported by Batch is used.
	// ECS_AL2 Amazon Linux 2 (https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-optimized_AMI.html#al2ami)
	// : Default for all non-GPU instance families. ECS_AL2_NVIDIA Amazon Linux 2
	...
	// used for all non Amazon Web Services Graviton-based instance types. ECS_AL1
	...
	// EKS_AL2 Amazon Linux 2 (https://docs.aws.amazon.com/eks/latest/userguide/eks-optimized-ami.html)
...
type EFSVolumeConfiguration struct {
	...
	// the authorizationConfig, the root directory parameter must either be omitted
	...
	// must be between 0 and 65,535. For more information, see EFS mount helper (https://docs.aws.amazon.com/efs/latest/ug/efs-mount-helper.html)
	// in the Amazon Elastic File System User Guide.
...
type EksContainer struct {
	...
	// IfNotPresent , and Never . This parameter defaults to IfNotPresent. However,
	// if the :latest  tag is specified, it defaults to Always. For more information,
	// see Updating images (https://kubernetes.io/docs/concepts/containers/images/#updating-images)
	// in the Kubernetes documentation.
	...
	// secretvolume types. For more information about volumes and volume mounts in
...
type EksContainerDetail struct {
	...
	// The image pull policy for the container. Supported values are Always ,
	// IfNotPresent , and Never . This parameter defaults to Always  if the :latest
	// tag is specified, IfNotPresent  otherwise. For more information, see Updating
...
type EksContainerOverride struct {
	...
	// information, see CMD (https://docs.docker.com/engine/reference/builder/#cmd)
	// in the Dockerfile reference and Define a command an arguments for a pod (https://kubernetes.io/docs/tasks/inject-data-application/define-command-argument-container/)
	// in the Kubernetes documentation.
	Args []string
	...
	// and nvidia.com/gpu . For more information, see Resource management for pods
	// and containers (https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/)
	// in the Kubernetes documentation.
	Resources *EksContainerResourceRequirements
...
type EksContainerResourceRequirements struct {
	...
	// using either the limits  or the requests objects. memory The memory hard limit
	// (in MiB) for the container, using whole integers, with a "Mi" suffix. If your
	// container attempts to exceed the memory specified, the container is terminated.
	// You must specify at least 4 MiB of memory for a job. memory can be specified
	// in limits , requests , or both. If memory is specified in both places, then
	// the value that's specified in limits must be equal to the value that's
	// specified in requests. To maximize your resource utilization, provide your
	// jobs with as much memory as possible for the specific instance type that you are
	// using. To learn how, see Memory management (https://docs.aws.amazon.com/batch/latest/userguide/memory-management.html)
	...
	// limits , requests , or both. If cpu is specified in both places, then the
	...
	// the container. Values must be a whole integer. memory  can be specified in
	// limits , requests , or both. If memory is specified in both places, then the
	// value that's specified in limits must be equal to the value that's specified
	// in requests .
	Limits map[string]string

	// The type and quantity of the resources to request for the container. The
	// values vary based on the name that's specified. Resources can be requested by
	// using either the limits  or the requests objects. memory The memory hard limit
	// (in MiB) for the container, using whole integers, with a "Mi" suffix. If your
	// container attempts to exceed the memory specified, the container is terminated.
	// You must specify at least 4 MiB of memory for a job. memory can be specified
	// in limits , requests , or both. If memory is specified in both, then the value
	...
	// container. Values must be an even multiple of 0.25 . cpu  can be specified in
	// limits , requests , or both. If cpu is specified in both, then the value
	// that's specified in limits must be at least as large as the value that's
	// specified in requests. nvidia.com/gpu The number of GPUs that are reserved for
	// the container. Values must be a whole integer. nvidia.com/gpu can be specified
	// in limits , requests , or both. If nvidia.com/gpu is specified in both, then
	// the value that's specified in limits must be equal to the value that's
...
type EksContainerSecurityContext struct {
	...
	// When this parameter is true, the container is given elevated permissions on
	// the host container instance. The level of permissions are similar to the root
	// user permissions. The default value is false . This parameter maps to
	...
	// specified in the image metadata. This parameter maps to RunAsGroup  and
	// MustRunAs policy in the Users and groups pod security policies (https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups)
	// in the Kubernetes documentation.
	RunAsGroup *int64
	...
	// parameter maps to RunAsUser  and MustRunAsNonRoot  policy in the Users and
	// groups pod security policies (https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups)
	// in the Kubernetes documentation.
	RunAsNonRoot *bool
	...
	// specified in the image metadata. This parameter maps to RunAsUser  and
	// MustRanAs policy in the Users and groups pod security policies (https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups)
	// in the Kubernetes documentation.
	RunAsUser *int64
...
// volume is first created when a pod is assigned to a node. It exists as long as
// that pod is running on that node. The emptyDir volume is initially empty. All
// containers in the pod can read and write the files in the emptyDir volume.
// However, the emptyDir volume can be mounted at the same or different paths in
// each container. When a pod is removed from a node for any reason, the data in
// the emptyDir  is deleted permanently. For more information, see emptyDir (https://kubernetes.io/docs/concepts/storage/volumes/#emptydir)
// in the Kubernetes documentation.
type EksEmptyDir struct {
	...
	// "Memory" Use the tmpfs volume that's backed by the RAM of the node. Contents
	// of the volume are lost when the node reboots, and any storage on the volume
...
// volume mounts an existing file or directory from the host node's filesystem into
...
type EksPodProperties struct {
	...
	// namespaces (https://kubernetes.io/docs/concepts/security/pod-security-policy/#host-namespaces)
...
type EvaluateOnExit struct {
	...
	// ExitCodereturned for a job. The pattern can be up to 512 characters long. It
...
type FairsharePolicy struct {
	...
	// A computeReservation value of 25 indicates that Batch should reserve 25% of
	// the maximum available vCPU if there's only one fair share identifier, 6.25% if
...
type JobDetail struct {
	...
	// RUNNINGstate. This parameter isn't provided for child jobs of array jobs or
	...
	// SUBMITTED state. This is specifically at the time SubmitJob was called. For
...
type JobTimeout struct {
	...
	// startedAttimestamp. After this time passes, Batch terminates your jobs if they
...
// securityGroupIds parameter of CreateComputeEnvironment and the launch
// template, the values in the securityGroupIds  parameter of
// CreateComputeEnvironmentwill be used. This object isn't applicable to jobs
...
type LaunchTemplateSpecification struct {
	...
	// . During an infrastructure update, if either $Latest  or $Default is
...
type LinuxParameters struct {
	...
	// necessary. A swappiness  value of 100 causes pages to be swapped aggressively.
	// Valid values are whole numbers between 0  and 100 . If the swappiness
	// parameter isn't specified, a default value of 60 is used. If a value isn't
	// specified for maxSwap , then this parameter is ignored. If maxSwap is set to
	// 0, the container doesn't use swap. This parameter maps to the
	// --memory-swappiness option to docker run (https://docs.docker.com/engine/reference/run/)
...
// in a SubmitJob API operation. This parameter isn't applicable to jobs that are
...
type NodeOverrides struct {
	...
	// this override, you must meet the following conditions:
...
type NodePropertyOverride struct {
	...
	// range value is omitted ( :n ), then 0 is used to start the range. If the
	// ending range value is omitted ( n:), then the highest possible node index is
	// used to end the range.
	//
	// This member is required.
	TargetNodes *string
...
type ResourceRequirement struct {
	...
	// 2 value = 5120, 6144, or 7168 VCPU  = 1 or 2 value = 8192 VCPU = 1, 2, or 4
...
// An object that contains the details of a scheduling policy that's returned in
// a ListSchedulingPolicy  action.
...
type Secret struct {
	...
	// the parameter in the Amazon Web Services Systems Manager Parameter Store. If the
