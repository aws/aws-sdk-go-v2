...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/securityir/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_SECURITY_IR")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Security IR", cfg.ConfigSources)
	if found && err == nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					...
					out.WriteString("https://security-ir-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://security-ir.")
				out.WriteString(_Region)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
