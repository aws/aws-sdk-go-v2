...
// sts:GetServiceBearerTokenpermissions. For more information about authorization
...
// GetAuthorizationTokenAPI, you can set a custom authorization period, up to a
// maximum of 12 hours, with the durationSeconds parameter. The authorization
// period begins after login  or GetAuthorizationToken  is called. If login  or
// GetAuthorizationTokenis called while assuming a role, the token lifetime is
// independent of the maximum session duration of the role. For example, if you
// call sts assume-role and specify a session duration of 15 minutes, then
// generate a CodeArtifact authorization token, the token will be valid for the
// full authorization period even though this is longer than the 15-minute session
// duration. See Using IAM Roles (https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use.html)
