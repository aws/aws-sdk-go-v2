...
// - ListRepositories,
// which lists all AWS CodeCommit repositories associated with your AWS account.
// -
...
// you change the name of a repository, no other users of that repository can
...
// - GetBranch, which returns information about a
// specified branch.
...
// - DeleteFile, which deletes the
// content of a specified file from a specified branch.
// - GetBlob, which returns
// the base-64 encoded content of an individual Git blob object in a repository.
// -
...
// - BatchGetCommits, which returns
// information about one or more commits in a repository.
// - CreateCommit, which
// creates a commit for changes to a repository.
...
// - MergeBranchesByFastForward, which
// merges two branches using the fast-forward merge option.
// -
...
// - DescribePullRequestEvents, which returns
// information about one or more pull request events.
// -
// EvaluatePullRequestApprovalRules, which evaluates whether a pull request has met
// all the conditions specified in its associated approval rules.
// -
...
// rules have been set aside (overriden) for a pull request, and if so, the Amazon
// Resource Name (ARN) of the user or identity that overrode the rules and their
// requirements for the pull request.
// - ListPullRequests, which lists all pull
// requests for a repository.
// - MergePullRequestByFastForward, which merges the
...
// squash merge option.
// - MergePullRequestByThreeWay. which merges the source
...
// - PostCommentForPullRequest, which posts a
// comment to a pull request at the specified line, file, or request.
// -
// UpdatePullRequestApprovalRuleContent, which updates the structure of an approval
// rule for a pull request.
// - UpdatePullRequestApprovalState, which updates the
// state of an approval on a pull request.
// - UpdatePullRequestDescription, which
// updates the description of a pull request.
// - UpdatePullRequestStatus, which
// updates the status of a pull request.
// - UpdatePullRequestTitle, which updates
...
// - CreateApprovalRuleTemplate, which creates a
// template for approval rules that can then be associated with one or more
// repositories in your AWS account.
// - DeleteApprovalRuleTemplate, which deletes
// the specified template. It does not remove approval rules on pull requests
...
// - ListApprovalRuleTemplates, which lists all approval
// rule templates in the AWS Region in your AWS account.
// -
...
// approval rule template.
// - UpdateApprovalRuleTemplateName, which updates the name
// of an approval rule template.
// - UpdateApprovalRuleTemplateContent, which updates
// the content of an approval rule template.
//
// Comments in a repository, by calling
// the following:
// - DeleteCommentContent, which deletes the content of a comment on
// a commit in a repository.
// - GetComment, which returns information about a
// comment on a commit.
// - GetCommentReactions, which returns information about
// emoji reactions to comments.
// - GetCommentsForComparedCommit, which returns
// information about comments on the comparison between two commit specifiers in a
// repository.
...
// - PutCommentReaction, which creates or
// updates an emoji reaction to a comment.
// - UpdateComment, which updates the
// content of a comment on a commit in a repository.
//
// Tags used to tag resources in
// AWS CodeCommit (not Git tags), by calling the following:
// - ListTagsForResource,
...
// - GetRepositoryTriggers, which
// returns information about triggers configured for a repository.
// -
// PutRepositoryTriggers, which replaces all triggers for a repository and can be
// used to create or delete triggers.
// - TestRepositoryTriggers, which tests the
// functionality of a repository trigger by sending data to the trigger
// target.
//
