...
func (v *ListMLInputChannelsInput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListMLInputChannelsRequest)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListMLInputChannelsInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.MaxResults != nil {
		s.WriteInt32(schemas.ListMLInputChannelsRequest_maxResults, *v.MaxResults)
	}
	if v.MembershipIdentifier != nil {
		s.WriteString(schemas.ListMLInputChannelsRequest_membershipIdentifier, *v.MembershipIdentifier)
	}
	if v.NextToken != nil {
		s.WriteString(schemas.ListMLInputChannelsRequest_nextToken, *v.NextToken)
	}
}
...
func (v *ListMLInputChannelsOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.ListMLInputChannelsResponse, func(s *smithy.Schema) error {
		switch s {
		case schemas.ListMLInputChannelsResponse_mlInputChannelsList:
			return deserializeMLInputChannelsList(d, schemas.ListMLInputChannelsResponse_mlInputChannelsList, &v.MlInputChannelsList)
		case schemas.ListMLInputChannelsResponse_nextToken:
			v.NextToken = new(string)
			return d.ReadString(schemas.ListMLInputChannelsResponse_nextToken, v.NextToken)
		}
		return nil
	})
}
func (c *Client) addOperationListMLInputChannelsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.ListMLInputChannels, schemas.ListMLInputChannelsRequest, schemas.ListMLInputChannelsResponse)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.ListMLInputChannels, schemas.ListMLInputChannelsRequest, schemas.ListMLInputChannelsResponse), output: &ListMLInputChannelsOutput{}}, middleware.After); err != nil {
		return err
	}
