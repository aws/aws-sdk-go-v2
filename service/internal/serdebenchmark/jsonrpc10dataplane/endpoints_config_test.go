...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-jsonrpc10dataplane.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-jsonrpc10dataplane.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-jsonrpc10dataplane.dev",
			expectURL:     aws.String("https://env-jsonrpc10dataplane.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-jsonrpc10dataplane.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-jsonrpc10dataplane.dev",
			expectURL:     aws.String("http://config-jsonrpc10dataplane.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-jsonrpc10dataplane.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-jsonrpc10dataplane.dev",
			clientEndpoint: aws.String("https://client-jsonrpc10dataplane.dev"),
			expectURL:      aws.String("https://client-jsonrpc10dataplane.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_JSONRPC10DATAPLANE", c.envService)
			}
