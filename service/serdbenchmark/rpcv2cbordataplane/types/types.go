...
func (v *ConsumedCapacity) SerializeMembers(s smithy.ShapeSerializer) {
	if v.CapacityUnits != nil { s.WriteFloat64(schemas.ConsumedCapacity_CapacityUnits, *v.CapacityUnits) }
	if v.ReadCapacityUnits != nil { s.WriteFloat64(schemas.ConsumedCapacity_ReadCapacityUnits, *v.ReadCapacityUnits) }
	if v.TableName != nil { s.WriteString(schemas.ConsumedCapacity_TableName, *v.TableName) }
	if v.WriteCapacityUnits != nil { s.WriteFloat64(schemas.ConsumedCapacity_WriteCapacityUnits, *v.WriteCapacityUnits) }
}
...
func (v *ExpectedAttributeValue) SerializeMembers(s smithy.ShapeSerializer) {
	...
	if v.ComparisonOperator != nil { s.WriteString(schemas.ExpectedAttributeValue_ComparisonOperator, *v.ComparisonOperator) }
	if v.Exists != nil { s.WriteBool(schemas.ExpectedAttributeValue_Exists, *v.Exists) }
