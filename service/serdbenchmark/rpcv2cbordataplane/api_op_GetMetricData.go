...
func (c *Client) GetMetricData(ctx context.Context, params *GetMetricDataInput, optFns ...func(*Options)) (*GetMetricDataOutput, error) {
	if params == nil { params = &GetMetricDataInput{} }
...
func (v *GetMetricDataInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.EndTime != nil { s.WriteTime(schemas.GetMetricDataInput_EndTime, *v.EndTime) }
	if v.LabelOptions != nil { s.WriteStruct(schemas.GetMetricDataInput_LabelOptions)
	v.LabelOptions.SerializeMembers(s)
	s.CloseStruct() }
	if v.MaxDatapoints != nil { s.WriteInt32(schemas.GetMetricDataInput_MaxDatapoints, *v.MaxDatapoints) }
	serializeMetricDataQueries(s, schemas.GetMetricDataInput_MetricDataQueries, v.MetricDataQueries)
	if v.NextToken != nil { s.WriteString(schemas.GetMetricDataInput_NextToken, *v.NextToken) }
	if v.ScanBy != "" { s.WriteString(schemas.GetMetricDataInput_ScanBy, string(v.ScanBy)) }
	if v.StartTime != nil { s.WriteTime(schemas.GetMetricDataInput_StartTime, *v.StartTime) }
}
...
func (v *GetMetricDataOutput) SerializeMembers(s smithy.ShapeSerializer) {
	...
	if v.NextToken != nil { s.WriteString(schemas.GetMetricDataOutput_NextToken, *v.NextToken) }
}
...
func (c *Client) addOperationGetMetricDataMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = stack.Initialize.Add(newServiceMetadataMiddleware(options.Region, "GetMetricData", ), middleware.Before); err != nil {
	return err
	}
