...
// GetBuckets action. The ableToUpdateBundle parameter in the response will
// indicate whether you can currently update a bucket's bundle. Update a bucket's
// bundle if it's consistently going over its storage space or data transfer quota,
// or if a bucket's usage is consistently in the lower range of its storage space
// or data transfer quota. Due to the unpredictable usage fluctuations that a
// bucket might experience, we strongly recommend that you update a bucket's bundle
// only as a long-term strategy, instead of as a short-term, monthly cost-cutting
// measure. Choose a bucket bundle that will provide the bucket with ample storage
...
type UpdateBucketBundleInput struct {
	...
	// The ID of the new bundle to apply to the bucket. Use the GetBucketBundles action
	// to get a list of bundle IDs that you can specify.
	//
	// This member is required.
	BundleId *string
}
