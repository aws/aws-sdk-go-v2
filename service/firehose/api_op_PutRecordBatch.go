...
// RequestResponsesincludes both successfully and unsuccessfully processed
// records. Kinesis Data Firehose tries to process all records in each
// PutRecordBatchrequest. A single record failure does not stop the processing of
...
// is one of the following values: ServiceUnavailableException  or InternalFailure
// . ErrorMessage provides more detailed information about the error. If there is
// an internal server error or a timeout, the write might have completed or it
// might have failed. If FailedPutCount is greater than 0, retry the request,
...
// destination. If PutRecordBatch  throws ServiceUnavailableException, back off
// and retry. If the exception persists, it is possible that the throughput limits
// have been exceeded for the delivery stream. Data records sent to Kinesis Data
