...
// - Security Hub
// is not configured to automatically add new organization accounts.
// - The account
// was disassociated or deleted in Security Hub.
//
// This action can only be used by
// an account that has Security Hub enabled. To enable Security Hub, you can use
// the EnableSecurityHub operation. For accounts that are not organization members,
// you create the account association and then send an invitation to the member
// account. To send the invitation, you use the InviteMembers operation. If the
// account owner accepts the invitation, the account becomes a member account in
// Security Hub. Accounts that are managed using Organizations do not receive an
// invitation. They automatically become a member account in Security Hub.
// - If the
// organization account does not have Security Hub enabled, then Security Hub and
// the default standards are automatically enabled. Note that Security Hub cannot
// be enabled automatically for the organization management account. The
