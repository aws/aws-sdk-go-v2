...
func TestCheckResponseSnapshot_QueryForecast(t *testing.T) {
	...
	status, header, body, err := serdeRespReadSnapshot("QueryForecast.response")
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "QueryForecast.response", err)
	}
}

func TestCheckResponseSnapshot_QueryWhatIfForecast(t *testing.T) {
	...
	status, header, body, err := serdeRespReadSnapshot("QueryWhatIfForecast.response")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "QueryWhatIfForecast.response", err)
	}
}
