...
// findings detected by Amazon GuardDuty. The Detective API primarily supports the
// creation and management of behavior graphs. A behavior graph contains the
// extracted data from a set of member accounts, and is created and managed by an
// administrator account. To add a member account to the behavior graph, the
// administrator account sends an invitation to the account. When the account
// accepts the invitation, it becomes a member account in the behavior graph.
...
// account for Detective in Organizations. The Detective administrator account can
// enable any organization account as a member account in the organization behavior
// graph. The organization accounts do not receive invitations. The Detective
// administrator account can also invite other accounts to the organization
// behavior graph. Every behavior graph is specific to a Region. You can only use
// the API to manage behavior graphs that belong to the Region that is associated
// with the currently selected endpoint. The administrator account for a behavior
// graph can use the Detective API to do the following:
...
//
// The organization management account can
// use the Detective API to select the delegated administrator for Detective. The
...
// as CloudTrail events. See Logging Detective API Calls with CloudTrail (https://docs.aws.amazon.com/detective/latest/adminguide/logging-using-cloudtrail.html)
