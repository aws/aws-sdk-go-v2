...
type CreateReplicationGroupInput struct {
	...
	// primary if the existing primary fails. AutomaticFailoverEnabled must be
	// enabled for Redis (cluster mode enabled) replication groups. Default: false
	...
	// DescribeCacheEngineVersionsoperation. Important: You can upgrade to a newer
	...
	// ReplicaAvailabilityZones , ReplicaCount , and Slots. If you're creating a
	// Redis (cluster mode disabled) or a Redis (cluster mode enabled) replication
	...
	// Redis (cluster mode enabled) cluster from a S3 rdb file. You must configure each
	// node group (shard) using this parameter because you must specify the slots for
	// each node group.
	...
	// ReplicasPerNodeGroup instead. If AutomaticFailoverEnabled  is true, the value
	// of this parameter must be at least 2. If AutomaticFailoverEnabled  is false
	// you can omit this parameter (it will default to 1), or you can explicitly set it
	// to a value between 2 and 6. The maximum permitted value for NumCacheClusters
	...
	// Value= mySecondKeyValue. Tags on replication groups will be replicated to all
	// nodes.
	Tags []types.Tag
	...
	// 3.2.6 , 4.x or later, and the cluster is being created in an Amazon VPC. If
	// you enable in-transit encryption, you must also specify a value for
	// CacheSubnetGroup. Required: Only available when creating a replication group
	// in an Amazon VPC using redis version 3.2.6 , 4.x  or later. Default: false For
	// HIPAA compliance, you must specify TransitEncryptionEnabled  as true , an
	// AuthToken , and a CacheSubnetGroup .
	TransitEncryptionEnabled *bool

	// A setting that allows you to migrate your clients to use in-transit
	// encryption, with no downtime. When setting TransitEncryptionEnabled  to true,
	// you can set your TransitEncryptionMode  to preferred in the same request, to
	// allow both encrypted and unencrypted connections at the same time. Once you
	// migrate all your Redis clients to use encrypted connections you can modify the
