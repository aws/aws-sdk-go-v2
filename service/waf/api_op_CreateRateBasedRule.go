...
// from a specified IP address in a five-minute period. The RateBasedRule also
// contains the IPSet  objects, ByteMatchSet objects, and other predicates that
// identify the requests that you want to count or block if these requests exceed
// the RateLimit . If you add more than one predicate to a RateBasedRule, a
// request not only must exceed the RateLimit, but it also must match all the
...
// requests that match these two conditions must be received at a rate of more than
// 1,000 requests every five minutes. If both conditions are met and the rate is
// exceeded, AWS WAF blocks the requests. If the rate drops below 1,000 for a
// five-minute period, AWS WAF no longer blocks the requests. As a second example,
...
type CreateRateBasedRuleInput struct {
	...
	// RateKey is IP . IP indicates that requests that arrive from the same IP
	// address are subject to the RateLimit  that is specified in the RateBasedRule .
	//
	// This member is required.
