...
// instance to DRAINING to manually remove an instance from a cluster, for
// example to perform system updates, update the Docker daemon, or scale down the
// cluster size. A container instance can't be changed to DRAINING until it has
// reached an ACTIVE status. If the instance is in any other status, an error
// will be received. When you set a container instance to DRAINING, Amazon ECS
// prevents new tasks from being scheduled for placement on the container instance
// and replacement service tasks are started on other container instances in the
...
// completed draining when it has no more RUNNING tasks. You can verify this
// using ListTasks. When a container instance has been drained, you can set a
...
type UpdateContainerInstancesStateInput struct {
	...
	// If a container instance is in REGISTERING , DEREGISTERING , or
	// REGISTRATION_FAILEDstate you can describe the container instance but can't
