...
// the task, or by the desired status of the task. Recently stopped tasks might
...
type ListTasksInput struct {
	...
	// Specifying a desiredStatus  of STOPPED limits the results to tasks that Amazon
	// ECS has set the desired status to STOPPED. This can be useful for debugging
	// tasks that aren't starting properly or have died or finished. The default status
	// filter is RUNNING, which shows tasks that Amazon ECS has set the desired
	// status to RUNNING. Although you can filter results based on a desired status
	// of PENDING, this doesn't return any results. Amazon ECS never sets the desired
	// status of a task to that value (only a task's lastStatus  may have a value of
	// PENDING ).
	...
	// ListTasks request with the returned nextToken value. This value can be between
	// 1 and 100. If this parameter isn't used, then ListTasks returns up to 100
