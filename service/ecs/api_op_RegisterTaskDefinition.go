...
// policy that's associated with the role. For more information, see IAM Roles
...
// networkModeparameter. The available network modes correspond to those
// described in Network settings (https://docs.docker.com/engine/reference/run/#/network-settings)
...
// NetworkConfigurationwhen you create a service or run a task with the task
...
type RegisterTaskDefinitionInput struct {
	...
	// recommend specifying container-level resources for Windows containers. If you're
	// using the EC2 launch type, this field is optional. Supported values are between
	// 128 CPU units ( 0.125  vCPUs) and 10240  CPU units ( 10 vCPUs). If you do not
	// specify a value, the parameter is ignored. If you're using the Fargate launch
	// type, this field is required and you must use one of the following values, which
	// determines your range of supported values for the memory parameter: The CPU
	// units cannot be less than 1 vCPU when you use Windows containers on Fargate.
	...
	// container agent and at least version 1.26.0-1  of the ecs-init package to use
	...
	// definition in a service, this value must match the runtimePlatform value of
	// the service.
