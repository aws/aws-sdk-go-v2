...
// cluster. To update an existing service, see the UpdateService action. In
...
// new tasks. If they're in the RUNNING state, tasks for services that don't use
// a load balancer are considered healthy . If they're in the RUNNING state and
...
// service that remain in the RUNNING state. This is while the container
...
// aren't used. This is the case even if they're currently visible when describing
// your service. When creating a service that uses the EXTERNAL deployment
// controller, you can specify only parameters that aren't controlled at the task
// set level. The only required parameter is the service name. You control your
// services using the CreateTaskSet  operation. For more information, see Amazon
...
type CreateServiceInput struct {
	...
	// defaultCapacityProviderStrategyfor the cluster is used. A capacity provider
	...
	// keep running on your cluster. This is required if schedulingStrategy  is
	// REPLICA or isn't specified. If schedulingStrategy  is DAEMON then this isn't
	// required.
	DesiredCount *int32
	...
	// use an Elastic Load Balancing, we recommend that you use the startPeriod in
	...
	// in the Amazon Elastic Container Service Developer Guide. If the service uses the
	//
	// CODE_DEPLOYdeployment controller, the service is required to use either an
	...
	// accelerators in which case you don't specify a role here. For more information,
	...
	// ACTIVErevision is used. A task definition must be specified if the service
	// uses either the ECS  or CODE_DEPLOY  deployment controllers.
...
type CreateServiceOutput struct {
	...
	// return either a capacityProviderStrategy  or launchType parameter, but not
	// both, depending where one was specified when it was created. If a service is
	// using the ECS  deployment controller, the deploymentController  and taskSets
	// parameters will not be returned. if the service uses the CODE_DEPLOY
	// deployment controller, the deploymentController , taskSets  and deployments
	// parameters will be returned, however the deployments parameter will be an
	// empty list.
