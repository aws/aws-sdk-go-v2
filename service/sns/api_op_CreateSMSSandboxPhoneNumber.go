...
// one-time password (OTP) to that phone number. When you start using Amazon SNS to
// send SMS messages, your AWS account is in the SMS sandbox. The SMS sandbox
...
// to move out of the sandbox to send messages without restrictions, see SMS
// sandbox (https://docs.aws.amazon.com/sns/latest/dg/sns-sms-sandbox.html) in the
...
func (c *Client) CreateSMSSandboxPhoneNumber(ctx context.Context, params *CreateSMSSandboxPhoneNumberInput, optFns ...func(*Options)) (*CreateSMSSandboxPhoneNumberOutput, error) {
	...
	result, metadata, err := c.invokeOperation(ctx, "CreateSMSSandboxPhoneNumber", params, optFns, addOperationCreateSMSSandboxPhoneNumberMiddlewares)
	if err != nil {
		return nil, err
	}
...
func addOperationCreateSMSSandboxPhoneNumberMiddlewares(stack *middleware.Stack, options Options) (err error) {
