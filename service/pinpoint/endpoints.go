...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/pinpoint/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_PINPOINT")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Pinpoint", cfg.ConfigSources)
	if found && err == nil {
...
var bddNodes = [54]int32{
	-1, 1, -1, 0, 17, 3, 1, 4, 100000015, 2, 5, 100000015, 3, 13, 6, 5, 12, 7, 7, 100000010, 8, 8, 100000011, 9, 9, 100000012, 10, 10, 100000013, 11, 11, 100000013, 100000014, 6, 100000008, 100000009, 4, 15, 14, 5, 100000005, 100000007, 5, 16, 100000006, 6, 100000004, 100000005, 3, 100000001, 18, 5, 100000002, 100000003}
...
func resolveResult(idx int32, params *EndpointParameters, c *conditionContext) (smithyendpoints.Endpoint, error) {
	...
	case 4:
		uriString := func() string {
			...
			out.WriteString("https://pinpoint-fips.")
	...
	case 8:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://pinpoint.")
			out.WriteString(*params.Region)
			out.WriteString(".")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
	...
	case 10:
		uriString := "https://pinpoint.us-east-1.amazonaws.com"
		uri, err := url.Parse(uriString)
		if err != nil {
			return smithyendpoints.Endpoint{}, fmt.Errorf("Failed to parse uri: %s", uriString)
		}
	...
	case 11:
		uriString := "https://pinpoint.us-west-2.amazonaws.com"
		uri, err := url.Parse(uriString)
		if err != nil {
			return smithyendpoints.Endpoint{}, fmt.Errorf("Failed to parse uri: %s", uriString)
		}
	...
	case 12:
		uriString := "https://pinpoint.us-gov-west-1.amazonaws.com"
		uri, err := url.Parse(uriString)
		if err != nil {
			return smithyendpoints.Endpoint{}, fmt.Errorf("Failed to parse uri: %s", uriString)
		}
