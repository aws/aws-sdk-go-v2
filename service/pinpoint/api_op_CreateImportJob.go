...
func (c *Client) CreateImportJob(ctx context.Context, params *CreateImportJobInput, optFns ...func(*Options)) (*CreateImportJobOutput, error) {
	stack := middleware.NewStack("CreateImportJob", smithyhttp.NewStackRequest)
	options := c.options.Copy()
	for _, fn := range optFns {
		fn(&options)
	}
	addawsRestjson1_serdeOpCreateImportJobMiddlewares(stack)
	...
	addOpCreateImportJobValidationMiddleware(stack)
	stack.Initialize.Add(newServiceMetadataMiddleware_opCreateImportJob(options.Region), middleware.Before)
...
func addawsRestjson1_serdeOpCreateImportJobMiddlewares(stack *middleware.Stack) {
	stack.Serialize.Add(&awsRestjson1_serializeOpCreateImportJob{}, middleware.After)
	stack.Deserialize.Add(&awsRestjson1_deserializeOpCreateImportJob{}, middleware.After)
}
