...
// for an input channel. If there’s no existing session for the channel, create a
// new streaming session and provide the Amazon Resource Name (ARN) of the
// signaling channel (channelArn) and client id (clientId). Currently for
...
// producing device and viewers can join a session first and wait for other
...
// generation, and more with ingested WebRTC media. If there’s an existing session
// with the same clientId that's found in the join session request, the new request
// takes precedence.
