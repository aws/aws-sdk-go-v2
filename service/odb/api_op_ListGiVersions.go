// Code generated by smithy-go-codegen DO NOT EDIT.

package odb

import (
	"context"
	"fmt"
	"github.com/aws/aws-sdk-go-v2/service/odb/schemas"
	"github.com/aws/aws-sdk-go-v2/service/odb/types"
	smithy "github.com/aws/smithy-go"
	"github.com/aws/smithy-go/middleware"
)

// Returns information about Oracle Grid Infrastructure (GI) software versions
// that are available for a VM cluster for the specified shape.
func (c *Client) ListGiVersions(ctx context.Context, params *ListGiVersionsInput, optFns ...func(*Options)) (*ListGiVersionsOutput, error) {
	if params == nil {
		params = &ListGiVersionsInput{}
	}

	result, metadata, err := c.invokeOperation(ctx, "ListGiVersions", params, optFns, c.addOperationListGiVersionsMiddlewares)
	if err != nil {
		return nil, err
	}

	out := result.(*ListGiVersionsOutput)
	out.ResultMetadata = metadata
	return out, nil
}

type ListGiVersionsInput struct {

	// The maximum number of items to return for this request. To get the next page of
	// items, make another request with the token returned in the output.
	//
	// Default: 10
	MaxResults *int32

	// The token returned from a previous paginated request. Pagination continues from
	// the end of the items returned by the previous request.
	NextToken *string

	// The shape to return GI versions for. For a list of valid shapes, use the
	// ListDbSystemShapes operation..
	Shape *string

	noSmithyDocumentSerde
}

func (v *ListGiVersionsInput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListGiVersionsInput)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListGiVersionsInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.MaxResults != nil {
		s.WriteInt32(schemas.ListGiVersionsInput_maxResults, *v.MaxResults)
	}
	if v.NextToken != nil {
		s.WriteString(schemas.ListGiVersionsInput_nextToken, *v.NextToken)
	}
	if v.Shape != nil {
		s.WriteString(schemas.ListGiVersionsInput_shape, *v.Shape)
	}
}

type ListGiVersionsOutput struct {

	// The list of GI versions and their properties.
	//
	// This member is required.
	GiVersions []types.GiVersionSummary

	// The token to include in another request to get the next page of items. This
	// value is null when there are no more items to return.
	NextToken *string

	// Metadata pertaining to the operation's result.
	ResultMetadata middleware.Metadata

	noSmithyDocumentSerde
}

func (v *ListGiVersionsOutput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListGiVersionsOutput)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListGiVersionsOutput) SerializeMembers(s smithy.ShapeSerializer) {
	serializeGiVersionList(s, schemas.ListGiVersionsOutput_giVersions, v.GiVersions)
	if v.NextToken != nil {
		s.WriteString(schemas.ListGiVersionsOutput_nextToken, *v.NextToken)
	}
}
func (v *ListGiVersionsOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.ListGiVersionsOutput, func(s *smithy.Schema) error {
		switch s {
		case schemas.ListGiVersionsOutput_giVersions:
			return deserializeGiVersionList(d, schemas.ListGiVersionsOutput_giVersions, &v.GiVersions)
		case schemas.ListGiVersionsOutput_nextToken:
			v.NextToken = new(string)
			return d.ReadString(schemas.ListGiVersionsOutput_nextToken, v.NextToken)
		}
		return nil
	})
}
func (c *Client) addOperationListGiVersionsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.ListGiVersions, schemas.ListGiVersionsInput, schemas.ListGiVersionsOutput)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.ListGiVersions, schemas.ListGiVersionsInput, schemas.ListGiVersionsOutput), output: &ListGiVersionsOutput{}}, middleware.After); err != nil {
		return err
	}

	if err = addResolveEndpointMiddleware(stack, options); err != nil {
		return err
	}
	if err = addComputePayloadSHA256(stack); err != nil {
		return err
	}
	if err = addRecordResponseTiming(stack, options); err != nil {
		return err
	}
	if err = addCredentialSource(stack, options); err != nil {
		return err
	}
	if err = addRequestIDRetrieverMiddleware(stack); err != nil {
		return err
	}
	if err = addResponseErrorMiddleware(stack); err != nil {
		return err
	}
	if err = addRequestResponseLogging(stack, options); err != nil {
		return err
	}
	if err = addDisableHTTPSMiddleware(stack, options); err != nil {
		return err
	}
	if err = addInterceptors(stack, options); err != nil {
		return err
	}
	return nil
}

// ListGiVersionsPaginatorOptions is the paginator options for ListGiVersions
type ListGiVersionsPaginatorOptions struct {
	// The maximum number of items to return for this request. To get the next page of
	// items, make another request with the token returned in the output.
	//
	// Default: 10
	Limit int32

	// Set to true if pagination should stop if the service returns a pagination token
	// that matches the most recent token provided to the service.
	StopOnDuplicateToken bool
}

// ListGiVersionsPaginator is a paginator for ListGiVersions
type ListGiVersionsPaginator struct {
	options   ListGiVersionsPaginatorOptions
	client    ListGiVersionsAPIClient
	params    *ListGiVersionsInput
	nextToken *string
	firstPage bool
}

// NewListGiVersionsPaginator returns a new ListGiVersionsPaginator
func NewListGiVersionsPaginator(client ListGiVersionsAPIClient, params *ListGiVersionsInput, optFns ...func(*ListGiVersionsPaginatorOptions)) *ListGiVersionsPaginator {
	if params == nil {
		params = &ListGiVersionsInput{}
	}

	options := ListGiVersionsPaginatorOptions{}
	if params.MaxResults != nil {
		options.Limit = *params.MaxResults
	}

	for _, fn := range optFns {
		fn(&options)
	}

	return &ListGiVersionsPaginator{
		options:   options,
		client:    client,
		params:    params,
		firstPage: true,
		nextToken: params.NextToken,
	}
}

// HasMorePages returns a boolean indicating whether more pages are available
func (p *ListGiVersionsPaginator) HasMorePages() bool {
	return p.firstPage || (p.nextToken != nil && len(*p.nextToken) != 0)
}

// NextPage retrieves the next ListGiVersions page.
func (p *ListGiVersionsPaginator) NextPage(ctx context.Context, optFns ...func(*Options)) (*ListGiVersionsOutput, error) {
	if !p.HasMorePages() {
		return nil, fmt.Errorf("no more pages available")
	}

	params := *p.params
	params.NextToken = p.nextToken

	var limit *int32
	if p.options.Limit > 0 {
		limit = &p.options.Limit
	}
	params.MaxResults = limit

	optFns = append([]func(*Options){
		addIsPaginatorUserAgent,
	}, optFns...)
	result, err := p.client.ListGiVersions(ctx, &params, optFns...)
	if err != nil {
		return nil, err
	}
	p.firstPage = false

	prevToken := p.nextToken
	p.nextToken = result.NextToken

	if p.options.StopOnDuplicateToken &&
		prevToken != nil &&
		p.nextToken != nil &&
		*prevToken == *p.nextToken {
		p.nextToken = nil
	}

	return result, nil
}

// ListGiVersionsAPIClient is a client that implements the ListGiVersions
// operation.
type ListGiVersionsAPIClient interface {
	ListGiVersions(context.Context, *ListGiVersionsInput, ...func(*Options)) (*ListGiVersionsOutput, error)
}

var _ ListGiVersionsAPIClient = (*Client)(nil)
