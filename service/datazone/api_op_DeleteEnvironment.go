...
func (v *DeleteEnvironmentInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.DomainIdentifier != nil {
		s.WriteString(schemas.DeleteEnvironmentInput_domainIdentifier, *v.DomainIdentifier)
	}
	if v.Identifier != nil {
		s.WriteString(schemas.DeleteEnvironmentInput_identifier, *v.Identifier)
	}
}
...
func (c *Client) addOperationDeleteEnvironmentMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteEnvironment, schemas.DeleteEnvironmentInput, nil)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.DeleteEnvironment, schemas.DeleteEnvironmentInput, nil), output: &DeleteEnvironmentOutput{}}, middleware.After); err != nil {
		return err
	}
