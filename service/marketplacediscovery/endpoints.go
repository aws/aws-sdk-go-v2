...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/marketplacediscovery/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_MARKETPLACE_DISCOVERY")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Marketplace Discovery", cfg.ConfigSources)
	if found && err == nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					...
					out.WriteString("https://discovery-marketplace-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://discovery-marketplace.")
				out.WriteString(_Region)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
