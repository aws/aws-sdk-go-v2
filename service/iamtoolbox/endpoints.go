...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/iamtoolbox/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_IAM_TOOLBOX")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "IAM Toolbox", cfg.ConfigSources)
	if found && err == nil {
...
func resolveResult(idx int32, params *EndpointParameters, c *conditionContext) (smithyendpoints.Endpoint, error) {
	...
	case 4:
		uriString := func() string {
			...
			out.WriteString("https://iam-toolbox-fips.")
	...
	case 8:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://iam-toolbox.")
			out.WriteString(*params.Region)
			out.WriteString(".")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
