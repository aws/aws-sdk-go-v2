...
type ExportTr34KeyBlock struct {
	...
	// wrapped key during TR-34 key block generation. Call GetParametersForExport (https://docs.aws.amazon.com/payment-cryptography/latest/APIReference/API_GetParametersForExport.html)
...
type KeyBlockHeaders struct {
	...
	// Cryptography. When set to EXPORTABLE , the key can be subsequently exported by
	// the receiver under a KEK using TR-31 or TR-34 key block export only. When set to
	...
	// export method. For further information refer to ANSI X9.143-2022 (https://webstore.ansi.org/standards/ascx9/ansix91432022)
	// .
	...
	// for information on allowed data type for optional blocks. Optional block
	// character limit is 112 characters. For each optional block, 2 characters are
	// reserved for optional block ID and 2 characters reserved for optional block
	// length. More than one optional blocks can be included as long as the combined
	// length does not increase 112 characters.
