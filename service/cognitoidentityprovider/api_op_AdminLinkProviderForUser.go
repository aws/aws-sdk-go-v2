...
// name and value from the external IdP. This allows you to create a link from the
// existing user account to an external federated user identity that has not yet
// been used to sign in. You can then use the federated user identity to sign in as
// the existing user account. For example, if there is an existing user with a
...
// existing user account. The maximum number of federated identities linked to a
// user is five. Because this API allows a user with an external federated identity
// to sign in as an existing user in the user pool, it is critical that it only be
// used with external IdPs and provider attributes that have been trusted by the
// application owner. This action is administrative and requires developer
// credentials.
...
type AdminLinkProviderForUserInput struct {
	...
	// DestinationUsershould be the username in the user pool. For a federated user,
	// it should be the provider-specific user_id . The ProviderAttributeName  of the
	// DestinationUser is ignored. The ProviderName  should be set to Cognito for
	// users in Cognito user pools. All attributes in the DestinationUser profile must
	// be mutable. If you have assigned the user any immutable custom attributes, the
	...
	// another native user. If the SourceUser is using a federated social IdP, such
	// as Facebook, Google, or Login with Amazon, you must set the
	// ProviderAttributeName to Cognito_Subject . For social IdPs, the ProviderName
	// will be Facebook , Google , or LoginWithAmazon, and Amazon Cognito will
	// automatically parse the Facebook, Google, and Login with Amazon tokens for id ,
	// sub , and user_id , respectively. The ProviderAttributeValue for the user must
	// be the same value as the id , sub , or user_id value found in the social IdP
	// token. For SAML, the ProviderAttributeName can be any value that matches a
	// claim in the SAML assertion. If you want to link SAML users based on the subject
	// of the SAML assertion, you should map the subject to a claim through the SAML
	// IdP and submit that claim name as the ProviderAttributeName . If you set
	// ProviderAttributeName to Cognito_Subject, Amazon Cognito will automatically
