...
// has not yet been used to sign in. You can then use the federated user identity
// to sign in as the existing user account. For example, if there is an existing
// user with a username and password, this API links that user to a federated user
// identity. When the user signs in with a federated user identity, they sign in as
// the existing user account. The maximum number of federated identities linked to
// a user is 5. Because this API allows a user with an external federated identity
// to sign in as an existing user in the user pool, it is critical that it only be
// used with external identity providers and provider attributes that have been
// trusted by the application owner. This action is administrative and requires
...
type AdminLinkProviderForUserInput struct {
	...
	// An external identity provider account for a user who doesn't exist yet in the
	// user pool. This user must be a federated user (for example, a SAML or Facebook
	// user), not another native user. If the SourceUser is using a federated social
	// identity provider, such as Facebook, Google, or Login with Amazon, you must set
	// the ProviderAttributeName to Cognito_Subject. For social identity providers, the
	// ProviderName will be Facebook, Google, or LoginWithAmazon, and Amazon Cognito
	// will automatically parse the Facebook, Google, and Login with Amazon tokens for
	// id, sub, and user_id, respectively. The ProviderAttributeValue for the user must
	// be the same value as the id, sub, or user_id value found in the social identity
	// provider token. For SAML, the ProviderAttributeName can be any value that
	// matches a claim in the SAML assertion. If you want to link SAML users based on
