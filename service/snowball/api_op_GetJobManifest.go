...
// API as the manifest and UnlockCode code value are used for securing your
// device and should only be used when you have the device. The manifest is
// decrypted by using the UnlockCode code value, when you pass both values to the
// Snow device through the Snowball client when the client is started for the first
// time. As a best practice, we recommend that you don't save a copy of an
// UnlockCodevalue in the same location as the manifest file for that job. Saving
// these separately helps prevent unauthorized parties from gaining access to the
// Snow device associated with that job. The credentials of a given job, including
// its manifest file and unlock code, expire 360 days after the job is created.
