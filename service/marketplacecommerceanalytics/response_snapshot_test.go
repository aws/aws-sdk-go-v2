...
func TestCheckResponseSnapshot_GenerateDataSet(t *testing.T) {
	want := &GenerateDataSetOutput{
		DataSetRequestId: ptr.String("__DataSetRequestId__"),
	}
	status, header, body, err := serdeRespReadSnapshot("GenerateDataSet.response")
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "GenerateDataSet.response", err)
	}
}

func TestCheckResponseSnapshot_StartSupportDataExport(t *testing.T) {
	want := &StartSupportDataExportOutput{
		DataSetRequestId: ptr.String("__DataSetRequestId__"),
	}
	status, header, body, err := serdeRespReadSnapshot("StartSupportDataExport.response")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "StartSupportDataExport.response", err)
	}
}

func TestCheckResponseSnapshot_Error_MarketplaceCommerceAnalyticsException(t *testing.T) {
	want := &types.MarketplaceCommerceAnalyticsException{
		Message: ptr.String("__Message__"),
	}
	status, header, body, err := serdeRespReadSnapshot("MarketplaceCommerceAnalyticsException.error")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	var got *types.MarketplaceCommerceAnalyticsException
	if !errors.As(opErr, &got) {
		t.Fatalf("expected types.MarketplaceCommerceAnalyticsException, got %v", opErr)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("error response snapshot mismatch for %s: %v", "MarketplaceCommerceAnalyticsException.error", err)
	}
}
