...
type DeleteResourcePolicyInput struct {
	...
	// The revision ID of the policy to delete. Use this parameter to ensure that you
	// are deleting the correct version of the policy.
	RevisionId *string
...
func (v *DeleteResourcePolicyInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.ClientToken != nil {
		s.WriteString(schemas.DeleteResourcePolicyRequest_clientToken, *v.ClientToken)
	}
	if v.RevisionId != nil {
		s.WriteString(schemas.DeleteResourcePolicyRequest_revisionId, *v.RevisionId)
	}
	if v.WorkspaceId != nil {
		s.WriteString(schemas.DeleteResourcePolicyRequest_workspaceId, *v.WorkspaceId)
	}
}
...
func (c *Client) addOperationDeleteResourcePolicyMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = addIdempotencyToken_opDeleteResourcePolicyMiddleware(stack, options); err != nil {
		return err
	}
...
type idempotencyToken_initializeOpDeleteResourcePolicy struct {
	tokenProvider IdempotencyTokenProvider
}

func (*idempotencyToken_initializeOpDeleteResourcePolicy) ID() string {
	return "OperationIdempotencyTokenAutoFill"
}

func (m *idempotencyToken_initializeOpDeleteResourcePolicy) HandleInitialize(ctx context.Context, in middleware.InitializeInput, next middleware.InitializeHandler) (
	...
	if !ok {
		return out, metadata, fmt.Errorf("expected middleware input to be of type *DeleteResourcePolicyInput ")
	}
...
func addIdempotencyToken_opDeleteResourcePolicyMiddleware(stack *middleware.Stack, cfg Options) error {
	return stack.Initialize.Add(&idempotencyToken_initializeOpDeleteResourcePolicy{tokenProvider: cfg.IdempotencyTokenProvider}, middleware.Before)
}
