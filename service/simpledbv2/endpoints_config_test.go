...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-simpledbv2.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-simpledbv2.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-simpledbv2.dev",
			expectURL:     aws.String("https://env-simpledbv2.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-simpledbv2.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-simpledbv2.dev",
			expectURL:     aws.String("http://config-simpledbv2.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-simpledbv2.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-simpledbv2.dev",
			clientEndpoint: aws.String("https://client-simpledbv2.dev"),
			expectURL:      aws.String("https://client-simpledbv2.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_SIMPLEDBV2", c.envService)
			}
