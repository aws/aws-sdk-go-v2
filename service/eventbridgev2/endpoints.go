...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/eventbridgev2/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_EVENTBRIDGEV2")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "EventBridgeV2", cfg.ConfigSources)
	if found && err == nil {
...
type EndpointParameters struct {
	...
	//
	// Defaults
	// to false if no value is provided.
	//
	// AWS::UseFIPS
	UseFIPS *bool
	...
	// The AWS account ID used for the request, sourced from the caller's
	// credentials.
	//
	...
	// The bus ARN from the request, bound via @contextParam. Its account is the bus
	// owner and takes precedence over the credentials-sourced account for cell routing
	// (cross-account calls).
	//
	// Parameter is required.
	EventBusArn *string

	// The behavior for account ID based endpoint routing: preferred, disabled, or
	// required.
	//
	// Parameter is required.
	//
...
var bddNodes = [105]int32{
	-1, 1, -1, 0, 34, 3, 1, 4, 100000000, 2, 5, 27, 3, 26, 6, 4, 7, 10, 5, 8, 10, 6, 9, 10, 7, 19, 10, 8, 11, 26, 10, 12, 100000015, 11, 15, 13, 12, 14, 100000014, 13, 100000013, 100000009, 12, 17, 16, 14, 100000012, 100000007, 13, 18, 100000005, 14, 100000011, 100000005, 11, 22, 20, 12, 21, 100000010, 13, 100000008, 100000009, 12, 24, 23, 14, 100000006, 100000007, 13, 25, 100000005, 14, 100000004, 100000005, 9, 100000016, 27, 11, 30, 28, 12, 29, 100000020, 13, 100000019, 100000009, 12, 32, 31, 14, 100000018, 100000007, 13, 33, 100000005, 14, 100000017, 100000005, 11, 100000001, 35, 12, 100000002, 100000003}

type conditionContext struct {
	PartitionResult *awsrulesfn.PartitionConfig
	ParsedBusArn    *awsrulesfn.ARN
}
...
func evalCondition(idx int, params *EndpointParameters, c *conditionContext) bool {
	...
	case 4:
		return params.EventBusArn != nil
	case 5:
		if v := awsrulesfn.ParseARN(*params.EventBusArn); v != nil {
			c.ParsedBusArn = v
			return true
		}
		return false
	case 6:
		return c.ParsedBusArn.Service == "events"
	case 7:
		return rulesfn.IsValidHostLabel(c.ParsedBusArn.AccountId, false)
	case 8:
...
func resolveResult(idx int32, params *EndpointParameters, c *conditionContext) (smithyendpoints.Endpoint, error) {
	...
	case 4:
		uriString := func() string {
			...
			out.WriteString(c.ParsedBusArn.AccountId)
			out.WriteString(".eventsv2-fips.")
	...
	case 17:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://eventsv2-fips.")
			out.WriteString(*params.Region)
			out.WriteString(".")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
	...
	case 19:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://eventsv2.")
			out.WriteString(*params.Region)
			out.WriteString(".")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
