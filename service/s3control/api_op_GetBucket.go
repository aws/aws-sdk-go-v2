...
// the account that owns the Outposts bucket, the calling identity must have the
// s3-outposts:GetBucket permissions on the specified Outposts bucket and belong to
// the Outposts bucket owner's account in order to use this action. Only users from
// Outposts bucket owner account with the right permissions can perform actions on
// an Outposts bucket. If you don't have s3-outposts:GetBucket permissions or
// you're not using an identity that belongs to the bucket owner's account, Amazon
// S3 returns a 403 Access Denied error. The following actions are related to
// GetBucket for Amazon S3 on Outposts: All Amazon S3 on Outposts REST API requests
