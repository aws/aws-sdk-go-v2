...
//
// A replication
// configuration must include at least one rule and can contain a maximum of 100.
// Each rule identifies a subset of objects to replicate by filtering the objects
// in the source Outposts bucket. To choose additional subsets of objects to
// replicate, add a rule for each subset. To specify a subset of the objects in the
// source Outposts bucket to apply a replication rule to, add the Filter element
// as a child of the Rule element. You can filter objects based on an object key
// prefix, one or more object tags, or both. When you add the Filter element in
// the configuration, you must also add the following elements:
// DeleteMarkerReplication , Status , and Priority . Using PutBucketReplication
// on Outposts requires that both the source and destination buckets must have
...
// s3-outposts:PutReplicationConfigurationpermissions for the bucket. The
