...
func (v *CreateAuthorizerInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.AuthType != nil {
		s.WriteString(schemas.CreateAuthorizerRequest_authType, *v.AuthType)
	}
	if v.AuthorizerCredentials != nil {
		s.WriteString(schemas.CreateAuthorizerRequest_authorizerCredentials, *v.AuthorizerCredentials)
	}
	...
	if v.IdentitySource != nil {
		s.WriteString(schemas.CreateAuthorizerRequest_identitySource, *v.IdentitySource)
	}
	...
	serializeListOfARNs(s, schemas.CreateAuthorizerRequest_providerARNs, v.ProviderARNs)
	if v.RestApiId != nil {
		s.WriteString(schemas.CreateAuthorizerRequest_restApiId, *v.RestApiId)
	}
	if v.Type != "" {
		s.WriteString(schemas.CreateAuthorizerRequest_type, string(v.Type))
	}
}
...
func (c *Client) addOperationCreateAuthorizerMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.CreateAuthorizer, schemas.CreateAuthorizerRequest, schemas.Authorizer)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.CreateAuthorizer, schemas.CreateAuthorizerRequest, schemas.Authorizer), output: &CreateAuthorizerOutput{}}, middleware.After); err != nil {
		return err
	}
