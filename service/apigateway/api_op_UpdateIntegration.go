...
func (v *UpdateIntegrationInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.HttpMethod != nil {
		s.WriteString(schemas.UpdateIntegrationRequest_httpMethod, *v.HttpMethod)
	}
	serializeListOfPatchOperation(s, schemas.UpdateIntegrationRequest_patchOperations, v.PatchOperations)
	if v.ResourceId != nil {
		s.WriteString(schemas.UpdateIntegrationRequest_resourceId, *v.ResourceId)
	}
	if v.RestApiId != nil {
		s.WriteString(schemas.UpdateIntegrationRequest_restApiId, *v.RestApiId)
	}
}
...
func (c *Client) addOperationUpdateIntegrationMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.UpdateIntegration, schemas.UpdateIntegrationRequest, schemas.Integration)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.UpdateIntegration, schemas.UpdateIntegrationRequest, schemas.Integration), output: &UpdateIntegrationOutput{}}, middleware.After); err != nil {
		return err
	}
