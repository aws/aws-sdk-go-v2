...
// weighted random set of machines is sampled on a ReceiveMessage call. Thus,
...
// messages than you requested per ReceiveMessage call. If the number of messages
// in the queue is extremely small, you might not receive any messages in a
// particular ReceiveMessage response. If this happens, repeat the request. For
...
//
// The receipt handle is the
// identifier you must provide when deleting the message. For more information, see
//
...
type ReceiveMessageInput struct {
	...
	// used for deduplication of ReceiveMessage calls. If a networking issue occurs
	// after a ReceiveMessage action, and instead of a response you receive a generic
	// error, it is possible to retry the same action with an identical
	// ReceiveRequestAttemptIdto retrieve the same set of messages, even if their
