...
// can change given different modifications to the baseline time series. For
...
// your goals. You could create a scenario where everything is given a 25%
...
// different scenarios against each other. Note that a what-if analysis is meant to
// display what the forecasting model has learned and how it will behave in the
// scenarios that you are evaluating. Do not blindly use the results of the what-if
// analysis to make business decisions. For instance, forecasts might not be
// accurate for novel scenarios where there is no reference available to determine
// whether a forecast is good. The TimeSeriesSelector object defines the items
// that you want in the what-if analysis.
...
type CreateWhatIfAnalysisInput struct {
	...
	// TimeSeriesIdentifiersobject. What-if analyses are performed only for the time
