...
// - The private key must be unencrypted. You cannot
// import a private key that is protected by a password or a passphrase.
// - The
// private key must be no larger than 5 KB (5,120 bytes).
// - If the certificate you
// are importing is not self-signed, you must enter its certificate chain.
// - If a
// certificate chain is included, the issuer must be the subject of one of the
// certificates in the chain.
// - The certificate, private key, and certificate chain
// must be PEM-encoded.
// - The current time must be between the Not Before and Not
// After certificate fields.
// - The Issuer field must not be empty.
// - The OCSP
// authority URL, if present, must not exceed 1000 characters.
// - To import a new
// certificate, omit the CertificateArn argument. Include this argument only when
// you want to replace a previously imported certificate.
// - When you import a
...
// by using an SDK, you must specify the certificate, the certificate chain, and
// the private key files in the manner required by the programming language you're
// using.
