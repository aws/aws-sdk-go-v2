...
type CreateTrailInput struct {
	...
	// IAM to the log files. Setting this value to true only delivers global service
	// events to the trail if the trail is multi-Region or if the trail's home Region
	// is the partition leader Region (for example, us-east-1).
...
func (v *CreateTrailInput) SerializeMembers(s smithy.ShapeSerializer) {
	...
	if v.RecursiveLogging != nil {
		s.WriteBool(schemas.CreateTrailRequest_RecursiveLogging, *v.RecursiveLogging)
	}
...
func (v *CreateTrailOutput) SerializeMembers(s smithy.ShapeSerializer) {
	...
	if v.RecursiveLogging != nil {
		s.WriteBool(schemas.CreateTrailResponse_RecursiveLogging, *v.RecursiveLogging)
	}
	if v.S3BucketName != nil {
...
func (v *CreateTrailOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.CreateTrailResponse, func(s *smithy.Schema) error {
		...
		case schemas.CreateTrailResponse_RecursiveLogging:
			v.RecursiveLogging = new(bool)
			return d.ReadBool(schemas.CreateTrailResponse_RecursiveLogging, v.RecursiveLogging)
