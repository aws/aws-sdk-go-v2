...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/bcmrecommendedactions/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_BCM_RECOMMENDED_ACTIONS")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "BCM Recommended Actions", cfg.ConfigSources)
	if found && err == nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					...
					out.WriteString("https://bcm-recommended-actions-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://bcm-recommended-actions.")
				out.WriteString(_PartitionResult.ImplicitGlobalRegion)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
