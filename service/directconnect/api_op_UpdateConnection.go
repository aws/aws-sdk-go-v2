...
func (v *UpdateConnectionInput) SerializeMembers(s smithy.ShapeSerializer) {
	...
	if v.ConnectionName != nil {
		s.WriteString(schemas.UpdateConnectionRequest_connectionName, *v.ConnectionName)
	}
	if v.EncryptionMode != nil {
		s.WriteString(schemas.UpdateConnectionRequest_encryptionMode, *v.EncryptionMode)
	}
}
...
func (c *Client) addOperationUpdateConnectionMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.UpdateConnection, schemas.UpdateConnectionRequest, schemas.Connection)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.UpdateConnection, schemas.UpdateConnectionRequest, schemas.Connection), output: &UpdateConnectionOutput{}}, middleware.After); err != nil {
		return err
	}
