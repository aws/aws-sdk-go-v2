...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-route53profiles.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53profiles.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53profiles.dev",
			expectURL:     aws.String("https://env-route53profiles.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-route53profiles.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53profiles.dev",
			expectURL:     aws.String("http://config-route53profiles.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-route53profiles.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-route53profiles.dev",
			clientEndpoint: aws.String("https://client-route53profiles.dev"),
			expectURL:      aws.String("https://client-route53profiles.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ROUTE53PROFILES", c.envService)
			}
