...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-s3files.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3files.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3files.dev",
			expectURL:     aws.String("https://env-s3files.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-s3files.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3files.dev",
			expectURL:     aws.String("http://config-s3files.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-s3files.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-s3files.dev",
			clientEndpoint: aws.String("https://client-s3files.dev"),
			expectURL:      aws.String("https://client-s3files.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_S3FILES", c.envService)
			}
