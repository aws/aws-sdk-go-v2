...
func (v *ExecutionTimeoutException) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ExecutionTimeoutException)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ExecutionTimeoutException) SerializeMembers(s smithy.ShapeSerializer) {
	if v.Message != nil {
		s.WriteString(schemas.ExecutionTimeoutException_message, *v.Message)
	}
}
func (v *ExecutionTimeoutException) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.ExecutionTimeoutException, func(s *smithy.Schema) error {
		switch s {
		case schemas.ExecutionTimeoutException_message:
			v.Message = new(string)
			return d.ReadString(schemas.ExecutionTimeoutException_message, v.Message)
		}
		return nil
	})
}
