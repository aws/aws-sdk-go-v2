...
// StartDocumentTextDetection . GetDocumentTextDetection  returns an array of
...
// lines of detected text on a page. A LINE Block object is a parent for each
// word that makes up the line. Words are represented by Block objects of type
// WORD. Use the MaxResults parameter to limit the number of blocks that are
// returned. If there are more results than specified in MaxResults , the value of
// NextTokenin the operation response contains a pagination token for getting the
// next set of results. To get the next page of results, call
// GetDocumentTextDetection , and populate the NextToken request parameter with
// the token value that's returned from the previous call to
// GetDocumentTextDetection . For more information, see Document Text Detection (https://docs.aws.amazon.com/textract/latest/dg/how-it-works-detecting.html)
// .
