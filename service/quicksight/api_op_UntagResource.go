...
func (v *UntagResourceOutput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.RequestId != nil {
		s.WriteString(schemas.UntagResourceResponse_RequestId, *v.RequestId)
	}
	if v.Status != 0 {
		s.WriteInt32(schemas.UntagResourceResponse_Status, v.Status)
	}
}
func (v *UntagResourceOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.UntagResourceResponse, func(s *smithy.Schema) error {
		switch s {
		case schemas.UntagResourceResponse_RequestId:
			v.RequestId = new(string)
			return d.ReadString(schemas.UntagResourceResponse_RequestId, v.RequestId)
		case schemas.UntagResourceResponse_Status:
			return d.ReadInt32(schemas.UntagResourceResponse_Status, &v.Status)
		}
		return nil
	})
}
