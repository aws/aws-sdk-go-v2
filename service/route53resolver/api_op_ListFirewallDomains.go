...
func (v *ListFirewallDomainsInput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListFirewallDomainsRequest)
...
func (v *ListFirewallDomainsInput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.FirewallDomainListId != nil {
		s.WriteString(schemas.ListFirewallDomainsRequest_FirewallDomainListId, *v.FirewallDomainListId)
	}
	if v.MaxResults != nil {
		s.WriteInt32(schemas.ListFirewallDomainsRequest_MaxResults, *v.MaxResults)
	}
	if v.NextToken != nil {
		s.WriteString(schemas.ListFirewallDomainsRequest_NextToken, *v.NextToken)
	}
}
...
func (v *ListFirewallDomainsOutput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListFirewallDomainsResponse)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListFirewallDomainsOutput) SerializeMembers(s smithy.ShapeSerializer) {
	serializeFirewallDomains(s, schemas.ListFirewallDomainsResponse_Domains, v.Domains)
	if v.NextToken != nil {
		s.WriteString(schemas.ListFirewallDomainsResponse_NextToken, *v.NextToken)
	}
}
func (v *ListFirewallDomainsOutput) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.ListFirewallDomainsResponse, func(s *smithy.Schema) error {
		switch s {
		case schemas.ListFirewallDomainsResponse_Domains:
			return deserializeFirewallDomains(d, schemas.ListFirewallDomainsResponse_Domains, &v.Domains)
		case schemas.ListFirewallDomainsResponse_NextToken:
			v.NextToken = new(string)
			return d.ReadString(schemas.ListFirewallDomainsResponse_NextToken, v.NextToken)
		}
		return nil
	})
}
func (c *Client) addOperationListFirewallDomainsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := stack.Serialize.Add(&serializeRequestMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.ListFirewallDomains, schemas.ListFirewallDomainsRequest, schemas.ListFirewallDomainsResponse)}, middleware.After); err != nil {
		return err
	}
	if err := stack.Deserialize.Add(&deserializeResponseMiddleware{options: &options, operationSchema: smithy.NewOperationSchema(schemas.ListFirewallDomains, schemas.ListFirewallDomainsRequest, schemas.ListFirewallDomainsResponse), output: &ListFirewallDomainsOutput{}}, middleware.After); err != nil {
		return err
	}
