...
// records. Kinesis Data Streams attempts to process all records in each
// PutRecordsrequest. A single record failure does not stop the processing of
...
// stream, use PutRecord  instead of PutRecords, and write to the same shard. A
// successfully processed record includes ShardId  and SequenceNumber  values. The
// ShardIdparameter identifies the shard in the stream where the record is
// stored. The SequenceNumber parameter is an identifier assigned to the put
// record, unique to all records in the stream. An unsuccessfully processed record
...
// ProvisionedThroughputExceededExceptionexception including the account ID,
