...
// expected increase in the volume of data records being ingested. This API is only
...
// hash key values for the ShardToSplit  and NewStartingHashKey parameters that
// are specified in the SplitShard  request. SplitShard is an asynchronous
// operation. Upon receiving a SplitShard request, Kinesis Data Streams
// immediately returns a response and sets the stream status to UPDATING. After
// the operation is completed, Kinesis Data Streams sets the stream status to
// ACTIVE . Read and write operations continue to work while the stream is in the
// UPDATING state. You can use DescribeStreamSummary to check the status of the
...
// DescribeStreamSummary returns a ResourceNotFoundException. If you try to
// create more shards than are authorized for your account, you receive a
// LimitExceededException. For the default shard limit for an Amazon Web Services
// account, see Kinesis Data Streams Limits (https://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html)
...
// LimitExceededException . SplitShard has a limit of five transactions per
// second per account.
...
type SplitShardInput struct {
	...
	// range of hash keys being mapped into the shard. The NewStartingHashKey hash
	// key value and all higher hash key values in hash key range are distributed to
	// one of the child shards. All the lower hash key values in the range are
