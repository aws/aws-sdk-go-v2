...
// after it is returned to the requester. When invoking this API, it is recommended
// you use the StreamARN  input parameter rather than the StreamName input
// parameter. A shard iterator specifies the shard position from which to start
// reading data records sequentially. The position is specified using the sequence
...
// record is put into the stream. Each stream has one or more shards. You must
// specify the shard iterator type. For example, you can set the ShardIteratorType
// parameter to read exactly from the position denoted by a specific sequence
// number by using the AT_SEQUENCE_NUMBER shard iterator type. Alternatively, the
// parameter can read right after the sequence number by using the
// AFTER_SEQUENCE_NUMBERshard iterator type, using sequence numbers returned by
// earlier calls to PutRecord , PutRecords , GetRecords , or DescribeStream. In
// the request, you can specify the shard iterator type AT_TIMESTAMP to read
// records from an arbitrary point in time, TRIM_HORIZON  to cause ShardIterator
// to point to the last untrimmed record in the shard in the system (the oldest
// data record in the shard), or LATEST so that you always read the most recent
// data in the shard. When you read repeatedly from a stream, use a
// GetShardIterator request to get the first shard iterator for use in your first
// GetRecordsrequest and for subsequent reads use the shard iterator returned by
// the GetRecords  request in NextShardIterator. A new shard iterator is returned
// by every GetRecords  request in NextShardIterator , which you use in the
// ShardIterator parameter of the next GetRecords  request. If a GetShardIterator
...
// GetShardIteratorreturns a valid iterator for the last sequence number of the
// shard. A shard can be closed as a result of using SplitShard  or MergeShards .
// GetShardIteratorhas a limit of five transactions per second per account per
// open shard.
...
type GetShardIteratorInput struct {
	...
	// . If a record with this exact time stamp does not exist, the iterator returned
	// is for the next (later) record. If the time stamp is older than the current trim
