...
// number or other attribute that marks it as the last record to process. Each data
...
// or throughput by using the Limit parameter to specify the maximum number of
...
// call is 10,000. The size of the data returned by GetRecords varies depending
// on the utilization of the shard. It is recommended that consumer applications
// retrieve records via the GetRecords command using the 5 TPS limit to remain
// caught up. Retrieving records less frequently can lead to consumer applications
// falling behind. The maximum size of data that GetRecords can return is 10 MiB.
// If a call returns this amount of data, subsequent calls made within the next 5
...
// other mechanisms (see Monitoring (https://docs.aws.amazon.com/kinesis/latest/dev/monitoring.html)
