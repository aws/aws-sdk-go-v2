...
// Composes an email message and immediately queues it for sending. This
// operation is more flexible than the SendEmail  API operation. When you use the
// SendRawEmailoperation, you can specify the headers of the message as well as
// its content. This flexibility is useful, for example, when you want to send a
...
// attachments. The SendRawEmail  operation has the following requirements:
...
type SendRawEmailInput struct {
	...
	// Instead of using this parameter, you can use the X-header X-SES-FROM-ARN in
	// the raw message of the email. If you use both the FromArn parameter and the
	...
	// and the ReturnPath  to be feedback@example.com. Instead of using this
	// parameter, you can use the X-header X-SES-RETURN-PATH-ARN in the raw message
	// of the email. If you use both the ReturnPathArn parameter and the
	// corresponding X-header, Amazon SES uses the value of the ReturnPathArn
	// parameter. For information about when to use this parameter, see the description
	// of SendRawEmail  in this guide, or see the Amazon SES Developer Guide (https://docs.aws.amazon.com/ses/latest/DeveloperGuide/sending-authorization-delegate-sender-tasks-email.html)
	// .
	ReturnPathArn *string

	// The identity's email address. If you do not provide a value for this
	// parameter, you must specify a "From" address in the raw text of the message.
	// (You can also specify both.) Amazon SES does not support the SMTPUTF8 extension,
	// as described in RFC6531 (https://tools.ietf.org/html/rfc6531). For this
	// reason, the local part of a source email address (the part of the email address
	// that precedes the @ sign) may only contain 7-bit ASCII characters (https://en.wikipedia.org/wiki/Email_address#Local-part)
	...
	// known as the friendly name) may contain non-ASCII characters. These characters
	// must be encoded using MIME encoded-word syntax, as described in RFC 2047 (https://tools.ietf.org/html/rfc2047)
	...
	// and the Source  to be user@example.com. Instead of using this parameter, you
	// can use the X-header X-SES-SOURCE-ARN in the raw message of the email. If you
	// use both the SourceArn parameter and the corresponding X-header, Amazon SES
	// uses the value of the SourceArn parameter. For information about when to use
	// this parameter, see the description of SendRawEmail  in this guide, or see the
