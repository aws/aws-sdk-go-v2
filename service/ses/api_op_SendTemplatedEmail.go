...
// only send to verified addresses or domains, or to email addresses associated
...
// - Calls
// to the SendTemplatedEmail operation may only include one Destination parameter.
// A destination is a set of recipients who will receive the same version of the
// email. The Destination parameter can include up to 50 recipients, across the
// To:, CC: and BCC: fields.
// - The Destination parameter must include at least one
...
// is not in the format UserName@[SubDomain.]Domain.TopLevelDomain), the entire
...
// doesn't send the email. Additionally, because it already accepted the message,
// Amazon SES doesn't return a message stating that it was unable to send the
// email. For these reasons, we highly recommend that you set up Amazon SES to send
// you notifications when Rendering Failure events occur. For more information, see
