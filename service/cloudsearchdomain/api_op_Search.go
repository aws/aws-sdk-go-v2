...
type SearchInput struct {
	...
	// specified in the queryOptions  parameter. By default, the simple query parser
	// is used to process requests. To use the structured , lucene , or dismax query
	...
	// {"EXPRESSIONNAME":"EXPRESSION"}. You can define and use multiple expressions
	// in a search request. For example: {"expression1":"_score*rating",
	...
	// facet count, with the facet values that have the most matching documents listed
	// first. Setting the size  option to 3 returns only the top three facet values.
	...
	// only which matching documents are included in the results, it has no effect on
	// how they are scored and sorted. The filterQuery parameter supports the full
	...
	// {"OPTION1":"VALUE1","OPTION2":VALUE2"..."OPTIONN":"VALUEN"}.The options you
	// can configure vary according to which parser you use:
	...
	//
	// sortparameter is specified, results are sorted by their default relevance
	// scores in descending order: _score desc . You can also sort by document ID (
	// _id asc ) and version ( _version desc ). For more information, see Sorting
