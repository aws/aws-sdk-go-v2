...
// started by StartLabelDetection. The label detection operation is started by a
// call to StartLabelDetection  which returns a job identifier ( JobId). When the
...
// is SUCCEEDED . If so, call GetLabelDetection  and pass the job identifier (
// JobId ) from the initial call to StartLabelDetection . GetLabelDetection
// returns an array of detected labels ( Labels) sorted by the time the labels
// were detected. You can also sort by the label name by specifying NAME  for the
// SortBy input parameter. If there is no NAME specified, the default sort is by
// timestamp. You can select how results are aggregated by using the AggregateBy
// input parameter. The default aggregation method is TIMESTAMPS. You can also
// aggregate by SEGMENTS, which aggregates all instances of labels detected in a
// given segment. The returned Labels array may include the following attributes:
...
// if aggregation is done by TIMESTAMPS . If aggregating by SEGMENTS, information
// about detected instances isn’t returned. The version of the label model used for
// the detection is also returned. Note DominantColors  isn't returned for
// Instances, although it is shown as part of the response in the sample seen
// below. Use MaxResults parameter to limit the number of labels returned. If
...
// of results. To get the next page of results, call GetlabelDetection and
...
// GetLabelDetectionPaginatorOptions is the paginator options for
// GetLabelDetection
