...
// SearchFacesByImageoperations. For more information, see Adding faces to a
...
// number of faces to index with the MaxFaces input parameter. This is useful
// when you want to index the largest faces in an image and don't want to index
// smaller faces, such as those belonging to people standing in the background. The
//
// QualityFilterinput parameter allows you to filter out detected faces that
...
// If you do not want to filter detected faces, specify NONE. To use quality
// filtering, you need a collection associated with version 3 of the face model or
// higher. To get the version of the face model associated with a collection, call
// DescribeCollection. Information about faces detected in an image, but not
// indexed, is returned in an array of UnindexedFace  objects, UnindexedFaces.
...
//
// In
// response, the IndexFaces operation returns an array of metadata for all
...
//
// If
// you request all facial attributes (by using the detectionAttributes
...
type IndexFacesInput struct {
	...
	// isn't supported. If you are using an AWS SDK to call Amazon Rekognition, you
	// might not need to base64-encode image bytes passed using the Bytes field. For
