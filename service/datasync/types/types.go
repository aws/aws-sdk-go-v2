...
// , "Operator": "Equals" , and "Values": "S3" . For more information, see
...
type Options struct {
	...
	// preserve the original Atime attribute on all source files (that is, the
	// version before the PREPARING  phase of the task execution). The behavior of
	// Atimeisn't fully standard across platforms, so DataSync can only do this on a
	// best-effort basis. Default value: BEST_EFFORT BEST_EFFORT: Attempt to preserve
	// the per-file Atime  value (recommended). NONE : Ignore Atime . If Atime is set
	// to BEST_EFFORT , Mtime  must be set to PRESERVE . If Atime  is set to NONE ,
	// Mtime must also be NONE .
	Atime Atime
	...
	// is required when you need to run the a task more than once. Default Value:
	// PRESERVE PRESERVE : Preserve original Mtime  (recommended) NONE : Ignore Mtime
	// . If Mtime  is set to PRESERVE , Atime  must be set to BEST_EFFORT . If Mtime
	// is set to NONE , Atime  must also be set to NONE .
	Mtime Mtime
	...
	// preserved. If set to NEVER, a destination file for example will not be
	// replaced by a source file (even if the destination file differs from the source
	// file). If you modify files in the destination and you sync the files, you can
	// use this value to protect against overwriting those changes. Some storage
	// classes have specific behaviors that can affect your Amazon S3 storage cost. For
	// detailed information, see Considerations when working with Amazon S3 storage
	...
	// source. If you set this parameter to REMOVE , you can't set TransferMode  to
	// ALL. When you transfer all data, DataSync doesn't scan your destination
	...
	// . Default value: OWNER_DACL OWNER_DACL: For each copied object, DataSync
	// copies the following metadata:
	//     - The object owner.
...
type QopConfiguration struct {
	...
	// corresponds to your hadoop.rpc.protection  setting in your core-site.xml file
...
// for the location. For more information, see filtering DataSync resources (https://docs.aws.amazon.com/datasync/latest/userguide/query-resources.html)
// .
