...
type Options struct {
	...
	// required when you need to run the a task more than once. Default Value:
	...
	// reading, writing, or execution of the file. For more information, see Metadata
	// copied by DataSync
	// (https://docs.aws.amazon.com/datasync/latest/userguide/special-files.html#metadata-copied).
	...
	// NONENONE: Ignore special devices (recommended). PRESERVE: Preserve character and
	// block device metadata. This option currently isn't supported for Amazon EFS.
	PreserveDevices PreserveDevices

	// Specifies which components of the SMB security descriptor are copied from source
	// to destination objects. This value is only used for transfers between SMB and
	// Amazon FSx for Windows File Server locations or between two FSx for Windows File
	// Server locations. For more information, see how DataSync handles metadata
	...
	// is ENABLED, which means the tasks will be queued. If you use the same agent to
	// run multiple tasks, you can enable the tasks to run in series. For more
	...
	// from the source (without comparing what's in the destination). CHANGED: DataSync
	// copies only data or metadata that is new or different content from the source
	// location to the destination location. ALL: DataSync copies all source location
	// content to the destination (without comparing what's in the destination).
	TransferMode TransferMode

	// Specifies the POSIX user ID (UID) of the file's owner. For more information, see
	...
	// transfer. Default value: POINT_IN_TIME_CONSISTENTONLY_FILES_TRANSFERRED
	...
	// (https://docs.aws.amazon.com/datasync/latest/userguide/create-s3-location.html#using-storage-classes).
	// POINT_IN_TIME_CONSISTENT: At the end of the transfer, DataSync scans the entire
	// source and destination to verify that both locations are fully synchronized. You
	// can't use this option when transferring to S3 Glacier Flexible Retrieval or S3
	// Glacier Deep Archive storage classes. For more information, see Storage class
