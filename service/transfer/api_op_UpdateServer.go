...
type UpdateServerInput struct {
	...
	// within your VPC. It is recommended that you use VPC as the EndpointType. With
	// this endpoint type, you have the option to directly associate up to three
	// Elastic IPv4 addresses (BYO IP included) with your server's endpoint and use VPC
	// security groups to restrict traffic by the client's public IP address. This is
	...
	// existing file transfer protocol-enabled server to a new server, don't update the
	// host key. Accidentally changing a server's host key can be disruptive. For more
...
func addOperationUpdateServerStackSlots(stack *middleware.Stack) error {
...
func addOperationUpdateServerMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := addOperationUpdateServerStackSlots(stack); err != nil {
		return err
	}
