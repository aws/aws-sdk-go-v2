...
type CreateAccessInput struct {
	...
	// must specify the Entry  and Target  pair, where Entry shows how the path is
	// made visible and Target is the actual Amazon S3 or Amazon EFS path. If you
	// only specify a target, it is displayed as is. You also must ensure that your
	// Identity and Access Management (IAM) role provides access to paths in Target.
	// This value can be set only when HomeDirectoryType is set to LOGICAL. The
	// following is an Entry  and Target  pair example. [ { "Entry": "/directory1",
	...
	// transfer protocol clients. If you set it LOGICAL, you need to provide mappings
	// in the HomeDirectoryMappings for how you want to make Amazon S3 or Amazon EFS
	...
	// inside this policy include ${Transfer:UserName} , ${Transfer:HomeDirectory},
	// and ${Transfer:HomeBucket} . This policy applies only when the domain of
	// ServerIdis Amazon S3. Amazon EFS does not use session policies. For session
	// policies, Transfer Family stores the policy as a JSON blob, instead of the
	// Amazon Resource Name (ARN) of the policy. You save the policy as a JSON blob and
	// pass it in the Policy  argument. For an example of a session policy, see
