...
type CreateServerInput struct {
	...
	// The certificate must be a
	// valid SSL/TLS X.509 version 3 certificate with FQDN or IP address specified and
	...
	// Elastic File System (Amazon EFS). The default value is S3. After the server is
	// created, the domain cannot be changed.
	Domain types.Domain
	...
	// of active keys that use different algorithms. Use the following command to
	// generate an RSA 2048 bit key with no passphrase: ssh-keygen -t rsa -b 2048 -N
	// "" -m PEM -f my-new-server-key . Use a minimum value of 2048 for the -b
	// option. You can create a stronger key by using 3072 or 4096. Use the following
	// command to generate an ECDSA 256 bit key with no passphrase: ssh-keygen -t
	// ecdsa -b 256 -N "" -m PEM -f my-new-server-key . Valid values for the -b
	// option for ECDSA are 256, 384, and 521. Use the following command to generate an
	// ED25519 key with no passphrase: ssh-keygen -t ed25519 -N "" -f
	// my-new-server-key. For all of these commands, you can replace
	// my-new-server-key with a string of your choice. If you aren't planning to
	...
	// disruptive. For more information, see Manage host keys for your SFTP-enabled
	...
	// API_GATEWAY. Accepts an array containing all of the information required to
	// use a directory in AWS_DIRECTORY_SERVICE or invoke a customer-supplied
	...
	// service. Use AWS_DIRECTORY_SERVICE to provide access to Active Directory
	// groups in Directory Service for Microsoft Active Directory or Microsoft Active
	...
	// the IdentityProviderDetails  parameter. Use the AWS_LAMBDA value to directly
	// use an Lambda function as your identity provider. If you choose this value, you
	...
	// when a file is uploaded completely, WorkflowDetails can also contain a
	// workflow ID (and execution role) for a workflow to execute on partial upload. A
