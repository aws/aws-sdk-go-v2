...
func (v *ConflictException) SerializeMembers(s smithy.ShapeSerializer) {
	if v.ConflictType != "" {
		s.WriteString(schemas.ConflictException_ConflictType, string(v.ConflictType))
	}
	if v.Message != nil {
...
func (v *ConflictException) Deserialize(d smithy.ShapeDeserializer) error {
	return smithy.ReadStruct(d, schemas.ConflictException, func(s *smithy.Schema) error {
		...
		case schemas.ConflictException_ConflictType:
			var ev string
			if err := d.ReadString(schemas.ConflictException_ConflictType, &ev); err != nil {
				return err
			}
			v.ConflictType = ConflictType(ev)
			return nil
