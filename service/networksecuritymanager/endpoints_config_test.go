...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-network-security-manager.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-network-security-manager.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-network-security-manager.dev",
			expectURL:     aws.String("https://env-network-security-manager.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-network-security-manager.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-network-security-manager.dev",
			expectURL:     aws.String("http://config-network-security-manager.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-network-security-manager.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-network-security-manager.dev",
			clientEndpoint: aws.String("https://client-network-security-manager.dev"),
			expectURL:      aws.String("https://client-network-security-manager.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_NETWORK_SECURITY_MANAGER", c.envService)
			}
