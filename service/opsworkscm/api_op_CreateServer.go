...
// ResourceAlreadyExistsExceptionis thrown when a server with the same name
...
// ValidationExceptionis thrown when parameters of the request are not valid. If
...
type CreateServerInput struct {
	...
	// that points the custom domain to the endpoint that is generated when the server
	// is created (the value of the CreateServer Endpoint attribute). You cannot access
	...
	// Optional engine attributes on a specified server. Attributes accepted in a
	// Chef createServer request:
	...
	// time (UTC). The default value is a random, daily start time. Example: 08:00,
	// which represents a daily start time of 08:00 UTC. Example: Mon:08:00, which
	...
	// following format: DDD:HH:MM . MM  must be specified as 00. The specified time
	// is in coordinated universal time (UTC). The default value is a random one-hour
