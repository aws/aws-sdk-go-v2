...
// separately for each share type. This API cannot be used for removing the
// portfolio share. You must use DeletePortfolioShare API for that action. When
// you associate a principal with portfolio, a potential privilege escalation path
// may occur when that portfolio is then shared with other accounts. For a user in
// a recipient account who is not an Service Catalog Admin, but still has the
...
// then Service Catalog recommends using PrincipalType  as IAM. With this
