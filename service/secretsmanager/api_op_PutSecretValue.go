...
// , then Secrets Manager automatically moves the staging label AWSCURRENT to
// this version. If this operation creates the first version for the secret, then
...
type PutSecretValueInput struct {
	...
	// The ARN or name of the secret to add a new version to. For an ARN, we
	...
	// must include SecretBinary  or SecretString, but not both. You can't access
	// this value from the Secrets Manager console.
	SecretBinary []byte

	// The text to encrypt and store in the new version of the secret. You must
	// include SecretBinary  or SecretString, but not both. We recommend you create
	// the secret string as JSON key/value pairs, as shown in the example.
	SecretString *string
	...
	// AWSCURRENT, and it is already attached to another version, then Secrets
	// Manager also moves the staging label AWSPREVIOUS  to the version that
	// AWSCURRENT was removed from. If you don't include VersionStages, then Secrets
