...
// to it. For database credentials you want to rotate, for Secrets Manager to be
// able to rotate the secret, you must make sure the JSON you store in the
// SecretString matches the JSON structure of a database secret (https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_secret_json_structure.html)
...
type CreateSecretInput struct {
	...
	// The name of the new secret. The secret name can contain ASCII letters,
	// numbers, and the following characters: /_+=.@- Do not end your secret name with
	// a hyphen followed by six characters. If you do so, you risk confusion and
	// unexpected results when searching for a secret by partial ARN. Secrets Manager
	...
	// creates it for you automatically the first time it encrypts the secret value. If
	// the secret is in a different Amazon Web Services account from the credentials
