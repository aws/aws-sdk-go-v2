...
// Package proton provides the API client, operations, and parameter types for
// AWS Proton.
//
...
// IdempotentParameterMismatcherror. Client tokens expire eight hours after a
...
//
// Idempotent create APIs Given a
// request action that has succeeded: If you retry the request with an API from
// this group, and the original resource hasn't been modified, the retry succeeds
// without performing any further actions other than returning the original
// resource detail data in the response. If the original resource has been
// modified, the retry throws a ConflictException. If you retry with different
// input parameters, the retry throws a ValidationException  with an
...
//   - CreateService
//
// Idempotent delete APIs Given a request action that has
// succeeded: When you retry the request with an API from this group and the
// resource was deleted, its metadata is returned in the response. If you retry and
// the resource doesn't exist, the response is empty. In both cases, the retry
...
// Given a request action that has succeeded: If you retry the request with an API
// from this group, if the original request delete operation status is
// DELETE_IN_PROGRESS, the retry returns the resource detail data in the response
// without performing any further actions. If the original request delete operation
// is complete, a retry returns an empty response. Asynchronous idempotent delete
// APIs:
//   - DeleteEnvironment
//   - DeleteService
package proton
