...
type awsRestjson1_serializeOpGenerateAuthRequestCryptogram struct {
}

func (*awsRestjson1_serializeOpGenerateAuthRequestCryptogram) ID() string {
	return "OperationSerializer"
}

func (m *awsRestjson1_serializeOpGenerateAuthRequestCryptogram) HandleSerialize(ctx context.Context, in middleware.SerializeInput, next middleware.SerializeHandler) (
	out middleware.SerializeOutput, metadata middleware.Metadata, err error,
) {
	_, span := tracing.StartSpan(ctx, "OperationSerializer")
	endTimer := startMetricTimer(ctx, "client.call.serialization_duration")
	...
	opPath, opQuery := httpbinding.SplitURI("/cryptogram/generate")
	request.URL.Path = smithyhttp.JoinPath(request.URL.Path, opPath)
	request.URL.RawQuery = smithyhttp.JoinRawQuery(request.URL.RawQuery, opQuery)
	request.Method = "POST"
	var restEncoder *httpbinding.Encoder
	...
	jsonEncoder := smithyjson.NewEncoder()
	if err := awsRestjson1_serializeOpDocumentGenerateAuthRequestCryptogramInput(input, jsonEncoder.Value); err != nil {
		return out, metadata, &smithy.SerializationError{Err: err}
	}
...
func awsRestjson1_serializeOpHttpBindingsGenerateAuthRequestCryptogramInput(v *GenerateAuthRequestCryptogramInput, encoder *httpbinding.Encoder) error {
	if v == nil {
		return fmt.Errorf("unsupported serialization of nil %T", v)
	}
...
func awsRestjson1_serializeOpDocumentGenerateAuthRequestCryptogramInput(v *GenerateAuthRequestCryptogramInput, value smithyjson.Value) error {
	object := value.Object()
	defer object.Close()
...
func awsRestjson1_serializeDocumentKekValidationRequest(v *types.KekValidationRequest, value smithyjson.Value) error {
	...
	if len(v.RandomKeyMaxLength) > 0 {
		ok := object.Key("RandomKeyMaxLength")
		ok.String(string(v.RandomKeyMaxLength))
	}
