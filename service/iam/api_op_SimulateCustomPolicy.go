...
// strings. The simulation does not perform the API operations; it only checks the
// authorization to determine if the simulated policies allow or deny the
// operations. You can simulate resources that don't exist in your account. If you
// want to simulate existing policies that are attached to an IAM user, group, or
// role, use SimulatePrincipalPolicy instead. Context keys are variables that are
// maintained by Amazon Web Services and its services and which provide details
// about the context of an API query request. You can use the Condition element
// of an IAM policy to evaluate context keys. To get the list of context keys that
// the policies require for correct simulation, use GetContextKeysForCustomPolicy
// . If the output is long, you can use MaxItems  and Marker parameters to
// paginate the results. The IAM policy simulator evaluates statements in the
...
// simulator, see Testing IAM policies with the IAM policy simulator  (https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_testing-policies.html)
...
type SimulateCustomPolicyInput struct {
	...
	// service identifier, such as iam:CreateUser. This operation does not support
	...
	// policy must be submitted with the ResourcePolicy parameter. The policies
	// cannot be "scope-down" policies, such as you could include in a call to
	...
	// API operations. CallerArn  is required if you include a ResourcePolicy so that
	...
	// a string in the ResourcePolicy  parameter. If you include a ResourcePolicy,
	// then it must be applicable to all of the resources included in the simulation or
	// you receive an invalid input error. For more information about ARNs, see
	...
	// of the identity provided in CallerArn. This parameter is required only if you
	// specify a resource-based policy and account that owns the resource is different
	// from the account that owns the simulated calling user CallerArn. The ARN for
	// an account uses the following syntax: arn:aws:iam::AWS-account-ID:root. For
	// example, to represent the account with the 112233445566 ID, use the following
	...
	// You can include only one resource-based policy in a simulation. The maximum
	// length of the policy document that you can pass in this operation, including
	// whitespace, is listed below. To view the maximum character counts of a managed
	// policy with no whitespaces, see IAM and STS character quotas (https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_iam-quotas.html#reference_iam-quotas-entity-length)
	// . The regex pattern (http://wikipedia.org/wiki/regex) used to validate this
