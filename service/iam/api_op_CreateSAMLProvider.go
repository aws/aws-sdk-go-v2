...
// supports SAML 2.0. The SAML provider resource that you create with this
...
// Amazon Web Services. When you create the SAML provider resource, you upload a
// SAML metadata document that you get from your IdP. That document includes the
// issuer's name, expiration information, and keys that can be used to validate the
// SAML authentication response (assertions) that the IdP sends. You must generate
// the metadata document using the identity management software that is used as
// your organization's IdP. This operation requires Signature Version 4 (https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html)
