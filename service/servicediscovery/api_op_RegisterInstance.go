...
//
// For the current quota on the number of
...
type RegisterInstanceInput struct {
	...
	// creating alias records that route traffic to AWS resources other than Elastic
	...
	// ID for the instance. If the AWS_EC2_INSTANCE_ID attribute is specified, then the
	...
	// will be filled out with the primary private IPv4 address. AWS_INIT_HEALTH_STATUS
	...
	// custom health check, HEALTHY or UNHEALTHY. If you don't specify a value for
	// AWS_INIT_HEALTH_STATUS, the initial status is HEALTHY. AWS_INSTANCE_CNAME If the
	// service configuration includes a CNAME record, the domain name that you want
	// Route 53 to return in response to DNS queries, for example, example.com. This
	// value is required if the service specified by ServiceId includes settings for an
	// CNAME record. AWS_INSTANCE_IPV4 If the service configuration includes an A
	// record, the IPv4 address that you want Route 53 to return in response to DNS
	// queries, for example, 192.0.2.44. This value is required if the service
	// specified by ServiceId includes settings for an A record. If the service
	...
	// service. Custom attributes You can add up to 30 custom attributes. For each
	// key-value pair, the maximum length of the attribute name is 255 characters, and
	// the maximum length of the attribute value is 1,024 characters. The total size of
