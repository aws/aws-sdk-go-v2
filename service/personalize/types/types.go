...
type BatchInferenceJobConfig struct {
	...
	// hyperparameters, including explorationWeight  and explorationItemAgeCutOff,
	// you want to use to configure the amount of item exploration Amazon Personalize
	// uses when recommending items. See User-Personalization (https://docs.aws.amazon.com/personalize/latest/dg/native-recipe-new-item-USER_PERSONALIZATION.html)
	// .
...
type DatasetExportJob struct {
	...
	// The status of the dataset export job. A dataset export job can be in one of
...
type DatasetGroupSummary struct {
	...
	// The status of the dataset group. A dataset group can be in one of the
	// following states:
	//     - CREATE PENDING > CREATE IN_PROGRESS > ACTIVE -or- CREATE FAILED
	//     - DELETE PENDING
	Status *string
...
type DatasetImportJob struct {
	...
	// The status of the dataset import job. A dataset import job can be in one of
	// the following states:
	//     - CREATE PENDING > CREATE IN_PROGRESS > ACTIVE -or- CREATE FAILED
	Status *string
...
type DataSource struct {
	...
	// your dataset is stored. For example: s3://bucket-name/folder-name/
	DataLocation *string
...
type MetricAttribute struct {
	...
	// For SUM() functions, provide the dataset type (either Interactions or Items) and
