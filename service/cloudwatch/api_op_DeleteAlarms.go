...
// operation, but you can't delete two composite alarms with one operation. If you
// specify an incorrect alarm name or make any other error in the operation, no
// alarms are deleted. To confirm that alarms were deleted successfully, you can
// use the DescribeAlarms (https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_DescribeAlarms.html)
// operation after using DeleteAlarms. It is possible to create a loop or cycle
// of composite alarms, where composite alarm A depends on composite alarm B, and
...
// of one of the alarms to false. Additionally, the evaluation of composite
// alarms stops if CloudWatch detects a cycle in the evaluation path.
