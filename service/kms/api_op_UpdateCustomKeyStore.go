...
// UpdateCustomKeyStoreoperation. However, you can use the file to help you
...
// CloudHSM key store must be DISCONNECTED. For an external key store, you can
// use this operation to change the custom key store friendly name (
// NewCustomKeyStoreName), or to tell KMS about a change to the external key
// store proxy authentication credentials ( XksProxyAuthenticationCredential),
// connection method ( XksProxyConnectivity ), external proxy endpoint (
// XksProxyUriEndpoint ) and path ( XksProxyUriPath). For external key stores
// with an XksProxyConnectivity  of VPC_ENDPOINT_SERVICE, you can also update the
// Amazon VPC endpoint service name ( XksProxyVpcEndpointServiceName). To update
// most properties of an external key store, the ConnectionState of the external
// key store must be DISCONNECTED . However, you can update the CustomKeyStoreName
// , XksProxyAuthenticationCredential , and XksProxyUriPath of an external key
// store when it is in the CONNECTED or DISCONNECTED state. If your update requires
// a DISCONNECTED  state, before using UpdateCustomKeyStore , use the
// DisconnectCustomKeyStoreoperation to disconnect the custom key store. After
// the UpdateCustomKeyStore  operation completes, use the ConnectCustomKeyStore
// to reconnect the custom key store. To find the ConnectionState of the custom
// key store, use the DescribeCustomKeyStores operation. Before updating the
...
// XksProxyUriPathvalue, verify that the external key store proxy is reachable at
// the new path. If the operation succeeds, it returns a JSON object with no
// properties. Cross-account use: No. You cannot perform this operation on a custom
// key store in a different Amazon Web Services account. Required permissions:
...
type UpdateCustomKeyStoreInput struct {
	...
	// AWS_CLOUDHSM. Enter the cluster ID of the cluster that you used to create the
	// custom key store or a cluster that shares a backup history and has the same
	// cluster certificate as the original cluster. You cannot use this parameter to
	// associate a custom key store with an unrelated cluster. In addition, the
	// replacement cluster must fulfill the requirements (https://docs.aws.amazon.com/kms/latest/developerguide/create-keystore.html#before-keystore)
	...
	// AccessKeyId and SecretAccessKey value in the authentication credential, even
	// if you are only updating one value. This parameter doesn't establish or change
	// your authentication credentials on the proxy. It just tells KMS the credential
	// that you established with your external key store proxy. For example, if you
	// rotate the credential on your external key store proxy, you can use this
	// parameter to update the credential in KMS. You can change this value when the
	// external key store is connected or disconnected.
	...
	// change the XksProxyUriEndpoint  and add an XksProxyVpcEndpointServiceName
	// value. If you change the XksProxyConnectivity  to PUBLIC_ENDPOINT, you must
	// also change the XksProxyUriEndpoint  and specify a null or empty string for the
	// XksProxyVpcEndpointServiceNamevalue. To change this value, the external key
	...
	// must end with /kms/xks/v1 , where v1 represents the version of the KMS
	// external key store proxy API. You can include an optional prefix between the
	// required elements such as /example/kms/xks/v1 . The combined
	// XksProxyUriEndpoint and XksProxyUriPath values must be unique in the Amazon
	// Web Services account and Region. You can change this value when the external key
