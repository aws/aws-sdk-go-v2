...
// ciphertext that was encrypted under a symmetric encryption KMS key or an
// asymmetric encryption KMS key. When the KMS key is asymmetric, you must specify
// the KMS key and the encryption algorithm that was used to encrypt the
// ciphertext. For information about asymmetric KMS keys, see Asymmetric KMS keys (https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html)
...
// Decryptoperation on a particular KMS key, instead of using &IAM; policies.
...
type DecryptInput struct {
	...
	// specify a different algorithm, the Decrypt operation fails. This parameter is
	// required only when the ciphertext was encrypted under an asymmetric KMS key. The
	// default value, SYMMETRIC_DEFAULT, represents the only supported algorithm that
	// is valid for symmetric encryption KMS keys.
