...
//
// For external key stores: Some external key managers provide a simpler method for
// creating an external key store. For details, see your external key manager
...
// use a proxy configuration with the CreateCustomKeyStore operation. However,
// you can use the values in the file to help you determine the correct values for
// the CreateCustomKeyStore parameters. When the operation completes
...
// to connect a new CloudHSM key store to its CloudHSM cluster, or to connect a new
// external key store to the external key store proxy for your external key
// manager. Even if you are not going to use your custom key store immediately, you
// might want to connect it to verify that all settings are correct and then
// disconnect it until you are ready to use it. For help with failures, see
...
type CreateCustomKeyStoreInput struct {
	...
	// required for custom key stores with a CustomKeyStoreType  of AWS_CLOUDHSM.
	// Enter the content of the trust anchor certificate for the CloudHSM cluster. This
	...
	// a unique identifier for the RawSecretAccessKey. For character requirements,
	// see XksProxyAuthenticationCredentialType. KMS uses this authentication
	// credential to sign requests to the external key store proxy on your behalf. This
	// credential is unrelated to Identity and Access Management (IAM) and Amazon Web
	// Services credentials. This parameter doesn't set or change the authentication
	// credentials on the XKS proxy. It just tells KMS the credential that you
	// established on your external key store proxy. If you rotate your proxy
	// authentication credential, use the UpdateCustomKeyStore operation to provide
	// the new credential to KMS.
	...
	// VPC_ENDPOINT_SERVICE , specify https:// followed by the private DNS name of
	// the VPC endpoint service. For external key stores with PUBLIC_ENDPOINT
	...
	// parameter is required when the value of CustomKeyStoreType  is
	// EXTERNAL_KEY_STORE and the value of XksProxyConnectivity  is
	// VPC_ENDPOINT_SERVICE . The Amazon VPC endpoint service must fulfill all
