...
type CustomKeyStoresListEntry struct {
	...
	// ConnectionStateindicates whether it is connected to the external key store
	...
	// it. If the value is CONNECTED but you are having trouble using the custom key
	...
	// ConnectionErrorCodefield in the response indicates the cause of the failure.
	...
	// key store backed by an external key store proxy and external key manager outside
...
type GrantListEntry struct {
	...
	// The identity that gets the permissions in the grant. The GranteePrincipal
	// field in the ListGrants response usually contains the user or role designated
	// as the grantee principal in the grant. However, when the grantee principal in
	// the grant is an Amazon Web Services service, the GranteePrincipal field
...
// Contains metadata about a KMS key. This data type is used as a response
// element for the CreateKey , DescribeKey , and ReplicateKey  operations.
type KeyMetadata struct {
	...
	// Management Service (KMS) (https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html#arn-syntax-kms)
	...
	// key with other encryption algorithms within KMS. This value is present only when
	// the KeyUsage  of the KMS key is ENCRYPT_DECRYPT .
	...
	// for regional KMS keys. For more information about multi-Region keys, see
	...
	// PendingReplicaDeletion. That indicates that the KMS key is the primary key in
	// a multi-Region key, it is scheduled for deletion, and it still has existing
	// replica keys. When a single-Region KMS key or a multi-Region replica key is
	// scheduled for deletion, its deletion date is displayed in the DeletionDate
	...
	// deleted. This value displays that waiting period. When the last replica key in
	// the multi-Region key is deleted, the KeyState of the scheduled primary key
	// changes from PendingReplicaDeletion  to PendingDeletion and the deletion date
	// appears in the DeletionDate  field.
...
// tag values are both required, but tag values can be empty (null) strings. For
