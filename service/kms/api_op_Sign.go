...
//
// When
// signing a message, be sure to record the KMS key and the signing algorithm. This
// information is required to verify the signature. Best practices recommend that
// you limit the time during which any signature is effective. This deters an
// attack where the actor uses a signed message to establish validity repeatedly or
// long after the message is superseded. Signatures do not include a timestamp, but
// you can include a timestamp in the signed message to help you detect when its
// time to refresh the signature. To verify the signature that this operation
...
// outside of KMS. The KMS key that you use for this operation must be in a
...
type SignInput struct {
	...
	// of the signing algorithm. Use RAW  for unhashed messages; use DIGEST for
	// message digests, which are already hashed. When the value of MessageType  is
	// RAW, KMS uses the standard signing algorithm, which begins with a hash
	// function. When the value is DIGEST, KMS skips the hashing step in the signing
	// algorithm. Use the DIGEST  value only when the value of the Message parameter
	// is a message digest. If you use the DIGEST value with an unhashed message, the
	// security of the signing operation can be compromised. When the value of
	// MessageType is DIGEST , the length of the Message value must match the length
	// of hashed messages for the specified signing algorithm. You can submit a message
	// digest and omit the MessageType  or specify RAW so the digest is hashed again
	// while signing. However, this can cause verification failures when verifying with
	// a system that assumes a single hash. The hashing algorithm in that Sign uses
	// is based on the SigningAlgorithm  value.
