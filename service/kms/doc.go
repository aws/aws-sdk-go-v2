...
// Tools for Amazon Web Services (http://aws.amazon.com/tools/). We recommend
// that you use the Amazon Web Services SDKs to make programmatic API calls to KMS.
...
// Reference. All KMS API calls must be signed and be transmitted using Transport
// Layer Security (TLS). KMS recommends you always use the latest supported TLS
// version. Clients must also support cipher suites with Perfect Forward Secrecy
// (PFS) such as Ephemeral Diffie-Hellman (DHE) or Elliptic Curve Ephemeral
// Diffie-Hellman (ECDHE). Most modern systems such as Java 7 and later support
// these modes. Signing Requests Requests must be signed using an access key ID and
// a secret access key. We strongly recommend that you do not use your Amazon Web
// Services account root access key ID and secret access key for everyday work. You
// can use the access key ID and secret access key for an IAM user or you can use
// the Security Token Service (STS) to generate temporary security credentials and
// use those to sign requests. All KMS requests must be signed with Signature
...
// Additional Resources For more information about credentials and request signing,
// see the following:
