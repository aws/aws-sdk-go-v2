...
// a PrimaryRegion  value of eu-west-2 , the primary key is now the key in
// eu-west-2 , and the key in us-east-1  becomes a replica key. For details, see
...
// the ReplicateKey operation. You can run this operation while using the
// affected multi-Region keys in cryptographic operations. This operation should
// not delay, interrupt, or cause failures in cryptographic operations. Even after
// this operation completes, the process of updating the primary Region might still
// be in progress for a few more seconds. Operations such as DescribeKey might
...
// restored when the update is complete. While the key state is Updating, you can
...
// disabling these keys. For details about the Updating  key state, see Key
