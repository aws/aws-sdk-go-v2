...
// status about a handshake. Handshakes that are ACCEPTED , DECLINED , CANCELED,
// or EXPIRED appear in the results of this API for only 30 days after changing
// to that state. After that, they're deleted and no longer accessible. Always
// check the NextToken  response parameter for a null  value when calling a List*
...
type ListHandshakesForOrganizationInput struct {
	...
	// . Alternatively, for the ENABLE-ALL-FEATURES handshake that generates a
	// separate child handshake for each member account, you can specify the
	// ParentHandshakeIdto see only the handshakes that were generated by that parent
	// request.
