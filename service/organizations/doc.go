...
// Forums Help (http://forums.aws.amazon.com/help.jspa). Endpoint to call When
// using the CLI or the Amazon Web Services SDK For the current release of
// Organizations, specify the us-east-1 region for all Amazon Web Services API
// and CLI calls made from the commercial Amazon Web Services Regions outside of
// China. If calling from one of the Amazon Web Services Regions in China, then
// specify cn-northwest-1. You can do this in the CLI by using these parameters
// and commands:
...
//
// Recording API Requests Organizations supports CloudTrail, a service that records
// Amazon Web Services API calls for your Amazon Web Services account and delivers
// log files to an Amazon S3 bucket. By using information collected by CloudTrail,
// you can determine which requests the Organizations service received, who made
// the request and when, and so on. For more about Organizations and its support
