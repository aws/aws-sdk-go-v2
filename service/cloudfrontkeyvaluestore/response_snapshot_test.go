...
func TestCheckResponseSnapshot_PutKey(t *testing.T) {
	want := &PutKeyOutput{
		ItemCount:        ptr.Int32(1),
		TotalSizeInBytes: ptr.Int64(1),
		ETag:             ptr.String("__ETag__"),
	}
	status, header, body, err := serdeRespReadSnapshot("PutKey.response")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "PutKey.response", err)
	}
}

func TestCheckResponseSnapshot_UpdateKeys(t *testing.T) {
	want := &UpdateKeysOutput{
		ItemCount:        ptr.Int32(1),
		TotalSizeInBytes: ptr.Int64(1),
		ETag:             ptr.String("__ETag__"),
	}
	status, header, body, err := serdeRespReadSnapshot("UpdateKeys.response")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "UpdateKeys.response", err)
	}
}
