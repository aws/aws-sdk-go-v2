...
type awsRestjson1_serializeOpRestartConnector struct {
}

func (*awsRestjson1_serializeOpRestartConnector) ID() string {
	return "OperationSerializer"
}

func (m *awsRestjson1_serializeOpRestartConnector) HandleSerialize(ctx context.Context, in middleware.SerializeInput, next middleware.SerializeHandler) (
	out middleware.SerializeOutput, metadata middleware.Metadata, err error,
) {
	_, span := tracing.StartSpan(ctx, "OperationSerializer")
	endTimer := startMetricTimer(ctx, "client.call.serialization_duration")
	...
	opPath, opQuery := httpbinding.SplitURI("/v1/connectors/{connectorArn}/restart")
	request.URL.Path = smithyhttp.JoinPath(request.URL.Path, opPath)
	request.URL.RawQuery = smithyhttp.JoinRawQuery(request.URL.RawQuery, opQuery)
	request.Method = "POST"
	var restEncoder *httpbinding.Encoder
	...
	if err := awsRestjson1_serializeOpHttpBindingsRestartConnectorInput(input, restEncoder); err != nil {
		return out, metadata, &smithy.SerializationError{Err: err}
	}
...
func awsRestjson1_serializeOpHttpBindingsRestartConnectorInput(v *RestartConnectorInput, encoder *httpbinding.Encoder) error {
	if v == nil {
		return fmt.Errorf("unsupported serialization of nil %T", v)
	}
	...
	if v.OnlyFailedTasks {
		encoder.SetQuery("onlyFailedTasks").Boolean(v.OnlyFailedTasks)
	}
