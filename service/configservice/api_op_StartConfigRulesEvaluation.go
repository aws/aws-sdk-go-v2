...
// StartConfigRulesEvaluationdoes not re-record the latest configuration state
// for your resources. It re-runs an evaluation against the last known state of
// your resources. You can specify up to 25 Config rules per request. An existing
// StartConfigRulesEvaluationcall for the specified rules must complete before
// you can call the API again. If you chose to have Config stream to an Amazon SNS
// topic, you will receive a ConfigRuleEvaluationStarted notification when the
// evaluation starts. You don't need to call the StartConfigRulesEvaluation API
// to run an evaluation for a new rule. When you create a rule, Config evaluates
