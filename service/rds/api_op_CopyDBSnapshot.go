...
// availablestate. You can copy a snapshot from one Amazon Web Services Region to
// another. In that case, the Amazon Web Services Region where you call the
// CopyDBSnapshotoperation is the destination Amazon Web Services Region for the
...
type CopyDBSnapshotInput struct {
	...
	// rds:mysql-instance1-snapshot-20130805. If the source snapshot is in a
	...
	// Don't specify PreSignedUrl when you are copying an encrypted DB snapshot in
	// the same Amazon Web Services Region. The presigned URL must be a valid request
