...
// groups that the user will join at log on. By default, the temporary credentials
...
// to the listed dbgroups . In addition, if the AutoCreate  parameter is set to
// True , then the policy must include the redshift:CreateClusterUser permission.
...
type GetClusterCredentialsInput struct {
	...
	// , a new user is created using the value for DbUser with PUBLIC permissions. If
	// a database user matching the value for DbUser  doesn't exist and Autocreate  is
	...
	// The name of a database that DbUser  is authorized to log on to. If DbName is
	// not specified, DbUser  can log on to any existing database. Constraints:
	//     - Must be 1 to 64 alphanumeric characters or hyphens
	//     - Must contain uppercase or lowercase letters, numbers, underscore, plus sign, period (dot), at symbol (@), or hyphen.
	//     - First character must be a letter.
	//     - Must not contain a colon ( : ) or slash ( / ).
...
type GetClusterCredentialsOutput struct {
	...
	// By default, the user is added to PUBLIC. If the DbGroups parameter is
	// specifed, DbUser is added to the listed groups for any sessions created using
	// these credentials.
	DbUser *string
