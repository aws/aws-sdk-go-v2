...
// for custom usage dimensions.
//
// For information about the permissions that you need to use this API, see [Amazon Web Services Marketplace metering and entitlement API permissions] in
...
//     Marketplace Metering Service and call the RegisterUsage operation. Free and
//     Bring Your Own License model (BYOL) products for Amazon ECS or Amazon EKS aren't
//     required to call RegisterUsage . However, you can do so if you want to receive
...
//
//   - MeterUsage API is available in GovCloud Regions but only supports AMI FCP
//     products in GovCloud Regions. Flexible Consumption Pricing (FCP) Container
...
//
//   - Custom metering for container products are called using the MeterUsage API.
//     The API is used for FCP AMI and FCP Container product metering.
//
...
//     Pricing (FCP) products using MeterUsage API. Amazon ECS is supported in all
...
// [CloudTrail User Guide]: https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-concepts.html
// [Amazon Web Services Marketplace metering and entitlement API permissions]: https://docs.aws.amazon.com/marketplace/latest/userguide/iam-user-policy-for-aws-marketplace-actions.html
