...
// bus in the US East (N. Virginia) us-east-1, US West (Oregon) us-west-2, or
// Europe (Ireland) eu-west-1 Regions as a target for a rule.
//
// * Firehose delivery
...
//
// * Amazon
// SQS queues (includes FIFO queues
//
// * SSM Automation
//
// * SSM OpsItem
...
//
// Creating rules with built-in targets
// is supported only in the AWS Management Console. The built-in targets are EC2
...
// to that account. Set that account's event bus as a target of the rules in your
// account. To send the matched events to the other account, specify that account's
// event bus as the Arn value when you run PutTargets. If your account sends events
// to another account, your account is charged for each sent event. Each event sent
// to another account is charged as a custom event. The account receiving the event
// is not charged. For more information, see Amazon EventBridge (CloudWatch Events)
// Pricing (https://aws.amazon.com/eventbridge/pricing/). Input, InputPath, and
// InputTransformer are not available with PutTarget if the target is an event bus
// of a different AWS account. If you are setting the event bus of another account
// as the target, and that account granted permission to your account through an
// organization instead of directly by the account ID, then you must specify a
// RoleArn with proper permissions in the Target structure. For more information,
...
// cross-account events, see PutPermission. Input, InputPath, and InputTransformer
