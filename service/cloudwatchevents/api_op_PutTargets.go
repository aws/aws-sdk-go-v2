...
// supported only in the Management Console. The built-in targets are EC2
...
// KinesisParametersargument. To invoke a command on multiple EC2 instances with
...
// InputPath , and InputTransformer  are not available with PutTarget if the
// target is an event bus of a different Amazon Web Services account. If you are
...
// notation, not bracket notation. When you add targets to a rule and the
