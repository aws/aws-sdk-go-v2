...
// taken in response to the event. The DescribeVolumeStatus operation provides
// the following information about the specified volumes: Status: Reflects the
// current status of the volume. The possible values are ok , impaired  , warning
// , or insufficient-data. If all checks pass, the overall status of the volume
// is ok . If the check fails, the overall status is impaired . If the status is
...
// for the volume by calling the EnableVolumeIO action and then check the volume
// for data consistency. Volume status is based on the volume status checks, and
