...
// bot, for example, you would create an OrderPizza intent. To create an intent
// or replace an existing intent, you must provide the following:
...
// uses the $LATEST  version of the intent is set to NOT_BUILT. For more
// information, see how-it-works . This operation requires permissions for the
...
type PutIntentInput struct {
	...
	// example, because there is a built-in intent called AMAZON.HelpIntent, you
	// can't create a custom intent called HelpIntent. For a list of built-in
	// intents, see Standard Built-in Intents (https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/built-in-intent-ref/standard-intents)
	...
	// must provide both the rejectionStatement  and the confirmationPrompt, or
	// neither.
	...
	// intent. For example, after the OrderPizza intent is fulfilled, you might
	// prompt the user to order a drink. The action that Amazon Lex takes depends on
	// the user's response, as follows:
...
type PutIntentOutput struct {
	...
	// After the Lambda function specified in the fulfillmentActivityintent fulfills
