...
// an Amazon GameLift fleet. Register computes with an Amazon GameLift Anywhere
// fleet or a container fleet. For an Anywhere fleet or a container fleet that's
// running the Amazon GameLift Agent, the Agent handles all compute registry tasks
// for you. For an Anywhere fleet that doesn't use the Agent, call this operation
// to register fleet computes. To register a compute, give the compute a name (must
// be unique within the fleet) and specify the compute resource's DNS name or IP
// address. Provide a fleet ID and a fleet location to associate with the compute
// being registered. You can optionally include the path to a TLS certificate on
// the compute resource. If successful, this operation returns compute details,
// including an Amazon GameLift SDK endpoint or Agent endpoint. Game server
// processes running on the compute can use this endpoint to communicate with the
// Amazon GameLift service. Each server process includes the SDK endpoint in its
// call to the Amazon GameLift server SDK action InitSDK() . To view compute
// details, call DescribeCompute (https://docs.aws.amazon.com/gamelift/latest/apireference/API_DescribeCompute.html)
// with the compute name. Learn more
