...
//
// To delete a
// game server group, identify the game server group to delete and specify the type
// of delete operation to initiate. Game server groups can only be deleted if they
// are in ACTIVE  or ERROR status. If the delete request is successful, a series
...
// stops automatic scaling activity. Once all game servers in the game server group
// are deregistered, GameLift FleetIQ can begin deleting resources. If any of the
// delete operations fail, the game server group is placed in ERROR status.
// GameLift FleetIQ emits delete events to Amazon CloudWatch. Learn more GameLift
