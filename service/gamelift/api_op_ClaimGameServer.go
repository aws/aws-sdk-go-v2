...
// session, such as a game map or player information. Filter options may be
// included to further restrict how a game server is chosen, such as only allowing
// game servers on ACTIVE instances to be claimed. When a game server is
...
// CLAIMED for up to 60 seconds. This time period gives the game server time to
// update its status to UTILIZED after players join. If the game server's status
...
// fixed value and is not configurable. If you try to claim a specific game server,
// this request will fail in the following cases:
//   - If the game server utilization status is UTILIZED .
//   - If the game server claim status is CLAIMED .
//   - If the game server is running on an instance in DRAINING status and provided
//     filter option does not allow placing on DRAINING instances.
//
