...
func (v *ListPlansInRegionOutput) Serialize(s smithy.ShapeSerializer) {
	s.WriteStruct(schemas.ListPlansInRegionResponse)
	v.SerializeMembers(s)
	s.CloseStruct()
}

func (v *ListPlansInRegionOutput) SerializeMembers(s smithy.ShapeSerializer) {
	if v.NextToken != nil {
		s.WriteString(schemas.ListPlansInRegionResponse_nextToken, *v.NextToken)
	}
	serializePlanList(s, schemas.ListPlansInRegionResponse_plans, v.Plans)
}
...
func (c *Client) addOperationListPlansInRegionMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = stack.Initialize.Add(newServiceMetadataMiddleware(options.Region, "ListPlansInRegion"), middleware.Before); err != nil {
		return err
	}
