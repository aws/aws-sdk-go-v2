...
// Retrieves a paginated list of Greengrass core devices. IoT Greengrass relies
// on individual devices to send status updates to the Amazon Web Services Cloud.
// If the IoT Greengrass Core software isn't running on the device, or if device
// isn't connected to the Amazon Web Services Cloud, then the reported status of
// that device might not reflect its current status. The status timestamp indicates
// when the device status was last updated. Core devices send status updates at the
// following times:
