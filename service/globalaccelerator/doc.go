...
//
// Global Accelerator is a
// global service that supports endpoints in multiple AWS Regions but you must
// specify the US West (Oregon) Region to create or update accelerators. By
...
// virtual private cloud (VPC) subnets. The static IP addresses remain assigned to
// your accelerator for as long as it exists, even if you disable the accelerator
// and it no longer accepts or routes traffic. However, when you delete an
...
// directed to healthy endpoints. For a list of the AWS Regions where Global
// Accelerator and other services are currently supported, see the AWS Region Table
// (https://docs.aws.amazon.com/about-aws/global-infrastructure/regional-product-services/).
// AWS Global Accelerator includes the following components: Static IP addresses
...
// (https://docs.aws.amazon.com/global-accelerator/latest/dg/using-byoip.html). The
// IP addresses serve as single fixed entry points for your clients. If you already
...
// use static IP addresses to access the resources. The static IP addresses remain
...
// can no longer route traffic by using them. You can use IAM policies like
// tag-based permissions with Global Accelerator to delete an accelerator. For more
// information, see Tag-based policies
// (https://docs.aws.amazon.com/global-accelerator/latest/dg/access-control-manage-access-tag-policies.html).
// Accelerator An accelerator directs traffic to endpoints over the AWS global
// network to improve the performance of your internet applications. Each
// accelerator includes one or more listeners. There are two types of
// accelerators:
//
...
//
// For more
// information, see Types of accelerators
// (https://docs.aws.amazon.com/global-accelerator/latest/dg/introduction-accelerator-types.html).
// DNS name Global Accelerator assigns each accelerator a default Domain Name
// System (DNS) name, similar to a1234567890abcdef.awsglobalaccelerator.com, that
// points to the static IP addresses that Global Accelerator assigns to you or that
// you choose from your own IP address range. Depending on the use case, you can
...
// from clients to Global Accelerator, based on the port (or port range) and
// protocol (or protocols) that you configure. A listener can be configured for
// TCP, UDP, or both TCP and UDP protocols. Each listener has one or more endpoint
// groups associated with it, and traffic is forwarded to endpoints in one of the
// groups. You associate endpoint groups with listeners by specifying the Regions
// that you want to distribute traffic to. With a standard accelerator, traffic is
// distributed to optimal endpoints within the endpoint groups associated with a
// listener. Endpoint group Each endpoint group is associated with a specific AWS
// Region. Endpoint groups include one or more endpoints in the Region. With a
// standard accelerator, you can increase or reduce the percentage of traffic that
// would be otherwise directed to an endpoint group by adjusting a setting called a
// traffic dial. The traffic dial lets you easily do performance testing or
// blue/green deployment testing, for example, for new releases across different
// AWS Regions. Endpoint An endpoint is a resource that Global Accelerator directs
// traffic to. Endpoints for standard accelerators can be Network Load Balancers,
...
// be useful, for example, to do performance testing within a Region. Endpoints for
// custom routing accelerators are virtual private cloud (VPC) subnets with one or
// many EC2 instances.
