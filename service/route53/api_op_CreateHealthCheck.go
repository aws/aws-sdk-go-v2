...
// following:
//
//     * Route 53 health checkers are outside the VPC. To check the
// health of an endpoint within a VPC by IP address, you must assign a public IP
// address to the instance in the VPC.
//
//     * You can configure a health checker to
// check the health of an external resource that the instance relies on, such as a
// database server.
//
//     * You can create a CloudWatch metric, associate an alarm
// with the metric, and then create a health check that is based on the state of
// the alarm. For example, you might create a CloudWatch metric that checks the
// status of the Amazon EC2 StatusCheckFailed metric, add an alarm to the metric,
...
func addOperationCreateHealthCheckStackSlots(stack *middleware.Stack) error {
...
func addOperationCreateHealthCheckMiddlewares(stack *middleware.Stack, options Options) (err error) {
	if err := addOperationCreateHealthCheckStackSlots(stack); err != nil {
		return err
	}
