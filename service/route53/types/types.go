...
type AliasTarget struct {
	...
	// CloudFront distribution. You can get the value of DnsName using the CLI
	...
	// one Amazon EC2 instance.) If you set EvaluateTargetHealth  to true and either
	...
	// requirements for setting EvaluateTargetHealth  to true when the alias target
	// is an S3 bucket. Other records in the same hosted zone If the Amazon Web
	// Services resource that you specify in DNSName is a record or a group of
...
// linking a resource record set to a CIDR location. A LocationName with an
// asterisk “*” can be used to create a default CIDR record. CollectionId is
// still required for default record.
...
type CloudWatchAlarmConfiguration struct {
	...
	// Reference (https://docs.aws.amazon.com/AmazonCloudWatch/latest/DeveloperGuide/CW_Support_For_AWS.html)
...
type HealthCheckConfig struct {
	...
	// ChildHealthCheckelement for each health check that you want to associate with
	...
	// FullyQualifiedDomainName to the endpoint in the client_hello message during
	// TLS negotiation. This allows the endpoint to respond to HTTPS health check
	// requests with the applicable SSL/TLS certificate. Some endpoints require that
	// HTTPS requests include the host name in the client_hello message. If you don't
	// enable SNI, the status of the health check will be SSL alert handshake_failure
	// . A health check can also have that status for other reasons. If SNI is enabled
	// and you're still getting the error, check the SSL/TLS configuration on your
	// endpoint and confirm that your certificate is valid. The SSL/TLS certificate on
	// your endpoint includes a domain name in the Common Name field and possibly
	// several more in the Subject Alternative Names field. One of the domain names
	// in the certificate should match the value that you specify for
	// FullyQualifiedDomainName . If the endpoint responds to the client_hello
	...
	// healthy or vice versa. For more information, see How Amazon Route 53
	...
	// 53 passes the value of FullyQualifiedDomainName  in the Host header, as it
	// does when you specify a value for IPAddress . If the value of Type  is TCP,
	...
	// console. You can't change the value of MeasureLatency after you create a
	// health check.
	MeasureLatency *bool

	// The port on the endpoint that you want Amazon Route 53 to perform health
	// checks on. Don't specify a value for Port  when you specify a value for Type
	...
	// Controller routing control. For more information about Route 53 Application
	// Recovery Controller, see Route 53 Application Recovery Controller Developer
...
type HostedZone struct {
	...
	// have registered with your DNS registrar. For information about how to specify
...
type KeySigningKey struct {
	...
	// Key Management Service (KMS). The KmsArn must be unique for each key-signing
	// key (KSK) in a single hosted zone. You must configure the customer managed key
	...
	// working with the customer managed key in KMS, see Key Management Service
...
type ResourceRecordSet struct {
	...
	// qualified domain name, for example, www.example.com. You can optionally
	// include a trailing dot. If you omit the trailing dot, Amazon Route 53 assumes
	// that the domain name that you specify is fully qualified. This means that Route
	// 53 treats www.example.com  (without a trailing dot) and www.example.com. (with
	// a trailing dot) as identical. For information about how to specify characters
	// other than a-z , 0-9 , and - (hyphen) and how to specify internationalized
	...
	// sets: A  | AAAA  | CAA  | CNAME  | DS  | MX  | NAPTR  | NS  | PTR  | SOA  | SPF
	// | SRV  | TXT Values for weighted, latency, geolocation, and failover resource
	// record sets: A  | AAAA  | CAA  | CNAME  | MX  | NAPTR  | PTR  | SPF  | SRV  |
	// TXT. When creating a group of weighted, latency, geolocation, or failover
	// resource record sets, specify the same value for all of the resource record sets
	// in the group. Valid values for multivalue answer resource record sets: A  |
	// AAAA | MX  | NAPTR  | PTR  | SPF  | SRV  | TXT SPF records were formerly used
	// to verify the identity of the sender of email messages. However, we no longer
	// recommend that you create resource record sets for which the value of Type  is
	// SPF. RFC 7208, Sender Policy Framework (SPF) for Authorizing Use of Domains in
	// Email, Version 1, has been updated to say, "...[I]ts existence and mechanism
	...
	// it." In RFC 7208, see section 14.1, The SPF DNS Record Type (http://tools.ietf.org/html/rfc7208#section-14.1)
	...
	// the Name  and Type elements as failover resource record sets. For failover
	...
	// with a Type  of A  and a ContinentCode  of AF. Although creating geolocation
	// and geolocation alias resource record sets in a private hosted zone is allowed,
	// it's not supported. If you create separate resource record sets for overlapping
	...
	// HealthCheckIdelement and specify the ID of the applicable health check. Route
	...
	// Specifying the Health Check Endpoint by
	// Domain Name If your health checks specify the endpoint only by domain name, we
	...
	// domain name of the server (such as us-east-2-www.example.com), not the name of
	// the resource record sets ( www.example.com). Health check results will be
	// unpredictable if you do the following:
	...
	// traffic policy instance that Route 53 created this resource record set for. To
