...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/appconfig/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_APPCONFIG")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "AppConfig", cfg.ConfigSources)
	if found && err == nil {
...
var bddNodes = [45]int32{
	-1, 1, -1, 0, 14, 3, 1, 4, 100000013, 2, 5, 100000013, 3, 8, 6, 4, 7, 100000012, 5, 100000010, 100000011, 4, 12, 9, 6, 10, 100000009, 7, 100000006, 11, 8, 100000007, 100000008, 5, 13, 100000005, 6, 100000004, 100000005, 3, 100000001, 15, 4, 100000002, 100000003}
...
func evalCondition(idx int, params *EndpointParameters, c *conditionContext) bool {
	...
	case 7:
		return *params.Region == "us-gov-east-1"
	case 8:
...
func resolveResult(idx int32, params *EndpointParameters, c *conditionContext) (smithyendpoints.Endpoint, error) {
	...
	case 4:
		uriString := func() string {
			...
			out.WriteString("https://appconfig-fips.")
	...
	case 6:
		uriString := "https://appconfig.us-gov-east-1.amazonaws.com"
		uri, err := url.Parse(uriString)
		if err != nil {
			return smithyendpoints.Endpoint{}, fmt.Errorf("Failed to parse uri: %s", uriString)
		}
	...
	case 7:
		uriString := "https://appconfig.us-gov-west-1.amazonaws.com"
		uri, err := url.Parse(uriString)
		if err != nil {
			return smithyendpoints.Endpoint{}, fmt.Errorf("Failed to parse uri: %s", uriString)
		}
	...
	case 10:
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://appconfig.")
			out.WriteString(*params.Region)
			out.WriteString(".")
			out.WriteString(c.PartitionResult.DualStackDnsSuffix)
			return out.String()
