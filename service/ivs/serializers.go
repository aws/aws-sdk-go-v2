...
type awsRestjson1_serializeOpUpdateAdConfiguration struct {
}

func (*awsRestjson1_serializeOpUpdateAdConfiguration) ID() string {
	return "OperationSerializer"
}

func (m *awsRestjson1_serializeOpUpdateAdConfiguration) HandleSerialize(ctx context.Context, in middleware.SerializeInput, next middleware.SerializeHandler) (
	out middleware.SerializeOutput, metadata middleware.Metadata, err error,
) {
	_, span := tracing.StartSpan(ctx, "OperationSerializer")
	endTimer := startMetricTimer(ctx, "client.call.serialization_duration")
	...
	opPath, opQuery := httpbinding.SplitURI("/UpdateAdConfiguration")
	request.URL.Path = smithyhttp.JoinPath(request.URL.Path, opPath)
	request.URL.RawQuery = smithyhttp.JoinRawQuery(request.URL.RawQuery, opQuery)
	request.Method = "POST"
	var restEncoder *httpbinding.Encoder
	...
	jsonEncoder := smithyjson.NewEncoder()
	if err := awsRestjson1_serializeOpDocumentUpdateAdConfigurationInput(input, jsonEncoder.Value); err != nil {
		return out, metadata, &smithy.SerializationError{Err: err}
	}
...
func awsRestjson1_serializeOpHttpBindingsUpdateAdConfigurationInput(v *UpdateAdConfigurationInput, encoder *httpbinding.Encoder) error {
	if v == nil {
		return fmt.Errorf("unsupported serialization of nil %T", v)
	}
...
func awsRestjson1_serializeOpDocumentUpdateAdConfigurationInput(v *UpdateAdConfigurationInput, value smithyjson.Value) error {
	object := value.Object()
	defer object.Close()
