...
type awsRestjson1_deserializeOpUpdateAdConfiguration struct {
}

func (*awsRestjson1_deserializeOpUpdateAdConfiguration) ID() string {
	return "OperationDeserializer"
}

func (m *awsRestjson1_deserializeOpUpdateAdConfiguration) HandleDeserialize(ctx context.Context, in middleware.DeserializeInput, next middleware.DeserializeHandler) (
	out middleware.DeserializeOutput, metadata middleware.Metadata, err error,
) {
	out, metadata, err = next.HandleDeserialize(ctx, in)
	if err != nil {
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return out, metadata, awsRestjson1_deserializeOpErrorUpdateAdConfiguration(response, &metadata)
	}
	output := &UpdateAdConfigurationOutput{}
	out.Result = output
	...
	err = awsRestjson1_deserializeOpDocumentUpdateAdConfigurationOutput(&output, shape)
	if err != nil {
		var snapshot bytes.Buffer
		io.Copy(&snapshot, ringBuffer)
		return out, metadata, &smithy.DeserializationError{
...
func awsRestjson1_deserializeOpErrorUpdateAdConfiguration(response *smithyhttp.Response, metadata *middleware.Metadata) error {
	var errorBuffer bytes.Buffer
	if _, err := io.Copy(&errorBuffer, response.Body); err != nil {
		return &smithy.DeserializationError{Err: fmt.Errorf("failed to copy error response body, %w", err)}
	}
...
func awsRestjson1_deserializeOpDocumentUpdateAdConfigurationOutput(v **UpdateAdConfigurationOutput, value interface{}) error {
	if v == nil {
		return fmt.Errorf("unexpected nil of type %T", v)
	}
	if value == nil {
	...
	var sv *UpdateAdConfigurationOutput
	if *v == nil {
		sv = &UpdateAdConfigurationOutput{}
	} else {
		sv = *v
	}
