...
func TestSerdeCheckSnapshot_PutAuditEvents(t *testing.T) {
	...
	header := map[string][]string{}
	svc := serdeNewClient()
	_, err := svc.PutAuditEvents(context.Background(), input, func(o *Options) {
	...
	if err := serdeTestSnapshot(method, rawPath, rawQuery, header, body.Bytes(), "PutAuditEvents"); err != nil {
		if err != nil && !strings.Contains(err.Error(), "error: success") {
			t.Fatal(err)
		}
	}
}
func TestSerdeUpdateSnapshot_PutAuditEvents(t *testing.T) {
	input := &PutAuditEventsInput{
		AuditEvents: []types.AuditEvent{
			{
				Id:                ptr.String("__Id__"),
	...
	if err := serdeUpdateSnapshot(method, rawPath, rawQuery, header, body.Bytes(), "PutAuditEvents"); err != nil {
		if err != nil && !strings.Contains(err.Error(), "error: success") {
			t.Fatal(err)
		}
	}
