...
func TestUpdateResponseSnapshot_GetStreamingResource(t *testing.T) {
	want := &GetStreamingResourceOutput{}
	proto := restjson1.New(schemas.RestJson1KitchenSink)
	opSchema := smithy.NewOperationSchema(schemas.GetStreamingResource, schemas.GetStreamingResourceOutput, schemas.GetStreamingResourceOutput)
	req := smithyhttp.NewStackRequest().(*smithyhttp.Request)
	if err := proto.SerializeRequest(context.Background(), opSchema, want, req); err != nil {
		t.Fatal(err)
	}
	...
	if err := serdeRespWriteSnapshot("GetStreamingResource.response", 200, built.Header, body); err != nil {
		t.Fatal(err)
	}
}
