...
func BenchmarkInMemory(b *testing.B) {
	memBreader := bytes.NewReader(make([]byte, 1*sdkio.GibiByte))

	baseSdkConfig := SDKConfig{WithUnsignedPayload: true, ExpectContinue: true}
	...
	for _, concurrency := range []int{s3manager.DefaultUploadConcurrency, 2 * s3manager.DefaultUploadConcurrency, 100} {
		b.Run(fmt.Sprintf("%d_Concurrency", concurrency), func(b *testing.B) {
			...
			for _, partSize := range []int64{s3manager.DefaultUploadPartSize, 25 * sdkio.MebiByte, 100 * sdkio.MebiByte} {
				b.Run(fmt.Sprintf("%s_PartSize", integration.SizeToName(int(partSize))), func(b *testing.B) {
					...
					for i := 0; i < b.N; i++ {
						...
						_, err := memBreader.Seek(0, io.SeekStart)
						if err != nil {
...
func BenchmarkUpload(b *testing.B) {
	...
	for _, fileSize := range []int64{5 * sdkio.MebiByte, sdkio.GibiByte, 10 * sdkio.GibiByte} {
