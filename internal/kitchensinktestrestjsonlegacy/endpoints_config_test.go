...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-restjson1-kitchen-sink.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-restjson1-kitchen-sink.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-restjson1-kitchen-sink.dev",
			expectURL:     aws.String("https://env-restjson1-kitchen-sink.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-restjson1-kitchen-sink.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-restjson1-kitchen-sink.dev",
			expectURL:     aws.String("http://config-restjson1-kitchen-sink.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-restjson1-kitchen-sink.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-restjson1-kitchen-sink.dev",
			clientEndpoint: aws.String("https://client-restjson1-kitchen-sink.dev"),
			expectURL:      aws.String("https://client-restjson1-kitchen-sink.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_RESTJSON1_KITCHEN_SINK", c.envService)
			}
