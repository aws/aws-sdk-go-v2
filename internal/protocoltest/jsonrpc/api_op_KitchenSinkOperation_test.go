...
func TestClient_KitchenSinkOperation_awsAwsjson11Serialize(t *testing.T) {
	...
	BodyAssert func(io.Reader) error
		...
		"serializes_string_shapes_with_jsonvalue_trait": {
			Params: &KitchenSinkOperationInput{
			JsonValue:
			ptr.String("{\"string\":\"value\",\"number\":1234.5,\"boolTrue\":true,\"boolFalse\":false,\"array\":[1,2,3,4],\"object\":{\"key\":\"value\"},\"null\":null}"),
			},
			ExpectMethod: "POST",
			ExpectURIPath: "/",
			ExpectQuery: []smithytesting.QueryItem {
		...
		"serializes_timestamp_shapes": {
			Params: &KitchenSinkOperationInput{
			Timestamp:
			ptr.Time(smithytime.ParseEpochSeconds(946845296)),
			},
			ExpectMethod: "POST",
			ExpectURIPath: "/",
			ExpectQuery: []smithytesting.QueryItem {
		...
		"serializes_structure_members_with_locationname_traits": {
			Params: &KitchenSinkOperationInput{
			StructWithJsonName:
			&types.StructWithJsonName{
			Value:
			ptr.String("some-value"),
			},
			},
			ExpectMethod: "POST",
			ExpectURIPath: "/",
		...
		"serializes_recursive_structure_shapes": {
			Params: &KitchenSinkOperationInput{
			String_:
			ptr.String("top-value"),
			Boolean:
			ptr.Bool(false),
			RecursiveStruct:
			&types.KitchenSink{
			String_:
			ptr.String("nested-value"),
			Boolean:
			ptr.Bool(true),
			RecursiveList:
			[]types.KitchenSink{
...
func TestClient_KitchenSinkOperation_awsAwsjson11Deserialize(t *testing.T) {
	...
	ExpectResult *KitchenSinkOperationOutput
		...
		"parses_double_shapes": {
			...
			ExpectResult: &KitchenSinkOperationOutput{
			Double:
			ptr.Float64(1.2345678912345679E8),
			},
		},
