...
func TestClient_NullOperation_Serialize(t *testing.T) {
...
func BenchmarkClient_NullOperation_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *NullOperationInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.NullOperation(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_NullOperation_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_NullOperation_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.NullOperation(context.Background(), &params)
			}
		})
	}
}
