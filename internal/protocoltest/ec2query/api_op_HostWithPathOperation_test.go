...
func TestClient_HostWithPathOperation_Serialize(t *testing.T) {
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			if name == "Ec2QueryHostWithPath" {
				t.Skip("disabled test aws.protocoltests.ec2#AwsEc2 aws.protocoltests.ec2#HostWithPathOperation")
			}
...
func BenchmarkClient_HostWithPathOperation_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *HostWithPathOperationInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.HostWithPathOperation(context.Background(), c.Params)
			}
		})
	}
}
