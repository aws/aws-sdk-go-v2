...
func TestClient_TimestampFormatHeaders_Serialize(t *testing.T) {
...
func BenchmarkClient_TimestampFormatHeaders_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *TimestampFormatHeadersInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.TimestampFormatHeaders(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_TimestampFormatHeaders_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_TimestampFormatHeaders_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.TimestampFormatHeaders(context.Background(), &params)
			}
		})
	}
}
