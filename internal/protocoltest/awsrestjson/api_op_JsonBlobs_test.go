...
func TestClient_JsonBlobs_Serialize(t *testing.T) {
...
func BenchmarkClient_JsonBlobs_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *JsonBlobsInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.JsonBlobs(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_JsonBlobs_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_JsonBlobs_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.JsonBlobs(context.Background(), &params)
			}
		})
	}
}
