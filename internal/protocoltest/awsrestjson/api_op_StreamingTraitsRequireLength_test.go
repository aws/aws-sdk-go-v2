...
func TestClient_StreamingTraitsRequireLength_awsRestjson1Deserialize(t *testing.T) {
	cases := map[string]struct {
		StatusCode    int
		Header        http.Header
		BodyMediaType string
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			var params StreamingTraitsRequireLengthInput
			result, err := client.StreamingTraitsRequireLength(context.Background(), &params)
			if err != nil {
				t.Fatalf("expect nil err, got %v", err)
			}
			if result == nil {
