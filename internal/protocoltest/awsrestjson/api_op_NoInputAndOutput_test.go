...
func TestClient_NoInputAndOutput_Serialize(t *testing.T) {
...
func BenchmarkClient_NoInputAndOutput_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *NoInputAndOutputInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.NoInputAndOutput(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_NoInputAndOutput_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_NoInputAndOutput_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.NoInputAndOutput(context.Background(), &params)
			}
		})
	}
}
