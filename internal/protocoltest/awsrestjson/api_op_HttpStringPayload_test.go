...
func TestClient_HttpStringPayload_Serialize(t *testing.T) {
...
func BenchmarkClient_HttpStringPayload_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *HttpStringPayloadInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.HttpStringPayload(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_HttpStringPayload_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_HttpStringPayload_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.HttpStringPayload(context.Background(), &params)
			}
		})
	}
}
