...
func TestClient_OptionalInputOutput_Serialize(t *testing.T) {
...
func BenchmarkClient_OptionalInputOutput_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *OptionalInputOutputInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.OptionalInputOutput(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_OptionalInputOutput_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_OptionalInputOutput_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.OptionalInputOutput(context.Background(), &params)
			}
		})
	}
}
