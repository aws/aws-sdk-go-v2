...
func TestClient_NoInputOutput_Serialize(t *testing.T) {
...
func BenchmarkClient_NoInputOutput_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *NoInputOutputInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.NoInputOutput(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_NoInputOutput_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_NoInputOutput_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.NoInputOutput(context.Background(), &params)
			}
		})
	}
}
