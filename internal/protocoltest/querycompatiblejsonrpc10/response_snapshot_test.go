...
func TestCheckResponseSnapshot_QueryCompatibleOperation(t *testing.T) {
	want := &QueryCompatibleOperationOutput{}
	status, header, body, err := serdeRespReadSnapshot("QueryCompatibleOperation.response")
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "QueryCompatibleOperation.response", err)
	}
}

func TestCheckResponseSnapshot_Error_CustomCodeError(t *testing.T) {
	want := &types.CustomCodeError{
		Message: ptr.String("__Message__"),
	}
	status, header, body, err := serdeRespReadSnapshot("CustomCodeError.error")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	svc := serdeRespClient(status, header, body)
	_, opErr := svc.QueryCompatibleOperation(context.Background(), &QueryCompatibleOperationInput{})
	if opErr == nil {
		t.Fatal("expected error, got nil")
	}
	var got *types.CustomCodeError
	if !errors.As(opErr, &got) {
		t.Fatalf("expected types.CustomCodeError, got %v", opErr)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("error response snapshot mismatch for %s: %v", "CustomCodeError.error", err)
	}
}

func TestCheckResponseSnapshot_Error_NoCustomCodeError(t *testing.T) {
	want := &types.NoCustomCodeError{
		Message: ptr.String("__Message__"),
	}
	status, header, body, err := serdeRespReadSnapshot("NoCustomCodeError.error")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	var got *types.NoCustomCodeError
	if !errors.As(opErr, &got) {
		t.Fatalf("expected types.NoCustomCodeError, got %v", opErr)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("error response snapshot mismatch for %s: %v", "NoCustomCodeError.error", err)
	}
}
