...
func TestClient_XmlMapWithXmlNamespace_Serialize(t *testing.T) {
...
func BenchmarkClient_XmlMapWithXmlNamespace_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *XmlMapWithXmlNamespaceInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.XmlMapWithXmlNamespace(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_XmlMapWithXmlNamespace_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_XmlMapWithXmlNamespace_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.XmlMapWithXmlNamespace(context.Background(), &params)
			}
		})
	}
}
