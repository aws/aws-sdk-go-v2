...
func TestClient_XmlMapsXmlName_Serialize(t *testing.T) {
...
func BenchmarkClient_XmlMapsXmlName_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *XmlMapsXmlNameInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.XmlMapsXmlName(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_XmlMapsXmlName_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_XmlMapsXmlName_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.XmlMapsXmlName(context.Background(), &params)
			}
		})
	}
}
