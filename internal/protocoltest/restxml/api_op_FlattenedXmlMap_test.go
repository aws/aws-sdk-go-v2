...
func TestClient_FlattenedXmlMap_Serialize(t *testing.T) {
...
func BenchmarkClient_FlattenedXmlMap_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *FlattenedXmlMapInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.FlattenedXmlMap(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_FlattenedXmlMap_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_FlattenedXmlMap_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.FlattenedXmlMap(context.Background(), &params)
			}
		})
	}
}
