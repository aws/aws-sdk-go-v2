...
func TestClient_BodyWithXmlName_Serialize(t *testing.T) {
...
func BenchmarkClient_BodyWithXmlName_Serialize(b *testing.B) {
	cases := map[string]struct {
	Params *BodyWithXmlNameInput
	ExpectMethod string
	ExpectURIPath string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.BodyWithXmlName(context.Background(), c.Params)
			}
		})
	}
}

func TestClient_BodyWithXmlName_Deserialize(t *testing.T) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
...
func BenchmarkClient_BodyWithXmlName_Deserialize(b *testing.B) {
	cases := map[string]struct {
	StatusCode int
	Header http.Header
	BodyMediaType string
	...
	for name, c := range cases {
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				client.BodyWithXmlName(context.Background(), &params)
			}
		})
	}
}
