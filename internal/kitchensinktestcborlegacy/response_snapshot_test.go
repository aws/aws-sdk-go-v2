...
func TestCheckResponseSnapshot_CborGetItem(t *testing.T) {
	want := &CborGetItemOutput{}
	status, header, body, err := serdeRespReadSnapshot("CborGetItem.response")
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "CborGetItem.response", err)
	}
}
...
func TestCheckResponseSnapshot_CborPutCompressedData(t *testing.T) {
	...
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("response snapshot mismatch for %s: %v", "CborPutCompressedData.response", err)
	}
}

func TestCheckResponseSnapshot_Error_CborItemNotFound(t *testing.T) {
	want := &types.CborItemNotFound{}
	status, header, body, err := serdeRespReadSnapshot("CborItemNotFound.error")
	if errors.Is(err, fs.ErrNotExist) {
		t.Skip("no response snapshot fixture")
	}
	if err != nil {
	...
	var got *types.CborItemNotFound
	if !errors.As(opErr, &got) {
		t.Fatalf("expected types.CborItemNotFound, got %v", opErr)
	}
	if err := smithytesting.CompareValues(want, got); err != nil {
		t.Errorf("error response snapshot mismatch for %s: %v", "CborItemNotFound.error", err)
	}
}
